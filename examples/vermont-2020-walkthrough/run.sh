...
#   - BISECT_BUILD_COMMIT is read from the build (no override required)
#
...
step 1 "Verifying bisect binary on PATH..."
command -v bisect >/dev/null 2>&1 || fail "bisect not on PATH (run bootstrap.sh first)"
ok "bisect version: $(bisect --version 2>&1 | head -1)"

step 2 "Fetching Vermont 2020 Census TIGER tracts (FIPS 50)..."
bisect fetch --type tiger --states "$STATE" --year "$YEAR" \
...
# Adjacency is built implicitly by `bisect state` if not present;
# we materialize it here so its SHA can be checked-in.
bisect fetch --type adjacency --states "$STATE" --year "$YEAR" \
    --output-base "$OUTPUT_BASE" || true   # tolerate "already built"
ok "Adjacency present"

step 4 "Running bisection: bisect state --state $STATE --year $YEAR --label $LABEL"
bisect state --state "$STATE" --year "$YEAR" \
...
step 5 "Running analyses: bisect analyze --label $LABEL --types all"
bisect analyze --label "$LABEL" --year "$YEAR" --version "$VERSION" \
    --output-base "$OUTPUT_BASE" --types all || fail "analyze failed"
ok "Analyses complete"

step 6 "Generating report: bisect report --label $LABEL --format html"
bisect report --label "$LABEL" --year "$YEAR" --version "$VERSION" \
    --output-base "$OUTPUT_BASE" --format html || fail "report failed"
ok "Report generated"

step 7 "Verifying provenance: bisect doctor --verify-manifest"
MANIFEST="${OUTPUT_BASE}/${VERSION}/${YEAR}/plans/${LABEL}/manifest.json"
bisect doctor --verify-manifest "$MANIFEST" || fail "manifest verification failed"
