#!/usr/bin/env python3
"""Verify v2 custody without accepting any closed v1 completion."""
...
V1_PROTOCOL_ID = "nrs-v0.3-block-ensemble-expansion-v1"


def fail(message: str) -> None:
    raise ValueError(f"block ensemble expansion v2 verification failed: {message}")
...
def reject_v1_protocol_claims(package: Path) -> None:
    for path in package.rglob("*.json"):
        value = load(path)
        if value.get("protocol_id") == V1_PROTOCOL_ID:
            fail(f"v1 protocol artifact present: {path.name}")
...
def main() -> None:
    ...
    print(
        "block ensemble expansion v2 verification: PASS "
