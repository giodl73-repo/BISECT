#!/usr/bin/env python3
"""Verify and regenerate the NRS v0.3 cross-census DFS package."""
...
def main() -> None:
    ...
    if len(states) != 86 or len(nodes) != 770:
        fail("State or node universe")
    if states != sorted(states, key=lambda row: (int(row["year"]), row["state"])):
        fail("State order")
    if nodes != sorted(
        nodes,
        key=lambda row: (
    ...
    for row in nodes:
        ...
        if not (0 < partitions <= cut <= deviation):
            fail(f"candidate counts for {row['year']}/{row['state']}/{row['path']}")
        if row["assignment_match"] != "true" or row["objective_match"] != "true":
            fail(f"node preservation for {row['year']}/{row['state']}/{row['path']}")
        node_id = f"{row['year']}/{row['state']}/{row['path']}"
        if row["physical_cut_opportunity"] == "true":
    ...
    runner = root / "scripts/research/run_nrs_cross_census_dfs.py"
    ...
    print("NRS v0.3 cross-census complete-tree DFS verification: PASS")
