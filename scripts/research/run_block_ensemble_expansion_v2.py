#!/usr/bin/env python3
"""Run the frozen NH/NM/GA v2 schedule behind capacity admission."""
...
LEDGER_SCHEMA = "nrs-block-ensemble-expansion-ledger-v2"
RESOURCE_SCHEMA = "nrs-block-ensemble-expansion-resource-v2"
TRACE_SCHEMA = "nrs-block-ensemble-trace-v1"
BASE_SEED = 20260811
PROTOCOL = ROOT / "docs/specs/2026-08-11-nrs-v0.3-block-ensemble-expansion-v2.md"
...
def validate_ledger_identity(ledger: dict) -> None:
    if ledger.get("schema_version") != LEDGER_SCHEMA:
        raise ValueError("v2 ledger schema mismatch")
    if ledger.get("protocol_id") != PROTOCOL_ID:
        raise ValueError("v2 ledger protocol mismatch")
...
def validate_ledger(ledger: dict) -> None:
    ...
    if ledger.get("status") not in {"active", "complete", "failed"}:
        raise ValueError("v2 ledger status is invalid")
    ...
    if not isinstance(completed, dict) or set(completed) != {
        ...
        raise ValueError("v2 ledger completion phases mismatch")
    ...
    if ledger["status"] == "complete" and completed["replay"] != ORDER:
        raise ValueError("v2 ledger completed before all replays")
    if ledger["status"] == "active" and completed["replay"] == ORDER:
        raise ValueError("v2 ledger remained active after all replays")
    failures = ledger.get("failures")
    if not isinstance(failures, list):
        raise ValueError("v2 ledger failures are invalid")
    if ledger["status"] == "failed" and not failures:
        raise ValueError("failed v2 ledger has no failure")
    if ledger["status"] != "failed" and failures:
        raise ValueError("non-failed v2 ledger contains failures")
    if not isinstance(ledger.get("runner_wall_seconds"), (int, float)):
        raise ValueError("v2 runner wall total is invalid")
    if not isinstance(ledger.get("retained_bytes"), int) or isinstance(
        ledger.get("retained_bytes"), bool
    ):
        raise ValueError("v2 retained-byte total is invalid")
...
def validate_trace(trace: dict, state: str, sampler: str, phase: str) -> None:
    ...
    expected = {
        ...
        "status": "complete",
        "execution_class": (
            "governed-stage2-v2" if governed else "excluded-expansion-v2-preflight"
        ),
        "state": state,
        "year": 2020,
...
def next_admission_path(package: Path, state: str, sampler: str, phase: str) -> Path:
    ...
    for attempt in range(1, 10_000):
        ...
        if not candidate.exists():
            return candidate
    raise ValueError(f"v2 admission attempt namespace exhausted for {phase} {tag}")
...
def main() -> None:
    ...
    if expected_next(ledger, args.phase) != key:
        raise SystemExit(f"v2 schedule violation: {key} is not the next {args.phase} run")
    paths = artifact_paths(package, args.state, args.sampler, args.phase)
    for name in ("final_trace", "runner_trace", "resource"):
        if paths[name].exists():
            raise SystemExit(f"v2 {name} path already exists: {paths[name]}")
    ...
    if not admission["process_launch_authorized"]:
        raise SystemExit("v2 host-capacity admission rejected; no runner process created")
    ...
    if record["status"] != "pass":
        ...
        write_json(ledger_path, ledger)
        raise SystemExit(f"v2 expansion run failed: {reason}")
    ...
    try:
        ...
        write_json(paths["resource"], record)
        write_json(ledger_path, ledger)
        raise SystemExit(f"v2 expansion validation failed: {error}") from error
    ...
    write_json(paths["resource"], record)
    write_json(ledger_path, ledger)
    print(f"v2 {args.phase} {key}: PASS")
