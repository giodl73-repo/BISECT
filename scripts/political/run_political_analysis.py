#!/usr/bin/env python3
"""
Run political analysis on all states in a redistricting run.

This script:
1. Finds all state directories in the output
2. For each state, runs analyze_districts.py and visualize_partisan_lean.py
...
def main():
    parser = argparse.ArgumentParser(description='Run political analysis on all states')
    ...
    if not send_status:
        print(f"\nRunning political analysis on {len(state_dirs)} states...")
    ...
    pbar = tqdm(state_dirs, desc="Political analysis", unit="state", ncols=100, disable=send_status)

    for i, state_dir in enumerate(pbar):
        state_name = state_dir.name.replace('_', ' ').title()

        if show_progress:
            pbar.set_description(f"Political analysis: {state_name}")
        else:
            # Report to parent
        ...
        # Check if political analysis already exists
        political_dir = state_dir / 'political_analysis'
        analysis_file = political_dir / f'district_political_{args.election_year}.csv'
        maps_dir = political_dir / 'maps'

        if analysis_file.exists() and maps_dir.exists():
            # Skip if already done
            skipped.append(state_name)
            continue

        try:
            # Set up environment for child processes to report progress
            env = os.environ.copy()
            if send_status:
                env['TQDM_POSITION'] = str(position)

            # Run analyze_districts.py
            analyze_cmd = [
                sys.executable,
                str(scripts_dir / 'analyze_districts.py'),
                str(state_dir),
                '--year', args.election_year
            ]

            if send_status:
                # Don't capture output so STATUS messages flow through
                result = subprocess.run(analyze_cmd, env=env)
            else:
                # Capture output in standalone mode
            ...
            # Run visualize_partisan_lean.py
            visualize_cmd = [
                sys.executable,
                str(scripts_dir / 'visualize_partisan_lean.py'),
                str(state_dir),
                '--census-year', args.census_year,
                '--election-year', args.election_year
            ]

            if send_status:
                # Don't capture output so STATUS messages flow through
                result = subprocess.run(visualize_cmd, env=env)
            else:
                # Capture output in standalone mode
    ...
    # Report final status
    if send_status:
        report_progress(f"Political Analysis - {len(successful)} analyzed, {len(skipped)} skipped, {len(failed)} failed")
    else:
        # Summary (standalone mode only)
        print(f"\n{'='*70}")
        print(f"POLITICAL ANALYSIS COMPLETE")
        print(f"{'='*70}")
        print(f"Analyzed: {len(successful)} states")
        print(f"Skipped: {len(skipped)} states (already done or no redistricting data)")
