...
\textbf{A critical design principle}: The algorithm has \textit{zero knowledge} of voting patterns, party affiliation, or electoral outcomes. Political data is not an input to the boundary-drawing process. The recursive bisection algorithm sees only two things: census tract populations and geographic adjacency. It cannot gerrymander because it cannot see what it would need to manipulate.

This section analyzes the partisan characteristics that emerge when we overlay election results onto the completed districts \textit{after} boundaries are drawn. Any partisan patterns in the results reflect underlying political geography, not algorithmic manipulation. The algorithm's inherent blindness to politics makes gerrymandering structurally impossible.
\subsection{Political Lean Distribution}

We overlay 2020 presidential election results onto the algorithmically-generated districts to assess partisan characteristics \cite{ansolabehere2012mecubed}. For each district, we calculate the Democratic margin (Biden vote share minus Trump vote share) and classify districts into five categories:
...
\subsection{Comparison to Actual Districts}

Actual 2022 congressional maps contain similar partisan distributions but achieve this through intentional considerations—gerrymandering or bipartisan incumbent protection. Our algorithm achieves comparable outcomes without partisan input, suggesting political geography constrains results more than drawing method. The efficiency gap matches patterns in independent commission states \cite{chen2013unintentional}, indicating our approach approximates neutral commissions with greater reproducibility.
