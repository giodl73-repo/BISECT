...
\textbf{Water-Based Adjacency:} Island tracts with no land-based connections receive water-based adjacency edges to the nearest tract within the same county, where "nearest" is defined by centroid-to-centroid Euclidean distance after projection to state-specific coordinate reference systems (e.g., California Albers EPSG:3310, Texas Conic EPSG:3083). We impose a maximum distance threshold of 1 km to prevent unreasonable water crossings. This ensures graph connectivity while respecting county boundaries. Without this modification, island populations would form disconnected components, preventing contiguous district creation.
