"""
L2 tests for bisect_py.adjacency_to_bin / adjacency_from_bin serialization.
"""
...
class TestAdjacencySerialize:
    ...
    def test_roundtrip_adjacency(self):
        data = bisect_py.adjacency_to_bin(ADJ, EW, N, E)
        g = bisect_py.adjacency_from_bin(list(data))
    ...
    def test_invalid_magic_raises(self):
        data = bytearray(bisect_py.adjacency_to_bin(ADJ, EW, N, E))
        data[0] = ord('X')  # corrupt magic
        with pytest.raises(ValueError, match='magic'):
            bisect_py.adjacency_from_bin(list(data))

    def test_truncated_raises(self):
        data = bisect_py.adjacency_to_bin(ADJ, EW, N, E)
        with pytest.raises(ValueError):
            bisect_py.adjacency_from_bin(list(data[:8]))

    def test_empty_graph_roundtrip(self):
        data = bisect_py.adjacency_to_bin([[], []], {}, 2, 0)
        g = bisect_py.adjacency_from_bin(list(data))
    ...
    def test_large_graph_roundtrip(self):
        ...
        n_e = len(ew)
        data = bisect_py.adjacency_to_bin(adj, ew, n, n_e)
        g = bisect_py.adjacency_from_bin(list(data))
        assert g['n_vertices'] == n
