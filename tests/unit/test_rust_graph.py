"""
Phase 0 exit criterion: verify bisect_py.Graph round-trips through PyO3.

Skipped when BISECT_NO_RUST=1 (CI without Rust compiled) or when the
bisect_py wheel hasn't been built yet (maturin develop not run).

To run locally after building:
    cd python/bisect_py && maturin develop && cd ..
...
pytestmark = pytest.mark.skipif(
    ...
    reason='bisect_py not available (set BISECT_NO_RUST=0 and run maturin develop)'
)
...
class TestGraphRoundTrip:
    ...
    def test_n_vertices(self):
        g = bisect_py.Graph.from_csr(SMALL_CSR)
    ...
    def test_rejects_2d_vertex_weights(self):
        ...
        with pytest.raises(ValueError, match='2D'):
            bisect_py.Graph.from_csr(bad)

    def test_empty_graph_raises(self):
        with pytest.raises(ValueError):
            bisect_py.Graph.from_csr({'adjacency': [], 'vertex_weights': [], 'n_vertices': 0})
...
class TestPartitionRoundTrip:
    ...
    def test_to_dict_round_trips(self):
        ...
        p = bisect_py.Partition.from_dict(assignments)
    ...
    def test_assert_balanced_fails(self):
        ...
        p = bisect_py.Partition.from_dict({0: 0, 1: 0, 2: 1, 3: 1, 4: 1})
    ...
    def test_assert_balanced_passes(self):
        ...
        p = bisect_py.Partition.from_dict({0: 0, 1: 0, 2: 0, 3: 1, 4: 1})
    ...
    def test_assert_balanced_default_tolerance(self):
        ...
        p = bisect_py.Partition.from_dict({0: 0, 1: 0, 2: 1, 3: 1})
...
class TestVraEdgeWeights:
    ...
    def test_minority_minority_edge_boosted(self):
        ...
        edges = [(0, 1), (1, 2)]
        weights = bisect_py.build_vra_edge_weights(edges, minority_fracs, threshold=0.40)
    ...
    def test_alpha_formula_correctness(self):
        ...
        edges = [(0, 1)]
        weights = bisect_py.build_vra_edge_weights(edges, minority_fracs)
        alpha = weights[(0, 1)]
    ...
    def test_default_threshold_is_040(self):
        ...
        weights_default = bisect_py.build_vra_edge_weights(edges, minority_fracs)
        assert (0, 1) not in weights_default

        # With threshold=0.38: both qualify → boosted
        weights_low = bisect_py.build_vra_edge_weights(edges, minority_fracs, threshold=0.38)
