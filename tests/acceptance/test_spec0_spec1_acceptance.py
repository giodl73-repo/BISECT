...
--force, --population-source flags and the `BISECT validate` command.

Requirements:
  - `BISECT` binary must be on PATH or at target/release/bisect
  - VT adjacency data must be present (from `BISECT fetch`)
...
def run_bisect(args: list[str], check=True, capture=True) -> subprocess.CompletedProcess:
...
def has_vt_adjacency() -> bool:
    ...
    try:
        result = run_bisect(["state", "--state", "VT", "--year", "2020",
...
class TestSpec1CustomParameters:
    ...
    def test_chamber_flag_recorded_in_manifest(self, tmp_BISECT_output):
        ...
        manifest_path = tmp_BISECT_output / "2020" / "plans" / "vt_house_accept" / "manifest.json"
        assert manifest_path.exists(), f"manifest not found at {manifest_path}"
        manifest = json.loads(manifest_path.read_text())
        assert manifest["chamber"] == "house"
        assert manifest["num_districts"] == 1

    def test_balance_tolerance_in_manifest(self, tmp_BISECT_output):
        ...
        if result.returncode != 0:
            pytest.skip(f"VT data not available: {result.stderr[:200]}")
        manifest = json.loads(
            (tmp_BISECT_output / "2020" / "plans" / "vt_tol_accept" / "manifest.json").read_text()
        )
        assert abs(manifest["balance_tolerance_pct"] - 5.0) < 0.01

    def test_population_source_in_manifest(self, tmp_BISECT_output):
        ...
        if result.returncode != 0:
            pytest.skip(f"VT data not available: {result.stderr[:200]}")
        manifest = json.loads(
            (tmp_BISECT_output / "2020" / "plans" / "vt_pop_accept" / "manifest.json").read_text()
        )
        assert manifest["population_source"] == "total"

    def test_label_collision_exits_nonzero_without_force(self, tmp_BISECT_output):
        ...
        result1 = run_bisect(common_args, check=False)
        if result1.returncode != 0:
            pytest.skip(f"VT data not available: {result1.stderr[:200]}")
        result2 = run_bisect(common_args, check=False)
    ...
    def test_force_flag_allows_second_run(self, tmp_BISECT_output):
        ...
        if result1.returncode != 0:
            pytest.skip(f"VT data not available: {result1.stderr[:200]}")
        result2 = run_bisect(common_args + ["--force"], check=False)
...
class TestSpec0RplanFormat:
    ...
    def test_manifest_required_fields_present(self, tmp_BISECT_output):
        ...
        if result.returncode != 0:
            pytest.skip(f"VT data not available: {result.stderr[:200]}")
        manifest = json.loads(
            (tmp_BISECT_output / "2020" / "plans" / "vt_fields_accept" / "manifest.json").read_text()
        )
        required_fields = [
    ...
    def test_manifest_adjacency_file_is_filename_not_path(self, tmp_BISECT_output):
        ...
        if result.returncode != 0:
            pytest.skip(f"VT data not available: {result.stderr[:200]}")
        manifest = json.loads(
            (tmp_BISECT_output / "2020" / "plans" / "vt_adj_accept" / "manifest.json").read_text()
        )
        adj = manifest["adjacency_file"]
        assert "/" not in adj and "\\" not in adj, \
            f"adjacency_file must be filename only, got: {adj!r}"

    def test_manifest_tiger_source_url_is_census_gov(self, tmp_BISECT_output):
        ...
        if result.returncode != 0:
            pytest.skip(f"VT data not available: {result.stderr[:200]}")
        manifest = json.loads(
            (tmp_BISECT_output / "2020" / "plans" / "vt_tiger_accept" / "manifest.json").read_text()
        )
        assert "tiger_source_url" in manifest
        assert "census.gov" in manifest["tiger_source_url"]

    def test_manifest_binary_download_url_is_github(self, tmp_BISECT_output):
        ...
        if result.returncode != 0:
            pytest.skip(f"VT data not available: {result.stderr[:200]}")
        manifest = json.loads(
            (tmp_BISECT_output / "2020" / "plans" / "vt_gh_accept" / "manifest.json").read_text()
        )
        assert "github.com" in manifest["binary_download_url"]

    def test_manifest_population_balance_valid_is_true(self, tmp_BISECT_output):
        ...
        if result.returncode != 0:
            pytest.skip(f"VT data not available: {result.stderr[:200]}")
        manifest = json.loads(
            (tmp_BISECT_output / "2020" / "plans" / "vt_bal_accept" / "manifest.json").read_text()
        )
        assert manifest["population_balance_valid"] is True


# ---------------------------------------------------------------------------
# BISECT validate command acceptance tests
# ---------------------------------------------------------------------------

class TestRedistValidateCommand:

    def test_validate_passes_on_valid_rplan(self, tmp_path):
        """BISECT validate exits 0 for a valid .rplan file."""
        rplan_data = {
            "rplan_version": "0.1",
            "metadata": {
                "label": "vt_test",
    ...
    def test_validate_shows_tract_and_district_counts(self, tmp_path):
        """BISECT validate output includes tract count and district count."""
        rplan_data = {
            "rplan_version": "0.1",
            "metadata": {
                "label": "count_test",
