...
- **Typst version pins** at `redist/crates/bisect-report/typst-templates/.typst-version` (0.12.0) + `.verapdf-version` (1.26.2). Templates README documents the integration path the next session will execute.
...
- **JSON Schema**: `redist/crates/bisect-analysis/schemas/bloc_voting.schema.json` validates outputs.
...
- **`redist doctor --check-tutorial-data`**: drift detection against `examples/{tutorial}-walkthrough/checksums.json` (schema `tutorial-checksums v1`). Per-row PASS/FAIL/MISSING; exit 0 only if no FAIL. 8 L0 tests in `redist/crates/bisect-cli/src/doctor.rs`. See `docs/REDIST_CLI.md`.
