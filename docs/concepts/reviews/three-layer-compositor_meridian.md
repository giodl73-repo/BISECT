...
The METIS backend section would benefit from noting that a verifier using the `bisect-metis` (pure Rust) engine may obtain different local optima from `c-ffi` even with the same seed, since the two implementations may differ in tie-breaking. This matters for statutory certification.
