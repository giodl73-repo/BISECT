...
**P2 — No `redist mv X Y` verb.** `bisect label X Y` copies the registry entry but does not rename `runs/X/` to `runs/Y/` or update index files that contain `"label": "X"`. This is not a rename — it is an alias. If an operator wants to rename `senate_draft2` to `senate_final`, there is no path. Add `redist mv X Y` that renames directories, updates index `"label"` fields, and updates the registry atomically. Without it, operators will do this by hand and corrupt the audit chain.
