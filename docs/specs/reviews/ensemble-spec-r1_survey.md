...
- The SMC standalone design (`bisect ensemble --method smc` rather than `bisect state --search smc`) is correct. SMC generates a full posterior sample over plans, not a single plan, and forcing it through the `bisect state` pipeline would obscure its output semantics.
...
```bash
bisect state --search bisection-ensemble --percentile 0.5 --ensemble-steps 500
bisect state --search short-burst --burst-length 20 --n-bursts 50
```
