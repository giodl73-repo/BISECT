...
| S4 Work Package Execution | complete_l0_pass_with_risk | WP-001..WP-008 satisfied; DCRs partially executed | Preserve closed work-package evidence and do not convert L1 DCR checklists into release/readiness claims without the named L2 evidence. |
| S5 Integration | not_started | partially unblocked for L1 fixture/contract scope; blocked for L2 release scope | Integrate package outputs, trace evidence, verification results, public evidence status, and any DCR closure evidence selected for release scope. |
| S6 Readiness / Transition | not_started | blocked until S5 integration and required DCR gates pass | Decide release/readiness posture and downstream adoption guidance. |
...
| S4 | Work-package evidence, changed files, verification commands, review findings, filed DCRs | L0 / L1 | Package-specific lanes per work package | WP-001 evidence: `TRACE.md`, `WORK_PACKAGES.md`, `IMPLEMENTATION_PLAN.md`; WP-002 evidence now includes `repo-map.toml`, `tools/repo_map.py`, `docs/REPO_MAP_STANDARD.md`, generated Cargo patches, Cargo metadata pass, CLI help pass, dual CLI-surface inventory, package-family schema version/draft inventory, implemented/stubbed adapter inventory, and Rust-gap follow-ups implementing RPLAN and direct shapefile label import; WP-003 evidence includes label build-index provenance fields, per-state executable hashing, label verification SHA-chain inspection, and historical replay-gap disposition; WP-004 evidence includes RPLAN/RCOUNT/RCTX/RHIST model inspection, package-family library tests, package CLI tests, RPLAN public audit fixtures, and explicit external-fixture risk for RCOUNT/RHIST; WP-005 evidence includes README and paper-index claim-boundary edits, public-claim pattern inspection, and L1/L2 quantitative/public-release risk disposition; WP-006 evidence includes `.gitignore` inspection, tracked/local artifact inventory, data/output policy review, embedded manifest repo correction, archive boundary review, and release-custody risk disposition; WP-007 evidence includes wave index and pulse sample inspection plus `context/waves/PHASES.md` closure-rule updates; WP-008 evidence includes CLI reference and quickstart repair for current label-pipeline workflows; release-readiness follow-ups are filed as DCR-001 through DCR-007 in `docs/vtrace/DCRS.md`; DCR execution artifacts now include public import fixtures, import matrix, smoke-bundle contract, external-walkthrough record, evidence-package contract/checklist, court-packaging boundary/checklist, and smoke-only reproducibility record. |
| S5 | Integration trace, cross-package evidence, validation results, claim/custody review | L2 where applicable | Integration and public evidence lanes | Future integration record. |
| S6 | Readiness decision, transition notes, release/publication disposition | L2 where applicable | Maintainers and external-user readiness lanes | Future readiness record. |
...
| No currently active wave exists in the wave index. | S4 / WP-007 | BENCHMARK / TRENCH | Accept L0 process-standard closure with risk; first future VTRACE-governed pulse must provide L1 evidence that the checklist works in practice. |
...
| Release-readiness residuals need controlled follow-up records. | S4 / S5 / S6 | Maintainers / role lanes | Filed DCR-001 through DCR-007 and executed artifacts for public import fixtures, release smoke scope, external-user walkthrough simulation, evidence package contract/checklist, import matrix, court/legal packaging boundary/checklist, and smoke-only reproducibility. DCR-001 is closed at L2, while DCR-004, DCR-005, and DCR-006 are closed at L1; DCR-002, DCR-003, and DCR-007 still block stronger release-health, external-user, and full-reproducibility claims as recorded in `DCRS.md`. |
