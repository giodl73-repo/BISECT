...
| EVID-CR-011 | CR-006, CR-007, CR-012, CR-013 | DCR-007 release-subset replay capture harness and preflight contract. | tooling_ready | `scripts/maintenance/dcr007_release_subset_replay.py` records environment, source-clean policy, config/data hashes, command outputs, and artifact hashes for a declared release-subset replay. It is tooling and preflight evidence only until run from a clean data-backed checkout and reviewed as DCR-007 L2 evidence. |
| EVID-CR-012 | CR-009, CR-011, CR-012 | DCR-003 external-operator walkthrough packet. | ready_for_external_run | `docs/vtrace/EXTERNAL_WALKTHROUGH.md` now defines the L2 external-operator packet, declared-path fields, observer task script, observation-record template, friction taxonomy, and promotion rule. It is not L2 user evidence until a real non-author completes the walkthrough and COMMONS/operator-review dispositions friction. |
