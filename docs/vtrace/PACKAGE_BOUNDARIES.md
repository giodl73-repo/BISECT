...
| PKG-010 | `crates/bisect-cli`, `crates/bisect-tui` | Rust | CLI/UI owners | User-facing BISECT commands, interactive views, and orchestration. Dashboard generation remains Python-owned until a Rust web implementation has a concrete slice. Local cross-repo dependencies must be recorded in `repo-map.toml`. | IF-001, IF-006 | Analysts, researchers, maintainers, reviewers | accepted |
