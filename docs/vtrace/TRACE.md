...
Entry point: use `INDEX.md` for the VTRACE reading order and current claim
boundaries before following individual trace rows.
...
| First live VTRACE-governed wave | `context/waves/2026-06-01-vtrace-baseline-maintenance/WAVE.md`; `context/waves/2026-06-01-vtrace-baseline-maintenance/CLOSE.md`; `context/waves/PHASES.md` | DREQ-003 selection and closure evidence exists for internal baseline maintenance; it does not upgrade release readiness. |
...
| Package spec register | `PACKAGE_SPEC_REGISTER.md`; PKG-004, PKG-005, PKG-006; IF-003, IF-004, IF-005; REQ-013 through REQ-023 | DREQ-004 control evidence exists for package schema/canonicalization routing; exact schemas remain owned by package specs and crate constants. |
| Communications strategy | `COMMUNICATIONS_STRATEGY.md`; REQ-005, REQ-010, REQ-024 through REQ-027, REQ-032, REQ-035; DCR-003, DCR-004, DCR-006, DCR-007 | Audience, channel, message-class, blocked-language, claim-packet, and review-lane controls exist for public/operator communications; it does not publish a release or upgrade S6 posture. |
| Communications implementation audit | `COMMUNICATIONS_IMPLEMENTATION_AUDIT.md`; README; `docs/BISECT_CLI.md`; `docs/quickstart/`; `docs/legal/`; `research/journals/`; `docs/superpowers/` | L1 evidence exists that current public/operator docs were searched against strategy-blocked wording and remediated where unsupported court-ready language appeared. It does not close DCR-003, DCR-004, DCR-006, or DCR-007. |
