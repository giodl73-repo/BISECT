...
**Goal:** Ship a `BISECT deposition-server` long-running daemon plus a `BISECT depo` family of subcommands that lets a §2 expert or special master answer parametric "what-if" questions in 1-2 seconds during a deposition, with a tamper-evident audit trail and provenance binding back to the original report.
...
- B-07 (`BISECT_BUILD_COMMIT` env override for build-commit spoof)
...
- `BISECT/crates/bisect-cli/src/provenance.rs` exposes `BUILD_COMMIT` and `Provenance::current()` (already true).
- `BISECT/crates/bisect-cli/build.rs` populates `BISECT_BUILD_COMMIT` from `git rev-parse HEAD` (already true). This plan extends the build script to honor an env-var override.
...
- [ ] **1.1** Create `docs/parameters/whitelist-dependencies.md` with the schema:
  ...
  Read-side: `BISECT depo eval` consults this map to decide which analyzers re-run
...
- [ ] **1.4** Cross-link from the spec (`docs/superpowers/specs/2026-04-30-deposition-prep.md` §"Implementation notes") and from `docs/BISECT_CLI.md` `BISECT depo` section.
...
## Task 2: One-shot `BISECT depo recompute` subcommand

**Files:** `BISECT/crates/bisect-cli/src/depo/mod.rs` (new module), `BISECT/crates/bisect-cli/src/depo/recompute.rs` (new), `BISECT/crates/bisect-cli/src/depo/whitelist.rs` (new), `BISECT/crates/bisect-cli/src/args.rs` (extend), `BISECT/crates/bisect-cli/src/main.rs` (wire), `BISECT/crates/bisect-cli/src/lib.rs` (re-export)
...
- [ ] **2.6** `manifest.json` shape mirrors `narrative_manifest.json` from Plan Comparison so consumers can read both with one parser. Required fields:
  ```json
  {
    ...
    "BISECT_version": "...", "BISECT_build_commit": "...", "rustc_version": "...",
    "generated_at": "2026-04-30T14:22:08Z",
    "note": "..." | null
  }
  ```
- [ ] **2.7** L0 tests in `BISECT/crates/bisect-cli/tests/depo_recompute.rs`:
...
**Exit:** `BISECT depo recompute --plan-label vt_2020 --param leaning_threshold=0.53` produces an output dir whose manifest binds back to the parent plan SHA, and the whitelist gate works.

---

## Task 3: IPC abstraction layer (PP-26)

**Files:** `BISECT/crates/bisect-cli/src/depo/ipc.rs` (new), `BISECT/crates/bisect-cli/Cargo.toml` (add dependency)
...
- [ ] **3.2** Define `enum IpcMode { UnixSocket(PathBuf), WindowsPipe(String) }` with platform-appropriate `Default::default()` resolution per spec:
  - Unix: `${XDG_RUNTIME_DIR:-/tmp}/BISECT-depo-${USER}.sock`. Permissions on creation: `0600` (only the user).
  - Windows: `\\.\pipe\BISECT-depo-${USERNAME}` (use `GetUserNameW`; fall back to `BISECT-depo-default`).
...
- [ ] **3.5** PID file at `${ipc_path}.pid` (Unix) or `%LOCALAPPDATA%\BISECT\depo-${USERNAME}.pid` (Windows) — written atomically (write-then-rename), removed on graceful shutdown. Liveness check: `is_pid_alive(pid: u32) -> bool` using `kill(pid, 0)` on Unix and `OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, pid)` on Windows.
...
## Task 4: Daemon scaffolding — `BISECT deposition-server`

**Files:** `BISECT/crates/bisect-cli/src/depo/server.rs` (new), `BISECT/crates/bisect-cli/src/depo/protocol.rs` (new), `BISECT/crates/bisect-cli/src/depo/state.rs` (new — in-memory plan state)
...
- [ ] **4.3** `server.rs::run_server(args: DepoServerArgs) -> anyhow::Result<()>`:
  ...
  2. `detect_stale` -> unlink if stale; refuse to start if `Live` (clear error: "another daemon is already serving on `<path>`; use `BISECT depo stop` first").
...
**Exit:** `BISECT deposition-server --plan-label vt_2020 --year 2020` warm-starts in ≤5s on VT, prints `READY`, and accepts an `eval` request from a separately-launched `BISECT depo eval` client.

---

## Task 5: Two-phase shutdown + canonical JSONL log + hash chain (PP-24, C-01)

**Files:** `BISECT/crates/bisect-cli/src/depo/log.rs` (new), `BISECT/crates/bisect-cli/src/depo/canonical.rs` (new — canonical JSON helpers)
...
- [ ] **5.7** L1: `daemon_log_replay` (10 evals; `BISECT depo log` reads them all back; SHA chain verifies). L0: `canonical_json_golden` (round-trip a known struct against a hex-pinned canonical string). L0: `log_chain_tamper_detected` (mutate one byte in entry #5; `verify-log` returns non-zero with the offending seq).
...
**Files:** `BISECT/crates/bisect-cli/src/depo/client.rs` (new), `BISECT/crates/bisect-cli/src/depo/{eval,sweep,log,verify_log,stop}.rs` (new)
...
- [ ] **6.2** `eval.rs::run_eval(args: DepoEvalArgs)`: parse `--param`, `--types`, `--note`; resolve `IpcMode` the same way the daemon did (use the `--socket`/`--pipe-name` discovery rules); send `{"op":"eval", ...}`; print response. If no daemon answers, the error is "no daemon found at `<path>`; start one with `BISECT deposition-server --plan-label LABEL`".
...
**Exit:** A user can run `BISECT deposition-server --plan-label vt_2020` in one terminal and `BISECT depo eval --param leaning_threshold=0.53` in another, getting sub-second responses.

---

## Task 7: Provenance binding + `--enforce-build-commit` default-on under `--case-mode` (BD-N2)

**Files:** `BISECT/crates/bisect-cli/src/depo/provenance.rs` (new — depo-specific), `BISECT/crates/bisect-cli/src/depo/server.rs` (extend)
...
- [ ] **7.5** L0: `enforce_build_commit_blocks_mismatch` — daemon started with `--enforce-build-commit` against a manifest whose `binary_build_commit` differs from `provenance::BUILD_COMMIT`; assert exit code 2 + actionable error. Uses `BISECT_BUILD_COMMIT` env override (Task 8) to spoof the running binary's commit.
- [ ] **7.6** L0: `case_mode_default_on` — invoke `BISECT deposition-server --case-mode --plan-label X` against a mismatched manifest WITHOUT explicitly passing `--enforce-build-commit`; assert it still blocks. Inverse: with `--case-mode --no-enforce-build-commit`, it warns but starts.
...
## Task 8: `BISECT_BUILD_COMMIT` env override for tests (B-07)

**Files:** `BISECT/crates/bisect-cli/build.rs` (extend), `docs/error-conventions.md` (one-line addition)

- [ ] **8.1** Extend `build.rs` to honor an optional env var: if `BISECT_BUILD_COMMIT_OVERRIDE` is set at build time, use its value instead of `git rev-parse HEAD`. This is the **build-time override** for reproducible-build pinning.
- [ ] **8.2** Separately, expose a **runtime override** for tests: `provenance::current_for_test()` (cfg-gated to `#[cfg(test)]` + a public escape hatch behind `cfg(feature = "test-provenance-override")`) reads `BISECT_BUILD_COMMIT` from the env at runtime and substitutes for `BUILD_COMMIT`. The L0 tests in Task 7 use this. **Production binaries do not consult the runtime override** — the feature is off in release builds.
...
- [ ] **8.4** L0 unit test in `BISECT/crates/bisect-cli/tests/build_commit_override.rs`: build with `BISECT_BUILD_COMMIT_OVERRIDE=deadbeef0000`, assert `provenance::BUILD_COMMIT == "deadbeef0000"`. Runtime override test sets `BISECT_BUILD_COMMIT=cafef00d0000` before invoking the daemon and asserts the daemon's logged commit matches.
...
**Files:** `BISECT/crates/bisect-cli/benches/depo_p99.rs` (new — Criterion benchmark), `.github/workflows/pr.yml` (extend), `.github/workflows/nightly.yml` (extend), `docs/superpowers/specs/2026-04-30-roadmap-five-star.md` (already documents the runner class — confirm it; do NOT modify)
...
- [ ] **10.1** Notebook with cells:
  ...
  2. Start daemon (`BISECT deposition-server --plan-label vt_2020 --case-mode &` via `subprocess.Popen`; wait for `READY`).
  ...
  6. Final cell: graceful `BISECT depo stop`.
- [ ] **10.2** Notebook declares `runtime_budget_secs: 120` in cell metadata (per roadmap CI strategy nightly notebook execution).
- [ ] **10.3** Cross-link from `docs/quickstart/quickstart-callais-expert.md` and `docs/BISECT_CLI.md`.
...
**Files:** `docs/BISECT_CLI.md` (extend), `docs/superpowers/specs/2026-04-30-roadmap-five-star.md` (one cell edit), `CLAUDE.md` (entry), `docs/CHANGELOG.md` (entry), `docs/file-formats/manifests.md` (extend — written by Court Reports plan as a followup-doc; this plan adds the `whatif-manifest v1` row)

- [ ] **11.1** **M-05**: edit the persona table in `docs/superpowers/specs/2026-04-30-roadmap-five-star.md`. The "§2 plaintiff's expert post-Callais" row's "What's missing" cell currently lists evidence layer items only. Append: "fast iterative re-analysis at trial (`BISECT depo`)". This is a pure doc edit; no other roadmap text changes.
- [ ] **11.2** Add a `BISECT depo` section to `docs/BISECT_CLI.md` covering all six subcommands with example invocations and expected output snippets. Include the platform-flag note (Unix `--socket`, Windows `--pipe-name`). Document `--case-mode` and the build-commit gate.
...
- [ ] **11.4** **M-04**: confirm `override_path_relative` in the depo manifest is relative to repo root (not absolute); add an explicit assertion test under `BISECT/crates/bisect-cli/tests/depo_path_portable.rs`.
...
- `BISECT depo recompute` runs one-shot with overrides and writes a `what_if/.../manifest.json` whose `parent_plan_manifest_sha256` matches the parent plan.
- `BISECT deposition-server --plan-label vt_2020` warm-starts in ≤5s for VT (≤30s for LA — nightly only).
- `BISECT depo eval` p99 ≤ 1.5s across the eight default whitelist parameters on the VT fixture (PR-gated, runner pinned per CI strategy).
...
- `BISECT depo verify-log` detects single-byte tampering at any seq position.
...
- `BISECT_BUILD_COMMIT_OVERRIDE` (build-time) and `BISECT_BUILD_COMMIT` (runtime, feature-gated) both work; documented in `docs/error-conventions.md`.
