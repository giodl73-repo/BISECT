...
- `BISECT doctor --verify-manifest` (already shipped) for the rebuild-instructions block.
...
- Existing HTML report path (`BISECT report --format html`) generates a clean `Report` struct via `BISECT_report::assemble_report` covering the analysis JSONs the PDF template will pull from.
...
- `BISECT doctor --verify-manifest` is callable from a script and exits 0 when manifest entries hash-match.
...
**Files:** `BISECT/rust-toolchain.toml` (no change), `docs/REPRODUCIBLE_BUILD.md`, `.github/workflows/*.yml`, `BISECT/crates/bisect-report/typst-templates/.typst-version` (new)
...
**Files:** `BISECT/crates/bisect-report/Cargo.toml`, `BISECT/crates/bisect-report/src/lib.rs`, `BISECT/crates/bisect-report/src/typst_render.rs` (new), `BISECT/crates/bisect-report/typst-templates/` (new directory tree), `BISECT/crates/bisect-cli/src/report_cmd.rs`
...
- [ ] **4.2** Delete `report_cmd::try_generate_pdf` and the wkhtmltopdf/pandoc fallback. `report_cmd.rs` for the PDF format now calls into `BISECT_report::typst_render::render_pdf` directly.
...
**Exit:** `BISECT report --format pdf --label vt_test` produces `vt_test_report.pdf` plus a `vt_test_report.typ` sidecar via Typst; legacy wkhtmltopdf/pandoc paths are gone.

---

## Task 5: Per-section template authoring — required content

**Files:** `BISECT/crates/bisect-report/typst-templates/_*.typ`
...
**Files:** `BISECT/crates/bisect-report/src/typst_render.rs`, `BISECT/crates/bisect-report/src/repro_zip.rs` (new), `BISECT/crates/bisect-report/typst-templates/_reproducibility.typ`
...
**Files:** `BISECT/crates/bisect-report/src/typst_render.rs`, `BISECT/crates/bisect-report/src/verapdf.rs` (new)
...
**Files:** `BISECT/crates/bisect-cli/src/args.rs`, `BISECT/crates/bisect-cli/src/report_cmd.rs`, `BISECT/crates/bisect-report/src/typst_render.rs`
...
**Files:** `BISECT/crates/bisect-report/tests/`, `tests/acceptance/test_court_report_*.py`
...
- [ ] **9.3** **B-01 P0a — value-correctness via PDF text extraction.** Render Vermont fixture; extract text via `pdftotext` (Poppler). Assert hand-computed numbers from the methodology fixture appear verbatim:
  ...
  Hand-computed numbers are committed under `BISECT/crates/bisect-report/tests/fixtures/court_report/expected_values.toml` so the assertions read the expected from disk, keeping the tests easy to re-pin when fixtures change.
...
- [ ] **9.5** **B-01 P0b — deliberately-malformed Typst output fails verapdf.** Negative test: a fixture template `tests/fixtures/court_report/_broken.typ` references an unembedded font OR removes `pdf-version: "PDF/A-2b"` from the document set; render via `BISECT report --format pdf`; assert exit non-zero, assert `[VERAPDF]` category, assert the failed rule ID is in stderr, assert no `.pdf` was written to the output dir (Task 7.2 invariant).
...
**Files:** `docs/BISECT_CLI.md`, `examples/louisiana-callais-walkthrough/expert_report_example/` (new), `CLAUDE.md`, `docs/CHANGELOG.md`

- [ ] **10.1** Document every new flag from Task 4 in `docs/BISECT_CLI.md` under `BISECT report`. Cross-link to `docs/file-formats/manifests.md` and `docs/file-formats/citation-strings.md`.
...
- `BISECT report --format pdf --label vt_test` produces `expert_report.pdf` (PDF/A-2b validated by verapdf), `expert_report.typ` (debugging source), and `reproducibility_package.zip` in under 10 seconds wall-clock.
...
- `docs/BISECT_CLI.md` documents every new flag; `CLAUDE.md` and `docs/CHANGELOG.md` reflect the addition.
...
- Replacing the existing `BISECT report --format html` path — HTML stays as the iteration / draft surface.
