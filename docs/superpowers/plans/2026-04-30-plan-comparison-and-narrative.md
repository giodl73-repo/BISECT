...
- `BISECT compare --plan-a A --plan-b B` already produces `PlanComparison` (Jaccard/population/compactness/splits) via `BISECT_analysis::compare_plans`
...
- `SOURCE_DATE_EPOCH` and `BISECT_BUILD_COMMIT_SHORT` envs are honored project-wide (BENCHMARK P1 patch in spec)
...
**Files:** `BISECT/crates/bisect-cli/src/args.rs`
...
**Exit:** `cargo test -p bisect-cli args::tests` green; `BISECT compare --help` shows the new surface.

---

## Task 2: Comparison report assembler crate module

**Files:** `BISECT/crates/bisect-report/src/comparison.rs` (new), `BISECT/crates/bisect-report/src/lib.rs`
...
- [ ] **2.1** Define `ComparisonReport` struct: `plan_a: PlanSide`, `plan_b: PlanSide`, `baseline: Option<PlanSide>`, `diff: DiffSummary`, `comments: Option<CommentsOverlay>`, `narrative: NarrativeBlock`, `manifest: NarrativeManifest`, `generated_at`, `BISECT_build_commit_short`. Each `PlanSide` has `label`, `manifest_sha256`, `partisan`, `vra`, `compactness`, `population`, plus `submission_type: Option<String>` (passthrough from plan manifest for civic-counter-proposal detection).
...
- [ ] **2.3** `DiffSummary` computes `tracts_changed`, `population_changed`, `districts_with_changes`, `tract_destinations: HashMap<String, (usize, usize)>` (GEOID → (district_in_a, district_in_b)). Population sourced from per-tract population in the existing adjacency/tract data path; reuse `BISECT_data` accessors.
...
**Files:**
- `BISECT/crates/bisect-report/templates/comparison-narrative.j2.md` (new)
- `BISECT/crates/bisect-report/templates/comparison-side-by-side.j2.html` (new)
- `BISECT/crates/bisect-report/src/narrative.rs` (new)
...
**Files:** `BISECT/crates/bisect-report/src/narrative.rs`, `BISECT/crates/bisect-report/src/moe.rs` (new)
...
**Files:** `BISECT/crates/bisect-report/src/narrative.rs`, `BISECT/crates/bisect-cli/src/compare.rs`
...
**Files:** `BISECT/crates/bisect-map/src/diff_map.rs` (new), `BISECT/crates/bisect-map/src/lib.rs`
...
**Files:** `BISECT/crates/bisect-report/templates/comparison-side-by-side.j2.html`, `BISECT/crates/bisect-report/src/comparison_html.rs` (new)
...
**Files:** `BISECT/crates/bisect-report/src/narrative_manifest.rs` (new), `BISECT/crates/bisect-report/src/lib.rs`
...
- [ ] **9.1** Define `NarrativeManifest` matching spec §7 schema:
  ```
  {
    ...
    "BISECT_build_commit_short": "<short SHA from BISECT_BUILD_COMMIT_SHORT>",
    "rustc_version": "...",
...
- [ ] **9.4** **Reproducibility (BENCHMARK P1 patched)**: when `SOURCE_DATE_EPOCH` env is set, `approved_at` is the corresponding ISO-8601 UTC; when unset for `[DRAFT]`, use `1970-01-01T00:00:00Z`. `BISECT_build_commit_short` is read from `BISECT_BUILD_COMMIT_SHORT` env (build-time) or falls back to compile-time `git rev-parse --short HEAD` baked in by `build.rs`. NEVER read `SystemTime::now()` for manifest fields.
...
- [ ] **10.6** Manifest: `comments_overlay_sha256` = SHA-256 of the input CSV bytes (post-canonicalization by `BISECT civic ingest`, so it's stable).

**Exit:** Overlay round-trips through the report; narrative paragraph correctly enumerates comments.

---

## Task 11: Wire `BISECT compare --format html|narrative|both` end-to-end

**Files:** `BISECT/crates/bisect-cli/src/compare.rs`

- [ ] **11.1** When `format` is `Html`, `Narrative`, or `Both`, dispatch to `BISECT_report::assemble_comparison` + the renderers. Otherwise keep the existing `Table`/`Json`/`Csv` path untouched.
...
**Files:** `BISECT/tests/reproducibility/test_narrative_byte_identical.rs` (new), `BISECT/crates/bisect-report/tests/byte_identical.rs` (new)

- [ ] **12.1** Test fixture: small synthetic two-plan dataset under `BISECT/tests/fixtures/comparison_repro/` (~10 tracts, hand-computed expected values).
- [ ] **12.2** Invoke `BISECT compare --plan-a ... --plan-b ... --format both --approved-by "Test"` twice with `SOURCE_DATE_EPOCH=1700000000` and `BISECT_BUILD_COMMIT_SHORT=abcd1234`; assert `narrative.md` and `narrative_manifest.json` are byte-identical between the two runs.
- [ ] **12.3** PP-31 test: place the fixture at a path containing a literal space and a non-ASCII character (e.g., `tests/fixtures/comparison repro café/`). Assert `narrative_manifest.json["template_path"]` is a forward-slash relative path with the original characters intact, AND that running `BISECT compare` from a different CWD still produces the same manifest bytes.
...
**Files:** `BISECT/crates/bisect-report/tests/narrative_value_correctness.rs` (new)
...
**Files:** `BISECT/crates/bisect-report/tests/counter_proposal.rs` (new)
...
- [ ] **14.2** Run full `BISECT compare --format both`. Assert:
...
**Files:** `docs/BISECT_CLI.md`, `docs/quickstart/quickstart-civic-advocate.md`, `docs/CHANGELOG.md`, `CLAUDE.md`

- [ ] **15.1** `docs/BISECT_CLI.md`: `BISECT compare` section gets the new flags, the output-directory layout, the `[DRAFT]`/`--approved-by` workflow, and a worked example using the Onboarding Vermont walkthrough plan as input.
- [ ] **15.2** `docs/quickstart/quickstart-civic-advocate.md` (owned by Onboarding plan; this plan amends it): add a "produce a comparison narrative" section ending in `BISECT compare --plan-a vt_proposed --plan-b vt_enacted --format both --approved-by "Your Name"`.
- [ ] **15.3** `docs/CHANGELOG.md` entry naming spec + plan.
- [ ] **15.4** `CLAUDE.md` "Recent Changes" entry.
- [ ] **15.5** Document the `--narrative-template <PATH>` override path in BISECT_CLI.md, with one minimal worked example showing how a state advocacy group can fork the template.
...
- `BISECT compare --plan-a X --plan-b Y --format both` produces every artifact in spec §"Outputs" (HTML, narrative, diff PNG, summary card PNG, comparison JSON, narrative manifest).
...
- Re-running the same `compare` command on the same inputs with `SOURCE_DATE_EPOCH` + `BISECT_BUILD_COMMIT_SHORT` pinned produces byte-identical `narrative.md` and `narrative_manifest.json` (CI gate).
...
- `docs/BISECT_CLI.md` documents the full surface; `quickstart-civic-advocate.md` references the comparison flow.
...
| Byte-identical re-render gate flakes on git-SHA / timestamp leaks | Task 9.4 routes every timestamp through `SOURCE_DATE_EPOCH` and every commit short SHA through `BISECT_BUILD_COMMIT_SHORT`; Task 12 is the explicit CI gate. |
...
- The write side of `BISECT civic ingest` (intake, validation, GEOID-leading-zero protection, URL snapshot). Owned by Civic Bidirectional plan; this plan only consumes its output.
