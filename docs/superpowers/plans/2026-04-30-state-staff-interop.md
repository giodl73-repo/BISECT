...
**Goal:** Make the BISECT CLI a first-class backend for Districtr, Dave's Redistricting App (DRA), and QGIS, so a state legislative staffer's loop is "draw in Districtr → `BISECT import` → `bisect analyze` → `BISECT report` → iterate," with a parallel civic-bypass entry point for community-submitted counter-proposals.

**Depends on:** `bisect-report` import infrastructure (`import_geojson_plan`, `import_gerrychain_to_assignments`), the existing partial `BISECT import --format districtr` shell, `write_manifest_atomic` already in `bisect-report::manifest`, and the Callais p.36 mutex centralised in `runner.rs::validate_partisan_config`.
...
- `bisect-core::validate_partisan_config` (in `runner.rs`) enforces the Callais p.36 mutex at the `bisect state` gate; this plan extends the same guard to `BISECT import` and `bisect analyze`.
...
**Files:** `BISECT/crates/bisect-cli/src/import_cmd.rs`, `BISECT/crates/bisect-report/src/manifest.rs` (extend), `BISECT/crates/bisect-cli/src/io_utils.rs`
...
**Files:** `BISECT/crates/bisect-report/src/districtr.rs` (new), `BISECT/crates/bisect-cli/src/args.rs`, `BISECT/crates/bisect-cli/src/import_cmd.rs`, `BISECT/crates/bisect-cli/src/export_cmd.rs`
...
- [ ] **2.6** Cache strategy for block→tract crosswalks (spec risk row): store under `~/.cache/BISECT/baf/{year}/{state_fips}.csv` on Linux/macOS, `%LOCALAPPDATA%\BISECT\baf\{year}\{state_fips}.csv` on Windows. Fetch on demand from Census-published BAF; SHA-256-pinned in `import_compat.json` (Task 5). Document in `docs/file-formats/districtr.md` (new file written by Task 9).
...
**Exit:** `BISECT import --format districtr <PATH>` produces a plan label that `bisect analyze` can consume; `BISECT export --format districtr` produces JSON whose top-level keys match Districtr's expected shape.

---

## Task 3: DRA import + export (spec §3)

**Files:** `BISECT/crates/bisect-report/src/dra.rs` (new), `BISECT/crates/bisect-cli/src/import_cmd.rs`, `BISECT/crates/bisect-cli/src/export_cmd.rs`
...
- [ ] **3.1** Promote `import_assignments_from_dra_csv` from `import_cmd.rs` into `BISECT_report::dra::parse_dra_csv`, returning `(assignments, fingerprint)`. The fingerprint is the column-set string (e.g., `"GEOID,DISTRICT"` or `"DISTRICT,GEOID"` or `"<no-header>"`) — consumed by the schema-version handshake.
...
**Files:** `BISECT/crates/bisect-cli/src/import_cmd.rs`, `BISECT/crates/bisect-cli/src/export_cmd.rs`, `BISECT/crates/bisect-report/src/shapefile.rs` (new)
...
- [ ] **4.3** Implement `export_shapefile_assignments` as **out of scope for v1** — emit the existing ogr2ogr guidance for the export side, but make it specific: "BISECT does not write shapefiles natively; use `BISECT export --format geojson` then `ogr2ogr -f \"ESRI Shapefile\" out.shp out.geojson`." Rationale: writing valid shapefiles requires geometry, which `BISECT export` doesn't always have (see `warn_if_null_geometry`); GeoJSON-then-ogr2ogr is the supported path. Document this trade-off in `docs/file-formats/state-staff-interop.md` (Task 9).
...
**Exit:** QGIS users can save a layer with a `district` column and import it with `BISECT import --format shapefile`; the export path goes through GeoJSON+ogr2ogr with documented guidance.

---

## Task 5: Schema-version handshake + `import_compat.json` (spec §8, PP-33, C-05)

**Files:** `BISECT/crates/bisect-cli/src/import_compat.json` (new), `BISECT/crates/bisect-report/src/import_compat.rs` (new), `BISECT/crates/bisect-cli/src/import_cmd.rs`
...
- [ ] **5.1** Author `BISECT/crates/bisect-cli/src/import_compat.json` with this schema:
    ```json
    {
    ...
    ```
- [ ] **5.2** Compile-time-embed via `include_str!`. Add `BISECT_report::import_compat::compat_sha256()` returning the SHA-256 of the embedded JSON bytes — written to every import manifest as `import_compat_sha256` (C-05).
...
**Files:** `BISECT/crates/bisect-cli/src/import_cmd.rs`, `BISECT/crates/bisect-cli/src/analyze.rs`, `BISECT/crates/bisect-cli/src/runner.rs` (extract preflight)
...
**Exit:** Callais disentanglement holds at three gates (`bisect state`, `BISECT import`, `bisect analyze`) and is centralised in one function.

---

## Task 7: Civic-bypass `--as-civic-counter-proposal` flag (spec §10, COMMONS)

**Files:** `BISECT/crates/bisect-cli/src/args.rs`, `BISECT/crates/bisect-cli/src/import_cmd.rs`, `BISECT/crates/bisect-report/src/manifest.rs`
...
**Files:** `BISECT/crates/bisect-report/src/canonical.rs` (new), `BISECT/crates/bisect-cli/src/import_cmd.rs::tests`, `BISECT/crates/bisect-report/src/districtr.rs::tests`, `BISECT/crates/bisect-report/src/dra.rs::tests`, `tests/fixtures/districtr/` (new — B-05)

Spec §6 fixes the equality definition: `T(BISECT(T(P))) == P` after district-label canonicalisation (re-number districts in increasing order of their lowest-GEOID member). This task delivers (a) the canonicalisation function, (b) a property-test harness, (c) the hermetic Districtr fixture (B-05).

- [ ] **8.1** Implement `BISECT_report::canonical::canonicalize_assignments(a: &HashMap<GEOID, usize>) -> BTreeMap<GEOID, usize>`: re-number districts by ascending min-GEOID; tie-break is impossible because GEOIDs are unique. Return BTreeMap so equality is order-independent.
...
- [ ] **8.7** L1 test `tests/integration/test_round_trip_vermont.rs` (or pytest equivalent under `BISECT/tests/`): drives the full CLI via `Command::new("BISECT")` against the in-repo fixtures; asserts canonical equality and exit code 0.
...
**Files:** `docs/quickstart/quickstart-state-staff.md` (new — claimed by Onboarding plan Task 5.4 but content lives here), `docs/file-formats/state-staff-interop.md` (new), `docs/file-formats/districtr.md` (new), `docs/BISECT_CLI.md` (extend), `docs/CHANGELOG.md`

The Onboarding plan creates the file at Task 5.4; this plan supplies the actual Districtr → BISECT → PDF walkthrough that lives inside it.
...
- [ ] **9.1** `quickstart-state-staff.md` follows the spec's persona-template (Who you are / What you'll have / Time / Steps / Expected output / Where to go next). Steps:
    ...
    2. `BISECT import --format districtr senate_v3.json --plan-label vt_senate_v3 --state VT`.
    3. `bisect analyze --plan-label vt_senate_v3 --types all`.
    4. `BISECT report --plan-label vt_senate_v3 --format pdf`.
...
- [ ] **9.6** Extend `docs/BISECT_CLI.md`: add `--format districtr|dra|shapefile` to the import + export sections; add `--as-civic-counter-proposal`, `--submitted-by`, `--submitted-at` to the import section.
...
**Exit:** `quickstart-state-staff.md` walks end-to-end; the two file-format docs are referenced from BISECT_CLI.md and the spec; CHANGELOG.md notes the new surface.

---

## Definition of Done

- `BISECT import --format districtr`, `--format dra`, `--format shapefile`, `--format geojson` all produce a plan label that `bisect analyze` consumes
- `BISECT export --format districtr` produces JSON loadable in Districtr's web app (manual UAT against a real Districtr instance)
...
- Callais p.36 preflight fires at `BISECT import` AND `bisect analyze`; tested at both gates
...
- All five quickstart artifacts reachable from `docs/BISECT_CLI.md` and `README.md`
