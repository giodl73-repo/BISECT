...
- [ ] **3.1** Build a `valid_geoids: HashSet<String>` for `(year, state)` by reusing the existing TIGER tract reader at `bisect-data/src/tiger.rs` — the same source of record `bisect fetch` uses today. Falls back to a checked-in tract list under `bisect-data/data/tracts/{year}/{state}.txt` if TIGER files are unavailable in the current environment (deterministic CI path).
...
- [ ] **4.1** Add `url = "2"` to `bisect-cli/Cargo.toml`.
...
- [ ] **6.2** **Accepted attestation-doc formats** (BD-R2 reconciled union — resolves v2.1.1 P0 mismatch with Callais plan): `.pdf`, `.docx`, `.md`, `.txt` (curator-narrative formats), AND `.png`, `.jpg`/`.jpeg`, `.tif`/`.tiff` (civic-submitter scanned-letterhead formats). Reject `.html`, `.rtf` — too easy to fake / no provenance. Format detection is by magic bytes (the `infer` crate, or add `infer = "0.16"`), not extension; the manifest records both the detected format and the file extension for cross-check. Court-mode gating (BD-R1) lives in the Court Reports plan: `--allow-non-strict-civic` is required to embed non-PDF attestation in a court submission. Tamper-evidence is provided by `attestation_doc_sha256` (Task 6.3) — computed on file bytes, format-agnostic.
