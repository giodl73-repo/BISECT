//! Plan Comparison plan Task 11 — `BISECT compare --format narrative` end-to-end.
//!
//! Builds two synthetic plan directories on disk, shells out to the production
//! `BISECT` binary with `--format narrative`, then asserts:
//! - exit code 0
...
//!
//! Depends on the production binary; run with `cargo test -p BISECT-cli --test
...
/// `PlanManifest` shape (see BISECT-report::manifest::PlanManifest); fields
...
fn narrative_dispatch_writes_files_and_records_provenance() {
    ...
    let status = Command::new(BISECT)
        ...
        .status()
        .expect("bisect compare failed to launch");
    assert!(
        status.success(),
        "bisect compare exit code = {:?}",
        status.code()
    );
    ...
    assert!(
        narrative_path.is_file(),
        "narrative.md missing at {}",
        narrative_path.display()
    );
    assert!(
        manifest_path.is_file(),
        "narrative_manifest.json missing at {}",
    ...
    assert_eq!(
        a_sha.len(),
        64,
        "plan_a_manifest_sha256 must be 64 hex chars"
    );
    assert_eq!(
        b_sha.len(),
        64,
        "plan_b_manifest_sha256 must be 64 hex chars"
    );
    assert_eq!(t_sha.len(), 64, "template_sha256 must be 64 hex chars");
    assert_ne!(
        a_sha, b_sha,
        "two distinct plans must have distinct manifest SHAs"
    );
    ...
    assert!(
        plan_a_analyses.contains_key("partisan.json"),
        "plan_a must record partisan.json SHA"
    );
    assert!(
        plan_a_analyses.contains_key("compactness.json"),
        "plan_a must record compactness.json SHA"
    );
    ...
    assert!(
        !attribution.is_null(),
        "civic_counter_proposal_attribution must be set when plan B is tagged"
    );
    assert_eq!(
...
fn narrative_dispatch_drops_draft_prefix_when_approved() {
    ...
    let status = Command::new(BISECT)
        .args([
            ...
            "narrative",
            "--approved-by",
            "J. Doe (state demographer)",
        ])
        .env("SOURCE_DATE_EPOCH", "1700000000")
        .status()
        .expect("bisect compare failed");
...
fn narrative_dispatch_format_both_prints_table_and_writes_files() {
    ...
    assert!(
        report_dir.join("narrative.md").is_file(),
        "--format both must still write narrative.md"
    );
    assert!(
...
fn html_dispatch_writes_self_contained_html_with_civic_banner() {
    ...
    let status = Command::new(BISECT)
        ...
        .env("SOURCE_DATE_EPOCH", "1700000000")
        .status()
        .expect("bisect compare --format html failed to launch");
    assert!(
        status.success(),
        "bisect compare --format html exit = {:?}",
        status.code()
    );
    ...
    assert!(
        ...
        "must not link external scripts/images"
    );
    ...
    assert!(
        html.contains("class=\"provenance\""),
        "provenance footer must render"
    );
}
...
fn narrative_dispatch_errors_when_plan_b_missing() {
    ...
    assert!(
        !out.status.success(),
        "narrative dispatch must reject --enacted-only invocations"
    );
