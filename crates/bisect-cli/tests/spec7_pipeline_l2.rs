...
fn find_ia_adj_bin() -> Option<PathBuf> {
    ...
    //    CARGO_MANIFEST_DIR = .../crates/bisect-cli
...
///   the tempdir so `bisect build` can find it.
...
impl PipelineFixture {
    ...
    fn ls_json(&self) -> Value {
        let out = Command::new(BISECT)
            ...
            .current_dir(self.root())
            .output()
            .expect("spawn bisect ls");
        assert!(
            out.status.success(),
            "bisect ls --json failed\n--- stderr ---\n{}",
            String::from_utf8_lossy(&out.stderr)
        );
        serde_json::from_slice(&out.stdout).unwrap_or_else(|e| {
            panic!(
                "bisect ls --json produced invalid JSON: {e}\nstdout: {}",
    ...
    fn show_json(&self) -> Value {
        let out = Command::new(BISECT)
            ...
            .current_dir(self.root())
            .output()
            .expect("spawn bisect show");
        assert!(
            out.status.success(),
            "bisect show --json failed\n--- stderr ---\n{}",
            String::from_utf8_lossy(&out.stderr)
        );
        serde_json::from_slice(&out.stdout).unwrap_or_else(|e| {
            panic!(
                "bisect show --json produced invalid JSON: {e}\nstdout: {}",
                String::from_utf8_lossy(&out.stdout)
            )
        })
    }
...
fn test_build_iowa_creates_correct_structure() {
    ...
    assert_success(&build_out, "bisect build ia_build_test");
    ...
    assert!(
        ...
        "bisect ls --json must contain 'ia_build_test' after build: {ls}"
    );
...
fn test_full_pipeline_build_analyze_verify() {
    ...
    // ── Step 1: build ─────────────────────────────────────────────────────────
    let build_out = fixture.build();
    assert_success(&build_out, "bisect build ia_pipeline_test");
...
fn test_build_then_mv_preserves_data() {
    ...
    let ls = {
        let ls_out = Command::new(BISECT)
            ...
            .output()
            .expect("spawn bisect ls");
        assert_success(&ls_out, "bisect ls after mv");
...
fn test_import_then_compare_with_build() {
    ...
    let build_out = fixture.build();
    assert_success(&build_out, "bisect build ia_compare_base");
...
";
    ...
    let import_out = Command::new(BISECT)
        ...
        .current_dir(fixture.root())
        .output()
        .expect("spawn bisect label-import");
    ...
    let analyze_ext_out = Command::new(BISECT)
        ...
        .current_dir(fixture.root())
        .output()
        .expect("spawn bisect label-analyze external");
    eprintln!(
    ...
    let compare_out = Command::new(BISECT)
        ...
        .current_dir(fixture.root())
        .output()
        .expect("spawn bisect label-compare");
