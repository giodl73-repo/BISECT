...
fn run_smc_ensemble(args: &EnsembleArgs) -> anyhow::Result<()> {
    ...
    eprintln!("[bisect ensemble smc] {} {} — loading {} adjacency tracts",
    ...
    eprintln!("[bisect ensemble smc] particles={}, pop_tolerance={:.3}, resample_threshold={:.2}",
    ...
    eprintln!("[bisect ensemble smc] Running SMC... (may take several minutes for N={}).",
        args.particles);
    let result = bisect_smc::run_smc(adjacency, pop, k, cfg)
        .map_err(|e| anyhow::anyhow!("SMC run failed: {e}"))?;

    eprintln!("[bisect ensemble smc] Complete: {} plans, {} resamplings",
    ...
    if !result.ess_trace.is_empty() {
        ...
        eprintln!("[bisect ensemble smc] Min ESS={min_ess:.0} at stage {min_stage}/{}",
