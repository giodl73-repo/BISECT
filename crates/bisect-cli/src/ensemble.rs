...
/// Run the `BISECT ensemble` command.
...
fn run_smc_ensemble(args: &EnsembleArgs) -> anyhow::Result<()> {
    ...
    eprintln!(
        "[bisect ensemble smc] {} {} — loading {} adjacency tracts",
        state_upper,
        year,
    ...
    eprintln!("[bisect ensemble smc] n={n} tracts, k={k} districts");
    eprintln!(
        "[bisect ensemble smc] particles={}, pop_tolerance={:.3}, resample_threshold={:.2}",
    ...
    eprintln!("[bisect ensemble smc] base_seed={base_seed}");

    // ── Run SMC ──────────────────────────────────────────────────────────────
    let cfg = bisect_smc::SmcConfig {
    ...
    eprintln!(
        "[bisect ensemble smc] Running SMC... (may take several minutes for N={}).",
        args.particles
    );
    let result = bisect_smc::run_smc(adjacency, pop, k, cfg)
        .map_err(|e| anyhow::anyhow!("SMC run failed: {e}"))?;

    eprintln!(
        "[bisect ensemble smc] Complete: {} plans, {} resamplings",
        result.n_plans(),
    ...
    if !result.ess_trace.is_empty() {
        ...
        let min_stage = result
            ...
            .unwrap_or(0);
        eprintln!(
            "[bisect ensemble smc] Min ESS={min_ess:.0} at stage {min_stage}/{}",
    ...
    let write_cfg = bisect_smc::WriteConfig {
        resample_threshold: args.resample_threshold,
        pop_tolerance: args.pop_tolerance,
    };
    result
        .write_ndjson(&mut writer, &write_cfg)
        .with_context(|| "failed to write NDJSON output")?;

    eprintln!("[bisect ensemble smc] Written: {output_path}");
    eprintln!("[bisect ensemble smc] Verify: check 'file_sha256' in the final metadata line.");
    Ok(())
}
