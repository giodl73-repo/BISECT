...
fn run_all_plans_doctor(version: &str, year: &str, output_base: &str) {
    ...
    if labels.is_empty() {
        ...
        println!("Run 'bisect state --label ...' to create a plan first.");
        return;
    }
    ...
    println!("\nRun 'bisect doctor --label <label>' for detailed diagnosis.");
}
...
fn run_plan_doctor(label: &str, version: &str, year: &str, output_base: &str) {
    ...
    for name in &required_analysis {
        if ctx.analysis_file_exists(name) {
            ...
            println!(
                "[MISS] {:<24} -> run: bisect analyze --label {} --types {}",
                name,
                label,
    ...
    if !has_maps {
        ...
        println!("       Run: bisect map --label {} --types districts", label);
        println!(
            "       (needs: bisect fetch --type tiger --states {} --year {})",
            ctx.state_code(),
            ctx.year()
        );
    }
    ...
    if audit_path.exists() {
        ...
        println!(
            "       Run: bisect report --label {} --audit-with-report --format html",
            label
        );
    }
    ...
    if rplan_path.exists() {
        ...
        println!(
            "[INFO] No .rplan file -- run: bisect export --label {} --format rplan",
            label
        );
    }
...
fn analysis_fetch_hint(name: &str, state_code: &str, year: &str) -> Option<String> {
    match name {
        "partisan.json" => Some(format!(
            "(also needs: bisect fetch --type elections --states {} --year {})",
            state_code, year
        )),
        "demographic.json" | "vra_analysis.json" => Some(format!(
            "(also needs: bisect fetch --type demographics --states {} --year {})",
            state_code, year
        )),
        _ => None,
    }
...
pub fn run_verify_manifest(manifest_path: &str) -> i32 {
    ...
    println!("=== bisect doctor --verify-manifest ===\n");
...
pub fn run_check_tutorial_data(tutorial: &str, repo_root: &std::path::Path) -> i32 {
    ...
    println!("=== bisect doctor --check-tutorial-data ===\n");
