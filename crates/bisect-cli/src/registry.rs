    ...
    fn us_census_path(
        ...
        Err(format!(
            ...
            if build_hint.is_empty() {
                format!("bisect fetch --type adjacency --states {state_upper} --year {year}")
            } else {
...
mod tests {
    ...
    fn test_us_state_doctor_hint_uses_fetch_command() {
        // For WA, the adjacency_path error must contain "bisect fetch" not the Python script.
        let reg = registry();
        ...
        match result {
            Err(msg) => {
                assert!(
                    ...
                    "doctor hint must suggest 'bisect fetch', got: {msg}"
                );
                assert!(
