...
fn build_default_client() -> Result<reqwest::blocking::Client, CivicError> {
    reqwest::blocking::Client::builder()
        ...
        .user_agent(format!(
            "bisect-civic-snapshot/{} (PP-30 archival fetch)",
...
fn run_ingest(args: &CivicIngestArgs) -> anyhow::Result<()> {
    ...
    let manifest = CivicManifest {
        ...
        build_commit_short: prov.build_commit.chars().take(8).collect(),
...
fn run_conflicts(args: &CivicConflictsArgs) -> anyhow::Result<()> {
    if args.label.len() < 2 {
        anyhow::bail!("[INPUT] bisect civic conflicts requires --label twice or more");
    }
    ...
    for label in &args.label {
        ...
        if !path.exists() {
            anyhow::bail!(
                "[INPUT] civic input '{label}' not found at {} (run `bisect civic ingest` first)",
                path.display()
            );
        }
...
fn run_add_candidate_race(args: &CivicAddCandidateRaceArgs) -> anyhow::Result<()> {
    ...
    let annotation_set = bisect_analysis::race_of_candidate::parse_race_of_candidate_csv(&args.csv)
