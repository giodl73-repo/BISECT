...
//!
//! When `bisect analyze --paper-mode` is invoked, this module writes a
...
//! - `environment.json` — `version`, `build_commit`,
...
pub fn emit_replication_package(inputs: &PaperModeInputs<'_>) -> Result<PathBuf, PaperModeError> {
    ...
    let cargo_lock_sha = sha_of_optional_repo_file("bisect/Cargo.lock");
...
fn render_readme(
    ...
    format!(
        "# AEA Replication Package\n\
         ...
         \n\
         This package was produced by `bisect analyze --paper-mode`. It is a\n\
         ...
         | `environment.json` | bisect + rustc + platform metadata |\n\
         | `seeds.json` | Master seed for stochastic operations (null if unseeded) |\n\
         | `CITATION.bib` | BibTeX citation for `bisect` |\n\
...
mod tests {
    ...
    fn fixture_inputs<'a>(
        ...
        (
            PaperModeInputs {
                ...
                analyze_invocation: "bisect analyze --label vt_2020".to_string(),
                seed: Some(42),
    ...
    fn environment_records_runtime_metadata() {
        ...
        assert_eq!(v["build_commit_short"].as_str().unwrap().len() <= 8, true);
    }
    ...
    fn citations_include_version_and_short_commit() {
        ...
        let prov = Provenance::current();
        let short: String = prov.build_commit.chars().take(8).collect();
        assert!(bib.contains(&prov.version), "BibTeX must include version");
        assert!(apa.contains(&prov.version), "APA must include version");
        assert!(chi.contains(&prov.version), "Chicago must include version");
        if !short.is_empty() {
