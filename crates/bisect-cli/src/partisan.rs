...
pub struct MetricsBlock {
    pub efficiency_gap: bisect_analysis::MetricWithCI,
    pub mean_median: bisect_analysis::MetricWithCI,
    pub partisan_bias: bisect_analysis::MetricWithCI,
}
    ...
    if !path.exists() {
        anyhow::bail!(
            "Election data not found at {}.\n\
             ...
             (`bisect fetch --type elections` is declared but not yet implemented; \
    ...
    if !election_path.exists() {
        let output = PartisanJsonOutput {
            ...
            unavailable_reason: Some(format!(
                "Election data not found. Run: bisect fetch --type elections --year {}",
                args.year
            )),
...
mod tests {
    ...
    fn test_partisan_district_output_dem_pct_100_percent() {
        ...
        let d = bisect_analysis::DistrictElection {
            district: 1,
            dem_votes: 1000.0,
            rep_votes: 0.0,
        };
