...
pub fn run_analyze(args: &AnalyzeArgs) -> anyhow::Result<()> {
    ...
    if !assignments_path.exists() {
        ...
        anyhow::bail!(
            "No assignments found at {}.{}\nRun: bisect state --state {state_code} --year {year} --version {}",
    ...
    if args.stdout && types.len() != 1 {
        anyhow::bail!(
            ...
            Example: bisect analyze --label {} --types summary --stdout",
            types.len(),
    ...
    for typ in &types {
        ...
        match typ {
                        ...
                        if msg.contains("demographics CSV not found") {
                            eprintln!(
                                "WARNING: VRA analysis for {state_code} {year}: demographics CSV not found.\n\
                                 Run: bisect fetch --type demographics --states {state_code} --year {year}"
                            );
            ...
            AnalyzerType::Proportionality => {
                let result = bisect_analysis::ProportionalityAnalyzer::run(&ctx)?;
                if args.stdout {
                    println!("{}", serde_json::to_string_pretty(&result)?);
                } else {
                    write_json_atomic(&out_path, &result)?;
...
fn load_precincts_by_candidate(
    path: &Path,
) -> anyhow::Result<HashMap<String, Vec<bisect_analysis::Precinct>>> {
    use bisect_analysis::Precinct;
...
fn stage_repro_artifacts(
    ...
    annotation_set: &bisect_analysis::AnnotationSet,
) -> anyhow::Result<()> {
...
fn load_adjacency_for_analysis(
    ...
    let adjacency = if let Some(bp) = bin_path {
        ...
        let graph = bisect_data::deserialize_adjacency(&data)
...
mod tests {
    ...
    fn test_vra_missing_demographics_hint() {
        use bisect_analysis::{Analyzer, AnalyzerContext, DemographicAnalyzer};
        use std::collections::HashMap;
        use std::path::Path;
        ...
        // Verify that the hint we emit contains "bisect fetch" and "--type demographics"
