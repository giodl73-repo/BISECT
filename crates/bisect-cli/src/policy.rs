/// Location policy database — loads and queries BISECT/data/location_policy.json.
///
/// The JSON is embedded directly in the binary at compile time so `BISECT policy`
/// works without any external files.
///
/// Override with BISECT_LOCATION_POLICY env var (BISECT_STATE_POLICY accepted for
...
pub fn load_policy() -> serde_json::Value {
    ...
    if let Ok(path) = std::env::var("BISECT_LOCATION_POLICY") {
        if let Ok(content) = std::fs::read_to_string(&path) {
            ...
            eprintln!(
                "WARNING: BISECT_LOCATION_POLICY={path} could not be parsed; \
                 falling back to embedded policy."
            );
        } else {
            eprintln!(
                "WARNING: BISECT_LOCATION_POLICY={path} could not be read; \
                 falling back to embedded policy."
            );
        }
    }
    ...
    if let Ok(path) = std::env::var("BISECT_STATE_POLICY") {
        if let Ok(content) = std::fs::read_to_string(&path) {
            ...
            eprintln!(
                "WARNING: BISECT_STATE_POLICY={path} could not be parsed; \
                 falling back to embedded policy."
            );
        } else {
            eprintln!(
                "WARNING: BISECT_STATE_POLICY={path} could not be read; \
                 falling back to embedded policy."
            );
        }
    }
    ...
    match get_state_policy(&policy, &state_code) {
        None => {
            ...
            if let Some(obj) = policy_obj {
                ...
                match matches.len() {
                    1 => {
                        ...
                        eprintln!(
                            "NOTE: '{}' matched as {} ({}). Use the 2-letter code next time.",
                    ...
                    _ => {
                        let suggestions: Vec<String> = matches
                            ...
                            .collect();
                        eprintln!(
                            "Multiple matches for '{}': {}. Use the 2-letter code.",
                            state_code,
                            suggestions.join(", ")
                        );
                    }
            ...
            anyhow::bail!(
                ...
                Check BISECT_LOCATION_POLICY or use a 2-letter code (e.g., IE for Ireland).",
                state_code
            )
        }
        Some(state) => {
            if args.format == "json" {
                ...
                let fields = [
                    ...
                    (
                        "Congressional balance tolerance",
                        "balance_tolerance_congressional_pct",
                    ),
...
mod tests {
    ...
    fn test_eldoria_subdivision_term() {
        ...
        assert_eq!(
            eldoria["subdivision_term_plural"].as_str().unwrap(),
            "realms"
        );
        assert_eq!(
            eldoria["balance_tolerance_house_pct"].as_f64().unwrap(),
            7.5
        );
        assert_eq!(
    ...
    fn test_policy_va_has_independent_city_note() {
        ...
        assert!(
            va.get("independent_city_note").is_some(),
            "VA must have independent_city_note"
        );
        let note = va["independent_city_note"].as_str().unwrap();
        assert!(
            note.contains("independent"),
            "VA note must mention independent cities"
        );
    }
    ...
    fn test_policy_wa_has_nesting_requirement() {
        ...
        assert_eq!(
            wa["nesting_requirement"].as_str().unwrap(),
            "senate_contains_two_house"
        );
    ...
    fn test_policy_hi_has_permanent_resident_basis() {
        ...
        assert_eq!(
            hi["population_basis"].as_str().unwrap(),
            "permanent_resident",
    ...
    fn test_policy_nj_has_student_counting() {
        ...
        assert_eq!(
            nj["student_counting"].as_str().unwrap(),
            "college_address",
    ...
    fn test_policy_ne_has_unicameral_note() {
        ...
        assert!(
            notes.to_lowercase().contains("unicameral"),
    ...
    fn test_eldoria_nesting_4to1() {
        ...
        let el = get_state_policy(&policy, "_TEST_EL").unwrap();
        assert_eq!(
            el["nesting_requirement"].as_str().unwrap(),
            "senate_contains_house"
        );
    ...
    fn test_eldoria_electoral_system_is_fantasy() {
        ...
        assert!(
            system.contains("arcane") || system.contains("proportional"),
    ...
    fn test_eldoria_independent_city_note_has_ironforge() {
        ...
        assert!(
            note.contains("Ironforge"),
            "Eldoria must mention the Free City of Ironforge"
        );
    }
    ...
    fn test_eldoria_prison_gerrymandering_home_address() {
        ...
        let el = get_state_policy(&policy, "_TEST_EL").unwrap();
        assert_eq!(
            el["prison_gerrymandering"].as_str().unwrap(),
            "home_address"
        );
    }
    ...
    fn test_load_policy_embedded_is_valid_json_object() {
        ...
        assert!(
            policy.is_object(),
            "embedded policy must be a valid JSON object"
        );
        // Must have at least 50 US states + some test entries
        let count = policy.as_object().unwrap().len();
        assert!(
            count >= 50,
            "policy must have at least 50 entries, got {count}"
        );
    }
    ...
    fn test_policy_tx_has_house_and_senate_districts() {
        ...
        assert!(
            tx.get("house_districts").is_some(),
            "TX must have house_districts"
        );
        assert!(
            tx.get("senate_districts").is_some(),
            "TX must have senate_districts"
        );
    ...
    fn test_policy_ri_subdivision_term_is_city_or_county() {
        ...
        assert!(
            !term.is_empty(),
            "RI must have a non-empty subdivision term"
        );
    }
    ...
    fn test_policy_location_registry_chamber_districts_congressional() {
        ...
        assert!(
            n >= 40 && n <= 60,
            "CA congressional districts {n} must be in [40,60]"
        );
    }
    ...
    fn test_policy_location_registry_from_str_invalid_json_returns_err() {
        ...
        assert!(
            result.is_err(),
            "invalid JSON must return Err from from_str"
        );
    }
    ...
    fn test_policy_ny_has_prison_gerrymandering_field() {
        ...
        assert!(
            ny.get("prison_gerrymandering").is_some(),
