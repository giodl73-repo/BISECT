use bisect_cli::aggregate::run_aggregate;
use bisect_cli::analyze::run_analyze;
use bisect_cli::analyze_label::{run_label_analyze, run_label_report};
...
use bisect_cli::plan_cmd::run_plan;
use bisect_cli::policy::run_policy;
use bisect_cli::policy::LocationRegistry;
use bisect_cli::report_cmd::run_report;
use bisect_cli::runner::{
    chamber_district_count, filter_incomplete, load_all_states, run_states_parallel,
    validate_multiscale_levels, AlgorithmConfig, StateConfig, StateResult,
};
use bisect_cli::suite::run_suite_stability;
use bisect_cli::sweep::run_sweep;
use bisect_cli::validate::run_validate;
    ...
    match cli.command {
        ...
        Commands::State(args) => {
            ...
            let year = if registry.has_location(&state_code) {
                registry
                    .validate_year(&state_code, &args.year)
                    .unwrap_or_else(|e| {
                        eprintln!("ERROR: {e}");
                        std::process::exit(1);
                    })
            ...
            let (state_name, num_districts) = if let Some(ref _adj_path) = args.adjacency {
                let name = args.state_name.clone().unwrap_or_else(|| {
                    registry
                ...
                let districts = args.districts.unwrap_or_else(|| {
                    registry
                        .chamber_districts(&state_code, &args.chamber, &year)
                        .unwrap_or_else(|| {
                            eprintln!(
                ...
                let ndist = registry
                    ...
                    .unwrap_or_else(|| {
                        ...
                        let all = load_all_states(&year).unwrap_or_else(|e| {
                            eprintln!("ERROR: {e}");
                            std::process::exit(1);
                        });
                        all.iter()
                            ...
                            .unwrap_or_else(|| {
                                eprintln!(
                                    "ERROR: no district count for '{state_code}' \
                                    chamber '{}' year '{year}'. Use --districts.",
                                    args.chamber
                                );
                                std::process::exit(1);
                            })
            ...
            if args.resolution.to_string() == "block" {
                let large_block_states = [
                    "CA", "TX", "NY", "FL", "PA", "IL", "OH", "GA", "NC", "MI", "VA", "WA", "AZ",
                    "CO",
                ];
            ...
            if args.chamber != "congressional" && args.balance_tolerance.is_none() {
                let tol_pct = registry
                    .chamber_districts(&state_code, &args.chamber, &year)
                    .map(|_| {
                        use bisect_cli::runner::chamber_balance_tolerance;
            ...
            let total_seats = args
                .total_seats
                .unwrap_or(args.seats_per_district * effective_num_districts);
            ...
            let results = run_states_parallel(
                vec![StateConfig {
        ...
        // ── bisect states: 50-state parallel ─────────────────────────────────
        Commands::States(args) => {
            ...
            let configs: Vec<StateConfig> = all
                .into_iter()
                ...
                .map(|(i, (code, name, districts))| {
                    ...
                    use bisect_cli::runner::{SeedCompositor, SplitStrategy};
                    match &mut cfg.algo.split {
                        SplitStrategy::GeoSection | SplitStrategy::AreaSection { .. } => {
                            cfg.algo.seeds = SeedCompositor::Multi {
                    ...
                    if let SplitStrategy::MovingKnife {
                        n_orientations,
                        metric,
                    } = &mut cfg.algo.split
                    {
                    ...
                    if let Some(search) = args.search {
                        use bisect_cli::args::SearchMode as SeM;
            ...
            let has_large_state = configs
                .iter()
                .any(|c| large_states.contains(&c.state_code.as_str()));
            ...
            eprintln!("[bisect states] Running... (this takes 5-15 minutes for 50 states)");
        ...
        Commands::Run(args) => {
            ...
            for year in &years {
                eprintln!("[bisect run] year={year} version={}", args.version);
                let all = load_all_states(year).unwrap_or_else(|e| {
                    eprintln!("ERROR {year}: {e}");
                    vec![]
                });
            ...
            eprintln!("[OK] bisect run complete");
        }

        // ── bisect analyze: per-district analytics ────────────────────────────
        Commands::Analyze(args) => {
            run_analyze(&args).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }
        ...
        Commands::Aggregate(args) => {
            run_aggregate(&args).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }
        ...
        Commands::Fetch(args) => {
            ...
            for year in &years {
                ...
                eprintln!("[bisect fetch] year={year} {} items", items.len());
        ...
        Commands::Validate(args) => {
            run_validate(&args).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }
        ...
        Commands::Migrate(args) => {
            ...
            run_migrate(&base, &args.state, &args.label, args.force).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        ...
        Commands::Compare(args) => {
            bisect_cli::compare::run_compare(&args).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }

        // ── BISECT suite: multi-chamber nested plans ──────────────────────────
        Commands::Suite(suite_args) => {
            match suite_args.command {
                SuiteCommands::Draw(args) => {
                    use bisect_cli::suite::{run_suite_draw, NestMode, SuiteDrawArgs};
                    use std::collections::HashMap;
                    use std::str::FromStr;

                    let nest_mode = NestMode::from_str(&args.nest).unwrap_or_else(|e| {
                        eprintln!("ERROR: {e}");
                        std::process::exit(1);
                    });
                    ...
                    eprintln!(
                        "[bisect suite draw] state={} year={} name={} nest={}",
                ...
                SuiteCommands::Stability(args) => {
                    let result = run_suite_stability(&args).unwrap_or_else(|e| {
                        eprintln!("ERROR: {e}");
                        std::process::exit(1);
                    });
                    println!(
                ...
                SuiteCommands::Validate(args) => {
                    use bisect_cli::suite::run_suite_validate_with_overrides;
        ...
        // ── BISECT report: generate commission report ─────────────────────────
        Commands::Report(args) => {
            run_report(&args).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }

        // ── BISECT export: export plan to GeoJSON/GerryChain/CSV ─────────────
        Commands::Export(args) => {
            run_export(&args).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }
        ...
        Commands::Civic(args) => {
            bisect_cli::civic::run_civic(&args.command).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }
        ...
        Commands::Research(args) => {
            bisect_cli::research::run_research(&args.command).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }

        // ── BISECT ensemble: weighted plan ensemble generation ────────────────
        Commands::Ensemble(args) => {
            run_ensemble(&args).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }
        ...
        Commands::Policy(args) => {
            run_policy(&args).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }

        // ── BISECT doctor: pre-flight check ───────────────────────────────────
        ...
        Commands::Sweep(args) => {
            run_sweep(&args).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }
        ...
        Commands::Config(config_args) => {
            use bisect_cli::algo_config::{validate_config, write_template_config};
            use bisect_cli::args::ConfigCommands;
            match config_args.command {
                ConfigCommands::New(args) => {
                    let weights = if args.weights.is_empty() {
                        None
                    } else {
                        Some(args.weights.as_str())
                    };
                    let search = if args.search.is_empty() {
                        None
                    } else {
                        Some(args.search.as_str())
                    };
                ...
                ConfigCommands::Validate(args) => {
                    let result = validate_config(&args.path).unwrap_or_else(|e| {
                        eprintln!("ERROR: {e}");
                        std::process::exit(1);
                    });
                    println!("{result}");
        ...
        // ── BISECT ls: list all labels (Spec 7 Phase 4) ───────────────────────
        Commands::Ls(args) => {
            run_ls(args.json).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }
        ...
        Commands::Show(args) => {
            run_show(&args.label, args.json).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }

        // ── BISECT mv: atomic label rename (Spec 7 Phase 4) ──────────────────
        Commands::Mv(args) => {
            run_mv(&args.src, &args.dst, args.force).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }
        ...
        Commands::LabelVerify(args) => {
            run_label_verify(&args.label, args.year.as_deref()).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }
        ...
        // ── BISECT label-report: label-aware reports (Spec 7 Phase 3) ────────
        ...
        Commands::LabelImport(args) => {
            bisect_cli::import_label::run_label_import(
                &args.label,
                &args.from,
                &args.year,
        ...
        Commands::LabelCompare(args) => {
            bisect_cli::import_label::run_label_compare(
                &args.label_a,
                &args.label_b,
                &args.year,
                args.json,
        ...
        // ── BISECT tui: interactive terminal UI (alias for BISECT plan) ──────
        Commands::Tui(args) => {
            // Backward-compatible: `BISECT tui` delegates to `BISECT plan`.
            ...
            if args.no_session {
                ...
                tui_bin.set_file_name(if cfg!(windows) {
                    "bisect-tui.exe"
                } else {
                    "bisect-tui"
                });
                ...
                match cmd.status() {
                    ...
                    Err(e) => {
                        eprintln!("[INFO] bisect-tui not found: {e}");
                        eprintln!("[INFO] Install with: cargo install --path crates/bisect-tui");
                        std::process::exit(1);
                    }
                }
            } else {
        ...
        // ── BISECT plan: label-aware TUI hub (Spec 7 Phase 6) ────────────────
        Commands::Plan(args) => {
            run_plan(args).unwrap_or_else(|e| {
                eprintln!("ERROR: {e}");
                std::process::exit(1);
            });
        }
...
#[cfg(test)]
mod tests {
    use bisect_cli::runner::StateResult;
    ...
    fn test_report_and_exit_nonzero_on_failure() {
        ...
        assert!(
            no_failures.is_empty(),
            "all-success → exit 0 (no non-zero exit)"
        );
    }
    ...
    fn test_large_state_set_contains_ca() {
        let large_states = ["CA", "TX", "NY", "FL", "PA", "IL", "OH", "GA", "NC", "MI"];
        assert!(
            large_states.contains(&"CA"),
            "CA must be in large states list"
        );
        assert!(
            large_states.contains(&"TX"),
            "TX must be in large states list"
        );
        assert!(
            large_states.contains(&"NY"),
            "NY must be in large states list"
        );
        // Small states must NOT be in the list
        assert!(
            !large_states.contains(&"VT"),
            "VT must not be in large states list"
        );
        assert!(
            !large_states.contains(&"WY"),
            "WY must not be in large states list"
        );
    }
    ...
    fn test_large_state_memory_warning_format() {
        ...
        assert!(
            msg.contains("1200MB"),
            "must show computed MB for 8 workers: {msg}"
        );
        assert!(
            msg.contains("--workers"),
            "must mention --workers flag: {msg}"
        );
    ...
    fn test_large_state_warning_triggers_above_4_workers() {
        use bisect_cli::runner::StateConfig;
        use std::path::PathBuf;
        ...
        assert!(
            should_warn_5,
            "should warn for 5 workers with CA in configs"
        );

        // Small-state only run: no warning
        let small_configs = vec![make_config("VT"), make_config("DE")];
        let has_large_small = small_configs
            .iter()
            .any(|c| large_states.contains(&c.state_code.as_str()));
        assert!(
            !has_large_small,
            "VT+DE run must not trigger large-state warning"
        );
    }
    ...
    fn test_block_resolution_warning_for_large_state() {
        let large_block_states = [
            "CA", "TX", "NY", "FL", "PA", "IL", "OH", "GA", "NC", "MI", "VA", "WA", "AZ", "CO",
        ];
        // Large states must be in the list
        assert!(
            large_block_states.contains(&"CA"),
            "CA must be in large block states"
        );
        assert!(
            large_block_states.contains(&"TX"),
            "TX must be in large block states"
        );
        assert!(
            large_block_states.contains(&"NY"),
            "NY must be in large block states"
        );
        assert!(
            large_block_states.contains(&"WA"),
            "WA must be in large block states"
        );
        // Small states must NOT be in the list
        assert!(
            !large_block_states.contains(&"VT"),
            "VT must not be in large block states"
        );
        assert!(
            !large_block_states.contains(&"DE"),
            "DE must not be in large block states"
        );
        assert!(
            !large_block_states.contains(&"WY"),
            "WY must not be in large block states"
        );
    }
    ...
    fn test_block_resolution_warning_message_format_large_state() {
        ...
        assert!(
            ...
            "must mention block_group alternative"
        );
    ...
    fn test_block_resolution_warning_only_for_block_resolution() {
        ...
        assert_ne!(
            "block_group", "block",
            "block_group must not trigger block warning"
        );
    ...
    fn test_run_any_failure_flag_logic() {
        ...
        let results_year2 = vec![StateResult {
            state_code: "CA".into(),
            success: true,
            error: None,
            elapsed_ms: 300,
        ...
        assert!(
            any_failure,
            "any_failure must be true when year 2020 had a failure"
        );
