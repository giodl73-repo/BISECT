...
use bisect_cli::runner::{StateConfig, StateResult, AlgorithmConfig, run_states_parallel, load_all_states, filter_incomplete, chamber_district_count, validate_multiscale_levels};
use bisect_cli::policy::LocationRegistry;
use bisect_cli::fetch::{load_manifest, build_fetch_list, print_check_report, download_items};
...
use bisect_cli::label_cmd::{run_ls, run_show, run_mv, run_verify as run_label_verify};
...
fn main() {
    ...
    match cli.command {
        ...
        Commands::States(args) => {
            ...
            let configs: Vec<StateConfig> = all.into_iter()
                ...
                .map(|(i, (code, name, districts))| {
                    ...
                    use bisect_cli::runner::{SplitStrategy, SeedCompositor};
            ...
            eprintln!("[bisect states] Complete: {} succeeded, {} failed", succeeded, failed);
        ...
        Commands::Compare(args) => {
            bisect_cli::compare::run_compare(&args)
                .unwrap_or_else(|e| { eprintln!("ERROR: {e}"); std::process::exit(1); });
        }
        ...
        Commands::Suite(suite_args) => {
            match suite_args.command {
                SuiteCommands::Draw(args) => {
                    use bisect_cli::suite::{SuiteDrawArgs, NestMode, run_suite_draw};
                    ...
                    eprintln!("[bisect suite draw] state={} year={} name={} nest={}",
        ...
        Commands::Civic(args) => {
            bisect_cli::civic::run_civic(&args.command)
                .unwrap_or_else(|e| { eprintln!("ERROR: {e}"); std::process::exit(1); });
        }

        // ── redist research: researcher toolkit (RT plan) ─────────────────────
        Commands::Research(args) => {
            bisect_cli::research::run_research(&args.command)
                .unwrap_or_else(|e| { eprintln!("ERROR: {e}"); std::process::exit(1); });
        }
        ...
        Commands::Config(config_args) => {
            ...
            use bisect_cli::algo_config::{write_template_config, validate_config};
        ...
        Commands::Tui(args) => {
            ...
            if args.no_session {
                ...
                tui_bin.set_file_name(if cfg!(windows) { "bisect-tui.exe" } else { "bisect-tui" });
