    ...
    for map_type in &types {
        match map_type {
            ...
            MapType::Splits => {
                ...
                println!("For now, use: bisect analyze --types splits to get splits.json, then");
...
fn render_choropleth_map(
    ...
    if !analysis_file.exists() {
        anyhow::bail!(
            "Analysis file not found: {}. Run: bisect analyze --state {} --year {year} --types {analysis_type}",
...
mod tests {
    ...
    fn test_canvas_size_from_dpi_150_landscape() {
        use bisect_map::canvas_size_from_dpi;
