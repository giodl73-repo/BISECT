...
fn load_rplan_from_plan_dir(plan_dir: &std::path::Path, label: &str) -> anyhow::Result<RplanFile> {
    ...
    let metadata = if manifest_path.exists() {
        ...
        let manifest: bisect_report::PlanManifest = serde_json::from_str(&manifest_str)?;
        bisect_report::RplanMetadata {
            ...
            created_by: format!("bisect {}", manifest.binary_version),
            ..Default::default()
        }
    } else {
        bisect_report::RplanMetadata {
    ...
    if !has_geometry {
        ...
        eprintln!(
            "WARNING: Plan '{}' has no geometry -- GeoJSON will have null geometries.\n\
             To add geometry: run 'bisect import' with a GeoJSON source, or use\n\
             'bisect fetch --type geometry --states {} --year {}' + reimport.",
            label, state, year
        );
    }
}
...
mod tests {
    ...
    fn test_export_warns_on_null_geometry() {
        use bisect_report::{RplanFile, RplanMetadata};
    ...
    fn make_test_manifest_for_repro() -> bisect_report::PlanManifest {
        bisect_report::PlanManifest {
            label: "vt_congressional_2020".into(),
            state_code: "VT".into(),
            year: "2020".into(),
