...
pub fn load_partisan_shares_map(tsv_path: &Path) -> Result<HashMap<String, f64>, String> {
    ...
    for (line_num, line) in content.lines().enumerate() {
        ...
        let share: f64 = parts[1].trim().parse().map_err(|e| {
            format!(
...
mod tests {
    ...
    fn test_pads_geoid_with_leading_zeros() {
        ...
        let shares = load_partisan_shares_map(&path).expect("should load");
        assert!(
            shares.contains_key("01001020100"),
    ...
    fn test_align_returns_swing_for_missing_geoid() {
        ...
        assert!(
            (aligned[1] - 0.5).abs() < 1e-9,
            "missing geoid → swing default 0.5"
        );
        std::fs::remove_file(&path).ok();
    }
}
...
mod dem_vote_tests {
    ...
    fn test_dem_vote_counts_missing_geoid_defaults() {
        ...
        assert!(
            (total[1] - 1.0).abs() < 1e-9,
            "missing geoid → total=1.0 (avoid div/0)"
        );
        std::fs::remove_file(&path).ok();
    }
    ...
    fn test_dem_vote_counts_missing_file_errors() {
        ...
        let result = load_dem_vote_counts(std::path::Path::new("/nonexistent_votes.csv"), &idx, 0);
    ...
    fn test_dem_vote_counts_correct_output_length() {
        ...
        assert_eq!(
            total.len(),
            5,
            "two_party vector length must equal n_tracts"
        );
        std::fs::remove_file(&path).ok();
    }
