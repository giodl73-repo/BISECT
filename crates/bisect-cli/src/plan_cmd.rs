...
fn print_configure_fallback(label: Option<&str>) {
    ...
    if let Some(lbl) = label {
        println!("       bisect config validate configs/{lbl}.yml");
        println!("       bisect config new --name {lbl} --dry-run");
    } else {
        println!("       bisect config new --name <LABEL> --dry-run");
        println!("       bisect config validate configs/<LABEL>.yml");
    }
}
...
pub fn run_plan(args: PlanArgs) -> Result<(), String> {
    ...
    if !tui_bin.exists() {
        println!(
            "[INFO] bisect-tui not found. Install with: cargo install --path crates/bisect-tui"
        );
        if args.configure {
...
mod tests {
    ...
    fn find_tui_binary_is_sibling_of_current_exe() {
        ...
        assert_eq!(
            tui.parent(),
            exe.parent(),
            "bisect-tui must be a sibling of the current exe"
        );
    }
