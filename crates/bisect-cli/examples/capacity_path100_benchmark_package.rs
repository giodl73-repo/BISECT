...
use bisect_clustering::{capacity_cluster_repaired, ClusterConfig, ClusterStatus};
use rplan_core::{
...
const PACKAGE_ID: &str = "T.15+capacity-path100-benchmark";
const PAPER: &str = "T.15+capacity-constrained-clustering";
...
fn main() -> Result<()> {
    ...
    let result = capacity_cluster_repaired(&adjacency, &populations, config)
        .map_err(|err| anyhow::anyhow!("construct capacity benchmark plan: {err}"))?;
    if result.status != ClusterStatus::Valid {
        anyhow::bail!(
            "capacity benchmark generator produced non-valid status: {:?}",
            result.status
        );
    }
    ...
    let source_fixture = json!({
        ...
        "fixture_id": "path100-capacity-synthetic",
    ...
    let mut units = PlanUnitIndex {
        ...
        unit_ids: (1..=populations.len())
            .map(|idx| format!("53015{:06}", idx * 100))
            .collect(),
        unit_universe_hash: String::new(),
        source_id: Some("path100-capacity-synthetic".to_string()),
    };
    ...
    let document = RplanDocument {
        ...
        metadata: RplanMetadataV02 {
            ...
            description: Some(
                "Synthetic 100-unit path constructed by bisect-clustering capacity clustering."
                    .to_string(),
            ),
        },
        provenance: RplanProvenance {
            producer: BTreeMap::from([
                ...
                (
                    "method".to_string(),
                    json!("capacity-constrained-clustering"),
                ),
            ]),
            source_hashes: BTreeMap::from([("fixture".to_string(), source_fixture_hash)]),
            conversion_lineage: vec![json!({
                "workflow": "cargo run -p bisect-cli --no-default-features --example capacity_path100_benchmark_package",
    ...
    let summary_path = out_dir.join("capacity-summary.json");
    ...
    let transcript = json!({
        ...
        "method": "capacity-constrained-clustering",
        "source_workflow": "cargo run -p bisect-cli --no-default-features --example capacity_path100_benchmark_package",
        "status": result.summary.capacity_status,
        "input_fixture": "path100-capacity-synthetic",
        "unit_count": populations.len(),
        "edge_count_undirected": undirected_edge_count(&adjacency),
        "seeds": result.seeds,
        "plan_hash": plan_hash,
        ...
        "summary_path": "capacity-summary.json",
        "summary_sha256": summary_sha256,
        "assignment": result.assignment,
        "summary": result.summary,
        "scope_note": "Synthetic benchmark-tier package for capacity-clustering lineage and verifier acceptance. It is not real-data quality, legal sufficiency, or wall-clock performance evidence."
    });
    ...
    let timing_notes = json!({
        ...
        "measurement_protocol": [
            "cargo run -p bisect-cli --no-default-features --example capacity_path100_benchmark_package",
            "cargo run -p rplan-cli -- verify-certificate --certificate docs/examples/rplan-benchmark-packages/T.15+capacity-path100-benchmark/audit-certificate.json --plan docs/examples/rplan-benchmark-packages/T.15+capacity-path100-benchmark/plan.rplan --context docs/examples/rplan-benchmark-packages/T.15+capacity-path100-benchmark/context.rctx",
            "cargo run -p bisect-cli -- verify --manifest docs/examples/rplan-benchmark-packages/T.15+capacity-path100-benchmark/manifest.json"
        ],
    ...
    extra["input_fixture"] = json!("path100-capacity-synthetic");
    extra["summary_path"] = json!("capacity-summary.json");
    ...
    if certificate.result == rplan_audit::AuditResult::Fail {
        anyhow::bail!(
            "capacity benchmark package audit failed; refusing to write public benchmark package"
        );
    }
    ...
    let files = vec![
        ...
        manifest_file(&out_dir, "capacity-summary.json", "construction-report")?,
    ...
    let manifest = json!({
        ...
        "status": "constructed",
        "benchmark_scope": "deterministic synthetic capacity-clustering scale-smoke package",
        ...
        "bisect_verification_command": "cargo run -p bisect-cli -- verify --manifest docs/examples/rplan-benchmark-packages/T.15+capacity-path100-benchmark/manifest.json",
        "scope_note": "Generated by the bisect-clustering capacity-constrained clustering routine over a deterministic 100-unit synthetic path. The package proves benchmark-tier packaging, construction lineage, and verifier acceptance, not real-data construction quality or wall-clock performance."
    });
