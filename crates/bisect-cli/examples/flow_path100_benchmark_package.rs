...
use bisect_flow::{construct_flow, FlowConfig};
use rplan_core::{
...
const PACKAGE_ID: &str = "T.17+flow-path100-benchmark";
const PAPER: &str = "T.17+flow-based-construction";
...
fn main() -> Result<()> {
    ...
    let config = FlowConfig::new(2, 0.01);
    let result = construct_flow(&adjacency, &populations, config)
        .map_err(|err| anyhow::anyhow!("construct flow benchmark plan: {err}"))?;
    if result.assignment.is_empty() {
        anyhow::bail!("flow benchmark generator produced no assignment");
    }

    let source_fixture = json!({
        "schema_version": "bisect-flow-synthetic-source-v1",
        "fixture_id": "path100-flow-synthetic",
    ...
    let mut units = PlanUnitIndex {
        ...
        unit_ids: (1..=populations.len())
            .map(|idx| format!("53017{:06}", idx * 100))
            .collect(),
        unit_universe_hash: String::new(),
        source_id: Some("path100-flow-synthetic".to_string()),
    };
    ...
    let document = RplanDocument {
        ...
        metadata: RplanMetadataV02 {
            ...
            description: Some(
                "Synthetic 100-unit path constructed by bisect-flow flow construction.".to_string(),
            ),
        },
        provenance: RplanProvenance {
            producer: BTreeMap::from([
                ("crate".to_string(), json!("bisect-cli")),
                ("method_crate".to_string(), json!("bisect-flow")),
                ("method".to_string(), json!("flow-construction")),
            ]),
            source_hashes: BTreeMap::from([("fixture".to_string(), source_fixture_hash)]),
            conversion_lineage: vec![json!({
                "workflow": "cargo run -p bisect-cli --no-default-features --example flow_path100_benchmark_package",
    ...
    let summary_path = out_dir.join("flow-summary.json");
    ...
    let transcript = json!({
        ...
        "source_workflow": "cargo run -p bisect-cli --no-default-features --example flow_path100_benchmark_package",
        "status": result.summary.status,
        "input_fixture": "path100-flow-synthetic",
        "unit_count": populations.len(),
        "edge_count_undirected": undirected_edge_count(&adjacency),
        "plan_hash": plan_hash,
        "context_hash": context.context_hash,
        "unit_universe_hash": units.unit_universe_hash,
        "summary_path": "flow-summary.json",
        "summary_sha256": summary_sha256,
        "assignment": result.assignment,
        "summary": result.summary,
        "scope_note": "Synthetic benchmark-tier package for flow construction lineage and verifier acceptance. It is not real-data quality, legal sufficiency, or wall-clock performance evidence."
    });
    ...
    let timing_notes = json!({
        ...
        "measurement_protocol": [
            "cargo run -p bisect-cli --no-default-features --example flow_path100_benchmark_package",
            "cargo run -p rplan-cli -- verify-certificate --certificate docs/examples/rplan-benchmark-packages/T.17+flow-path100-benchmark/audit-certificate.json --plan docs/examples/rplan-benchmark-packages/T.17+flow-path100-benchmark/plan.rplan --context docs/examples/rplan-benchmark-packages/T.17+flow-path100-benchmark/context.rctx",
            "cargo run -p bisect-cli -- verify --manifest docs/examples/rplan-benchmark-packages/T.17+flow-path100-benchmark/manifest.json"
        ],
    ...
    extra["input_fixture"] = json!("path100-flow-synthetic");
    extra["summary_path"] = json!("flow-summary.json");
    ...
    if certificate.result == rplan_audit::AuditResult::Fail {
        anyhow::bail!(
            "flow benchmark package audit failed; refusing to write public benchmark package"
        );
    }
    ...
    let files = vec![
        ...
        manifest_file(&out_dir, "flow-summary.json", "construction-report")?,
    ...
    let manifest = json!({
        ...
        "status": "constructed",
        "benchmark_scope": "deterministic synthetic flow-construction scale-smoke package",
        ...
        "bisect_verification_command": "cargo run -p bisect-cli -- verify --manifest docs/examples/rplan-benchmark-packages/T.17+flow-path100-benchmark/manifest.json",
        "scope_note": "Generated by the bisect-flow construction routine over a deterministic 100-unit synthetic path. The package proves benchmark-tier packaging, construction lineage, and verifier acceptance, not real-data construction quality or wall-clock performance."
    });
