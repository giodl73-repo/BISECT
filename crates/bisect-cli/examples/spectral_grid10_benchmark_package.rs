...
const PACKAGE_ID: &str = "T.14+spectral-grid10-benchmark";
...
fn main() -> Result<()> {
    ...
    let source_fixture = json!({
        ...
        "fixture_id": "grid10-synthetic",
        "grid_side": GRID_SIDE,
        "unit_count": populations.len(),
        "edge_count_undirected": undirected_edge_count(&adjacency),
        "populations": populations,
        "description": "One hundred tract-shaped synthetic units on a 10x10 rook-adjacency grid with equal populations.",
    });
    ...
    let mut units = PlanUnitIndex {
        ...
        unit_ids: (1..=populations.len())
            .map(|idx| format!("53001{:06}", idx * 100))
            .collect(),
        unit_universe_hash: String::new(),
        source_id: Some("grid10-synthetic".to_string()),
    };
    ...
    let document = RplanDocument {
        ...
        metadata: RplanMetadataV02 {
            ...
            description: Some(
                "Synthetic 10x10 grid partitioned by bisect-apportion spectral bisection."
                    .to_string(),
            ),
        },
        provenance: RplanProvenance {
            ...
            conversion_lineage: vec![json!({
                "workflow": "cargo run -p bisect-cli --no-default-features --example spectral_grid10_benchmark_package",
    ...
    let transcript = json!({
        ...
        "method": "spectral",
        "source_workflow": "cargo run -p bisect-cli --no-default-features --example spectral_grid10_benchmark_package",
        "status": "partitioned",
        "seed": null,
        "input_fixture": "grid10-synthetic",
        "grid_side": GRID_SIDE,
        "unit_count": populations.len(),
        "edge_count_undirected": undirected_edge_count(&adjacency),
        "plan_hash": plan_hash,
        ...
        "scope_note": "Synthetic benchmark-tier package for verifier scale and spectral lineage. It is larger than tiny fixtures and method-produced path fixtures, but it is not real-data performance evidence."
    });
    ...
    let timing_notes = json!({
        ...
        "package_id": PACKAGE_ID,
        "timing_kind": "deterministic-scale-smoke",
        "unit_count": populations.len(),
        "edge_count_undirected": undirected_edge_count(&adjacency),
        "reference_hardware": {
            "cpu": "not pinned",
        ...
        "measurement_protocol": [
            "cargo run -p bisect-cli --no-default-features --example spectral_grid10_benchmark_package",
            "cargo run -p rplan-cli -- verify-certificate --certificate docs/examples/rplan-benchmark-packages/T.14+spectral-grid10-benchmark/audit-certificate.json --plan docs/examples/rplan-benchmark-packages/T.14+spectral-grid10-benchmark/plan.rplan --context docs/examples/rplan-benchmark-packages/T.14+spectral-grid10-benchmark/context.rctx",
            "cargo run -p bisect-cli -- verify --manifest docs/examples/rplan-benchmark-packages/T.14+spectral-grid10-benchmark/manifest.json"
        ],
        "wall_clock_claim": "none",
        "scope_note": "This first benchmark-tier package records benchmark protocol and scale metadata. It is a verifier-scale smoke benchmark, not a formal runtime claim."
    });
    ...
    let manifest = json!({
        ...
        "status": "partitioned",
        "benchmark_scope": "deterministic synthetic scale-smoke package",
        ...
        "bisect_verification_command": "cargo run -p bisect-cli -- verify --manifest docs/examples/rplan-benchmark-packages/T.14+spectral-grid10-benchmark/manifest.json",
        "scope_note": "Generated by the bisect-apportion spectral bisection routine over a deterministic 100-unit synthetic grid. The package proves benchmark-tier packaging, scale-smoke verification, and lineage, not real-data construction quality or wall-clock performance."
    });
