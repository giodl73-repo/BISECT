    ...
    if bin_path.exists() {
        ...
        eprintln!(
            "WARNING: {} not found — falling back to legacy .pkl shim. \
             Build the .adj.bin via: bisect fetch --year YEAR --states <CODE>",
            bin_path.display()
        );
...
mod tests {
    ...
    fn pkl_fallback_message_contains_useful_hint() {
        // The fallback message for missing .adj.bin should mention bisect fetch
        // This is verified by reading the source string in load_adjacency_pkl
        let warning_snippet = "Build the .adj.bin via: bisect fetch";
