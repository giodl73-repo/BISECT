...
pub fn load_centroids_from_tiger(
    ...
    let dbf_path =
        format!("data/{year}/tiger/tracts/tl_{year}_{fips}_tract/tl_{year}_{fips}_tract.dbf");
    if !Path::new(&dbf_path).exists() {
        return HashMap::new();
    }

    let geoid_to_idx: HashMap<String, usize> = index_to_geoid
        .iter()
        .map(|(&i, g)| (g.clone(), i))
        .collect();
...
fn read_tiger_centroids(dbf_path: &str) -> HashMap<String, (f64, f64)> {
    ...
    let mut geoid_off = None;
    let mut lat_off = None;
    let mut lon_off = None;
        ...
        .map(|(&(u, v), &w)| {
                ...
                (centroids.get(&u), centroids.get(&v))
            {
                ...
                while theta > std::f64::consts::FRAC_PI_2 {
                    theta = (std::f64::consts::PI - theta).abs();
                }
...
mod tests {
    ...
    fn pca_minor_axis_horizontal_band() {
        ...
        let centroids: CentroidMap = (0..20)
            .map(|i| {
    ...
    fn compute_minor_axis_empty_returns_none() {
        ...
        assert!(
            compute_minor_axis(&verts, &centroids).is_none(),
    ...
    fn compute_minor_axis_returns_some_for_two_distinct_points() {
        ...
        let verts: std::collections::HashSet<usize> = vec![0usize, 1].into_iter().collect();
        assert!(
            compute_minor_axis(&verts, &centroids).is_some(),
    ...
    fn apply_directional_penalty_zero_lambda_no_change() {
        ...
        assert!(
            (result[&(0, 1)] - 100.0).abs() < 1e-9,
            "lambda=0 must leave weight unchanged"
        );
        assert!(
            (result[&(2, 3)] - 250.0).abs() < 1e-9,
            "lambda=0 must leave weight unchanged"
        );
    }
    ...
    fn apply_directional_penalty_always_positive() {
        ...
        for (&edge, &w) in &result {
            assert!(
                w > 0.0,
                "weight for edge {edge:?} must be positive, got {w}"
            );
        }
    }
    ...
    fn compute_minor_axis_diagonal_band_ne_sw() {
        ...
        let centroids: CentroidMap = (0..4usize).map(|i| (i, (i as f64, i as f64))).collect();
        let verts: std::collections::HashSet<usize> = (0..4).collect();
        // Must return Some and have a finite angle.
        let angle = compute_minor_axis(&verts, &centroids).unwrap();
        assert!(
            angle.is_finite(),
            "angle must be finite for diagonal band, got {angle}"
        );
    }
    ...
    fn compute_minor_axis_large_cluster_is_finite() {
        ...
        let centroids: CentroidMap = (0..50usize)
            .map(|i| {
        ...
        assert!(
            angle.is_finite(),
            "large cluster must yield finite angle, got {angle}"
        );
    }
    ...
    fn apply_directional_penalty_very_large_lambda_still_positive() {
        ...
        assert!(
            result[&(0, 1)] > 0.0,
            "weight must be positive even with lambda=1000"
        );
    }
    ...
    fn apply_directional_penalty_large_lambda_scales_perpendicular_edge() {
        ...
        assert!(
            (w - 11.0).abs() < 1e-4,
            "perpendicular edge with lambda=10 must scale to 11.0, got {w}"
        );
    }
    ...
    fn apply_directional_penalty_preserves_edge_count() {
        ...
        assert_eq!(
            result.len(),
            ew.len(),
            "output must have same number of edges as input"
        );
    }
