...
use bisect_metis::graph::CsrGraph;
use bisect_metis::api::{MetisPartitioner, MetisParams, Partitioner};
...
fn split_weighted_asymmetric_fracs_errors_on_empty() {
    ...
    assert!(matches!(result, Err(bisect_metis::PartitionError::ZeroParts)));
}
...
fn all_oracle_partitions_are_contiguous() {
    use bisect_metis::graph::check_contiguity;
...
fn repair_contiguity_fixes_broken_partition() {
    use bisect_metis::graph::{repair_contiguity, check_contiguity};
    ...
    let g = make_path(6);
    let mut p = bisect_metis::Partition {
