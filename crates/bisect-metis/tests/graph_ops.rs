...
use bisect_metis::graph::{
    CsrGraph, Partition, check_contiguity, repair_contiguity, extract_subgraph,
};
use bisect_metis::api::{
    MetisPartitioner, MetisParams, Partitioner,
    CoarseningMethod, ObjectiveType,
};
use bisect_metis::coarsen::Coarsener;
use bisect_metis::coarsen::hem::{HeavyEdgeMatch, build_coarse_graph};
use bisect_metis::coarsen::shem::{SortedHeavyEdgeMatch, SortedHeavyEdgeMatchWithParams};
use bisect_metis::multilevel::hierarchy::CoarseningHierarchy;
...
fn csr_is_valid_returns_true_for_well_formed_graph() {
    ...
    assert!(g.is_valid(), "5×5 grid must pass is_valid()");
}
...
fn metis_uniform_bisection_within_ufactor_tolerance() {
    ...
    // ufactor=5 → tolerance = 5/1000 * total = 0.5%; for a 20-vertex graph the
    // exact integer balance may exceed 0.5% but must be well under 10%.
    // We verify the structural invariant: both parts are non-empty and the
    // per-part population is within 2 vertices of the target (10 ± 2).
