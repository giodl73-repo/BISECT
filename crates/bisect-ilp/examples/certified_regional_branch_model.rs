use bisect_ilp::{
    compile_certified_split_regional_boundary_relaxation, CertifiedSplitDiscovery,
    CertifiedSplitInstance, RegionalPopulationConstraint,
};
use std::error::Error;
use std::path::Path;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 6 {
        return Err(
            "usage: certified_regional_branch_model <instance.json> <discovery.json> <right-population> <regional-constraints.json> <out-dir>"
                .into(),
        );
    }
    ...
    println!(
        "Certified regional branch: {} variables, {} constraints",
