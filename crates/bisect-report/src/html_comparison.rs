...
fn render_provenance_footer(report: &ComparisonReport, ctx: &HtmlComparisonContext) -> String {
    format!(
        "<footer class=\"provenance\">\n\
         ...
         <dt>bisect version</dt><dd><code>{ver}</code> (build <code>{commit}</code>)</dd>\n\
         ...
         <code>bisect compare --plan-a {a_label} --plan-b {b_label} --format html</code>.\
         </p>\n\
         </footer>\n",
        ...
        ver = escape_html(&ctx.version),
        commit = escape_html(&ctx.build_commit_short),
...
mod tests {
    ...
    fn fixture_ctx() -> HtmlComparisonContext {
        HtmlComparisonContext {
            ...
            build_commit_short: "deadbeef".into(),
            version: "0.1.0".into(),
    ...
    fn provenance_footer_includes_all_shas() {
        ...
        assert!(html.contains("0.1.0"), "bisect version");
    }
