...
pub fn generate_verification_command(
    ...
    let adj_check = format!(
        ...
        \n# If not present locally: bisect fetch --type adjacency --states {} --year {}",
    ...
    format!(
        ...
        bisect state --state {state} --year {year} --chamber {chamber} --districts {num_districts} --label {label}{seed}\n\
        {tiger_check}\n\
...
mod tests {
    ...
    fn test_verification_script_contains_bisect_state() {
        ...
        let script = generate_verification_script(&manifest);
        assert!(
            script.contains("bisect state"),
            "verification_script must contain the bisect state command, got: {script}"
        );
    }
