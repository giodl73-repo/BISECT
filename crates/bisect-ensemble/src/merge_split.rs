...
impl MergeSplitChain {
    ...
    fn try_step<R: Rng>(&mut self, rng: &mut R, rng_reverse: &mut R) -> StepRecord {
        ...
        let max_attempts = (self.k as usize)
            .min(pairs.len())
            .max(DEFAULT_MAX_PAIR_ATTEMPTS.min(pairs.len()));
...
fn count_balanced_cuts(
    ...
    for (a, b) in tree.edges() {
        ...
        let pop_a: i64 = comp_a
            .iter()
            .map(|&local| pop[region[local as usize] as usize])
            .sum();
...
mod tests {
    ...
    fn grid_chain_4x4() -> MergeSplitChain {
        ...
        for r in 0..4usize {
            for c in 0..4usize {
                ...
                if r + 1 < 4 {
                    adj[v].push((v + 4) as u32);
                    adj[v + 4].push(v as u32);
                }
            }
        }
    ...
    fn merge_split_acceptance_rate_in_range() {
        ...
        assert!(
            rate > 0.0,
            "acceptance rate must be > 0 on a well-connected grid"
        );
    ...
    fn merge_split_deterministic() {
        ...
        for _ in 0..30 {
            ...
            assert_eq!(
                rec1.valid_cuts_forward, rec2.valid_cuts_forward,
                "fwd cuts must match"
            );
            assert_eq!(
                rec1.valid_cuts_reverse, rec2.valid_cuts_reverse,
                "rev cuts must match"
            );
        ...
        assert_eq!(
            ...
            "same seeds → same final assignment"
        );
    }
    ...
    fn merge_split_ratio_direction_correct() {
        ...
        for _ in 0..500 {
            ...
            if rec.valid_cuts_forward > 0 && rec.valid_cuts_reverse > 0 {
                ...
                assert!(
                    ...
                    rec.ratio,
                    expected,
                    rec.valid_cuts_forward,
                    rec.valid_cuts_reverse
                );
        ...
        assert!(
            found_nontrival,
            "must encounter at least one step with both nonzero cut counts"
        );
    }
