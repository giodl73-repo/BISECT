...
impl<'de> Deserialize<'de> for StepRecord {
    fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        ...
        Ok(StepRecord {
            step: h.step,
            cut_edges: h.cut_edges,
            cut_fraction: h.cut_fraction,
            pop_deviation: h.pop_deviation,
            accepted: h.accepted,
        })
    }
}
...
pub fn run_ensemble(
    ...
    let pooled_cut_std = {
        let var = all_cuts
            .iter()
            .map(|&x| (x - pooled_cut_mean).powi(2))
            .sum::<f64>()
            / all_cuts.len().max(1) as f64;
        var.sqrt()
    };

    let r_hat = if n_chains >= 2 {
        Some(compute_r_hat(&chains))
    } else {
        None
    };
...
fn compute_r_hat(chains: &[ChainResult]) -> f64 {
    ...
    let chain_means: Vec<f64> = chains
        .iter()
        .map(|c| c.steps.iter().map(|s| s.cut_fraction as f64).sum::<f64>() / n)
        .collect();
    ...
    let w = chains
        ...
        .map(|c| {
            ...
            c.steps
                .iter()
                .map(|s| (s.cut_fraction as f64 - mu).powi(2))
                .sum::<f64>()
                / (n - 1.0)
        })
        .sum::<f64>()
...
mod tests {
    ...
    fn chain_seed_all_different_for_100_indices() {
        let seeds: std::collections::HashSet<u64> = (0..100).map(|i| chain_seed(42, i)).collect();
    ...
    fn two_chains_give_r_hat() {
        ...
        for r in 0..4usize {
            for c in 0..5usize {
                ...
                if r + 1 < 4 {
                    adj[v].push((v + 5) as u32);
                    adj[v + 5].push(v as u32);
                }
            }
        }
    ...
    fn hamming_autocorr_has_correct_length() {
        ...
        assert_eq!(
            result.hamming_autocorr.len(),
            20,
            "must have lag-1 through lag-20"
        );
    }
    ...
    fn different_base_seeds_produce_different_chains() {
        ...
        assert_ne!(
            cuts1, cuts2,
            "different seeds should produce different cut sequences"
        );
    }
    ...
    fn pooled_std_is_zero_for_constant_cut() {
        ...
        assert!(
            result.pooled_cut_std < 0.01,
            "std should be near-zero for constant cut"
        );
    }
}
