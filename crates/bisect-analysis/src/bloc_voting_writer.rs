...
/// fills these from the CLI args + `bisect-cli/src/provenance.rs`.
#[derive(Debug, Clone)]
...
pub fn render_summary_md(json: &BlocVotingJson) -> String {
    ...
    s.push_str(&format!(
        "Schema: {}\nbinary: {} (commit {})\n\n",
        json.schema_version, json.provenance.version, json.provenance.build_commit
    ));
...
mod tests {
    ...
    fn fixture_provenance_block() -> ProvenanceBlock {
        ProvenanceBlock {
            ...
            build_commit: "deadbeef0000000000000000000000000000beef".into(),
            build_commit_short: Some("deadbeef".into()),
