...
\[
  \mathrm{gap}(\mathtt{ubvec}[1]) \geq \mathrm{gap}(\text{T.2}) - C \cdot \sigma
\]
where $C$ is a state-specific constant bounded by the Lorenz curve slope at $x = k_R/k$.
For Wisconsin ($\sigma = 0.003$) and Georgia ($\sigma = 0.001$), the T.5 constraint
cannot reduce the proportionality gap by more than $C \cdot 0.003$ and $C \cdot 0.001$
respectively, regardless of $\eta = \mathtt{ubvec}[1]$.

This analytically establishes that the tradeoff curve for competitive states is
flat: no amount of partisan constraint closes the Rodden gap.
