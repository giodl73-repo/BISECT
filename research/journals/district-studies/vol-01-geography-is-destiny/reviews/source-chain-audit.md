...
|--------|-----|----------------|------|
| T.5 ProportionalSection | Formal geography/proportionality argument | Strong lead candidate; separate audit note complete | HOLD numeric claims until METIS parameter vector, seed variance, and C(G) estimation are documented |
...
| T.4 ApportionRegions | NC 7D/7R and prime-factor bisection context | Useful supporting source | Keep descriptive; avoid tool advocacy |
...
- **T.5 reproducibility gap**: Round 3 review flags missing METIS `niter`,
  `ncuts`, and `numbering` values. Treat T.5 as strong but not fully locked
  ...
  do not by themselves prove the T.5 paper-run vector.
- **T.5 seed variance gap**: Round 3 reviews ask whether Table 1 values are
...
- **T.5 C(G) provenance**: Round 3 review asks whether C(G) is
  Lorenz-analytical or METIS-empirical. Do not present C(G) constants as
  settled until the estimator is named.
- **T.5 legal-scope leakage**: the source's `Scope of Claims` section is
...
- **T.5 scope strength**: Review notes praise the "Scope of Claims" section,
  especially the warning that proportional constraints do not guarantee
  proportional outcomes. Preserve that caveat.
- **T.5 cross-source grounding**: Review notes still ask for GA/WI
  reconciliation with T.2 data. Do not publish GA/WI numeric gaps until this is
...
- **Legal overread**: T.5-style geography findings do not prove partisan intent
...
1. Close the T.5 gates listed in `reviews/b12-reproducibility-and-scope-audit.md`
   and `reviews/b12-implementation-provenance-note.md`.
2. Add T.5 METIS `niter`, `ncuts`, and `numbering` values to the source chain.
