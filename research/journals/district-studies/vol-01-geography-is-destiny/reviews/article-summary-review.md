...
status: article-summary-review
updated: 2026-05-09
review-target: articles/
---
...
The article-summary package is suitable for **audition display** and internal
editorial selection. It is not yet suitable for **provisional** because the
central quantitative gates remain open.

The summaries do the most important public-cleanup job: they make the issue's
diagnostic promise without turning it into an adoption claim, a legal claim, or
a theorem.

## Board Review

| Role | Finding | Publication effect |
|------|---------|--------------------|
...
| DATUM | The package has a usable index, lineup, source request file, and ledger. Add paper-ledger updates whenever candidates move. | pass |
| COMMONS | The summaries are readable but still skeletal. Before public lock, each article needs a clearer lead, one concrete example, and one source note. | revision before lock |
| LEDGER | The paper-use ledger prevents accidental reuse but needs to become part of the normal issue-promotion checklist. | pass with process note |
| SURVEY | The reader promise is now bounded enough to show as an audition packet. A public reader will still need signposting that this is a staged research press. | revision before lock |
| TRENCH | The issue is not adversarially complete. It correctly leaves gaming, adoption, and proof standards to later issues. | pass |

## Promotion Decision

Do not promote Vol. 1 to provisional yet.
...
The scope-down path is faster and probably better for a first public issue. It
lets Vol. 1 explain the frame while later issues carry the heavier numerical
claims after the source chain is closed.

## Required Edits Before Provisional
...
> District Studies Vol. 1 is an audition issue about voter geography. It asks
> how much partisan outcome is already constrained by where voters live before
> any claim about reform, adoption, or legal proof. The current package is a
> curated draft, not a locked publication.
