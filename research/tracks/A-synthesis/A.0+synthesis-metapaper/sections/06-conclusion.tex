...
The sequential-cut ceremony adds an implemented publication layer to that
agenda: $k-1$ cut decisions are bound into depth-ordered, reviewable releases
rather than revealed only as one completed map. Its software transition and
prefix-verification contract is tested. A bounded four-district fixture now
regenerates a separate procedural scoreboard and rejects a timestamp mutation,
but uses synthetic receipts; an externally witnessed State ceremony and
prospective State-scale ceremony-enabled bakeoff are not yet completed evidence.
