...
\noindent\textbf{Superseded draft warning.} This supplement records an early
demographic-similarity proposal and is not the method used by the current
geographic benchmark. It must not be cited as the authoritative BISECT edge
weight formula or as evidence of VRA compliance. The current default uses
TIGER shared-boundary length; VRA-aligned weighting is a separate,
race-conscious profile documented in the current code and concept guide.
...
\subsection{Historical Draft Summary}
...
This superseded formula is preserved only to document an earlier proposal. It
does not verify or reproduce the current benchmark results.
