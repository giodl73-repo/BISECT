...
\begin{itemize}[leftmargin=*,noitemsep,topsep=4pt]
    ...
    \item \textbf{Tracks G--H}: Ensemble methods, comparison, and search strategies
...
This structure enables progressive deepening: Tracks B--H generate empirical, algorithmic, legal, and ensemble results; Tracks I--M extend the program to incumbency, apportionment, compactness, partisan fairness, and community character weights. Track A synthesizes across all twelve tracks.

\section*{Paper Summaries}

The following summaries cover the eleven foundation papers (B.1--B.11 / original series) that initiated the program. The portfolio is an ongoing research program; papers across all twelve tracks are at varying stages of completion.
...
\subsection*{Track H --- Ensemble Search Strategies (4 papers)}

Track H translates Track G's ensemble science into deployable statutory tools. PercentileSweep (H.0, \emph{Political Analysis}) provides statutory legal posture selection: by choosing a plan at the $p$th compactness percentile of the ensemble, practitioners can select among compactness doctrine ($p=0.0$), representativeness doctrine ($p=0.5$), or full ensemble representativeness. BisectionEnsemble (H.1, ICML) embeds local ReCom exploration at each bisection node, combining the bisection tree structure with ensemble sampling. The \texttt{redist-ensemble} Rust implementation (H.2, USENIX ATC) achieves 2,500$\times$ the speed of Python GerryChain, enabling real-time plan generation during commission deliberations. Resolution-Aware Redistricting (H.3) formalises geographic granularity as a first-class parameter with a manifest system enabling independent plan verification. All four Track H papers are accepted (mean 3.35/4).
