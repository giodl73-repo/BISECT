...
\large 190+ Papers Across 18 Research Tracks}
...
Congressional redistricting in the United States suffers a legitimacy crisis: partisan mapmakers manipulate district boundaries to predetermine election outcomes, eroding public confidence in representative democracy. This research program of 190+ Papers Across 18 Research Tracks demonstrates that \textbf{recursive graph bisection}---a computational method used for supercomputer workload distribution---provides an algorithmic alternative that \emph{cannot be instructed to gerrymander because it cannot access partisan data}.
...
\begin{itemize}[leftmargin=*,noitemsep,topsep=4pt]
    \item \textbf{Philosophical}: \emph{Impossibility defense}---algorithm cannot see partisan data, therefore cannot be instructed to gerrymander; compact neutral maps can still reflect geographic voter concentration
    \item \textbf{Technical}: Edge-weighted graph partitioning achieves +56\% compactness improvement over unweighted METIS bisection while enabling VRA compliance
...
The portfolio follows a track-based structure reflecting methodological dependencies:

\begin{itemize}[leftmargin=*,noitemsep,topsep=4pt]
    \item \textbf{Track A}: Portfolio synthesis and overview (this paper series)
    \item \textbf{Tracks B--C}: Algorithm design, variants, validation, and robustness
    \item \textbf{Track D}: VRA and legal compliance
    \item \textbf{Tracks E--F}: Experimental extensions and state legislative redistricting
    \item \textbf{Track G}: Ensemble methods, comparison, and diagnostics
    \item \textbf{Tracks T--U}: Plan construction plus search, optimization, and selection
    \item \textbf{Tracks I--J}: Incumbency effects and apportionment methods
    \item \textbf{Tracks K--L}: Compactness taxonomy and partisan fairness metrics
    \item \textbf{Track M}: Community character weights (economic, land use, housing, commuting, topographic, administrative, transit) — operationalises the ``communities of interest'' redistricting criterion through seven measurable, non-partisan edge-weight signals
    \item \textbf{Track N}: Population counting — who counts in the balance metric; N.5 provides the definitive 50-state comparison of total population vs.\ citizen VAP vs.\ prison-adjusted; Evenwel v.\ Abbott implications for algorithmic redistricting
    \item \textbf{Tracks O}: Outcomes and democratic quality — the Representation Quality Index (RQI) synthesises electoral competitiveness ($+23$pp), voter turnout ($+1.1$pp), legislative polarization ($-0.07$ DW-NOMINATE), and constituent distance ($-14$min)
    \item \textbf{Track P}: Reform pathways — the only track showing how algorithmic redistricting gets adopted; P.4 provides model court orders, P.5 shows implementation costs under \$50{,}000
    \item \textbf{Track Q}: 2030 forward analysis — demographic projections, data infrastructure planning, TX $k=41$ prime validation
    \item \textbf{Track R}: Adversarial robustness — gaming resistance proof; all vectors bounded $< 0.5$ seats nationally and detectable via audit chain
    \item \textbf{Track S}: Statistical inference — ESS-corrected p-values, Bayesian Detection Score, FDR-corrected gerrymandering detection across all L-track metrics
\end{itemize}

This structure enables progressive deepening: Tracks B--G generate empirical, algorithmic, legal, and ensemble results; Tracks I--M extend the program to incumbency, apportionment, compactness, partisan fairness, and community character weights; Tracks T--U separate plan construction from search, optimization, and selection. Track A synthesizes across the active tracks.

\section*{Paper Summaries}

The following summaries cover the eleven foundation papers (B.1--T.4 / original series) that initiated the program. The portfolio is an ongoing research program; papers across all twelve tracks are at varying stages of completion.

\subsection*{Paper 01: Recursive Bisection for Congressional Redistricting}

This foundation paper establishes the core method and philosophical framework. We demonstrate that recursive graph bisection---the same algorithm used to distribute workloads across supercomputer processors---can partition census tracts into congressional districts meeting all constitutional requirements. The method operates exclusively on population counts and geographic adjacency, structurally unable to access partisan voter data. This creates an \emph{impossibility defense}: the algorithm cannot be instructed to gerrymander not because it chooses not to, but because it lacks the information required to do so. Compact neutral maps can still reflect geographic voter concentration, as shown in Track C.5. Applied to all 50 states, the method produces 435 districts with population equality within 0.5\%, full geographic contiguity, and Polsby-Popper compactness scores 23\% higher than enacted plans. The paper frames algorithmic redistricting as extending the Huntington-Hill precedent for congressional apportionment (successful since 1941) to the boundary design problem, demonstrating that mathematical governance can restore electoral legitimacy through structural objectivity rather than institutional reform.
...
\subsection*{Track G --- Ensemble Methods and Comparison (15 papers)}

Track G answers the question: where does the bisection plan sit in the full space of all valid redistricting plans? The ensemble-baseline sub-track (G.0--G.5) establishes that bisection plans sit at the 0.1--0.7th compactness percentile in the feasible-plan ensemble for most states --- near the compactness extremum, not the partisan extremum. The MCMC-algorithms sub-track (G.6--G.13) contributes eight specific ensemble search methods, each implemented in the \texttt{bisect} Rust binary as a \texttt{--search} flag: short-burst optimisation (G.6), flip proposals for local sensitivity analysis (G.8), Forest ReCom via spanning forest sampling (G.9), merge-split MCMC with explicit reversibility (G.10), multiscale MCMC for large-$k$ chambers (G.11), short-burst with calibrated chains (G.12), and VRA-aware chains preserving majority-minority districts (G.13). The track's synthesis paper, G.14 (\emph{A Practitioner's Comparison of Redistricting Ensemble Algorithms}, accepted at 4.0/4), is the highest-scored paper in the full 82-paper portfolio, providing a decision tree that routes practitioners to the appropriate algorithm for their use case.

\subsection*{Track U --- Search and Optimization (11 papers)}

Track U translates Track G's ensemble science and Track T's construction methods into deployable search, optimization, and certification tools. PercentileSweep (U.8, \emph{Political Analysis}) provides statutory legal posture selection: by choosing a plan at the $p$th compactness percentile of the ensemble, practitioners can select among compactness doctrine ($p=0.0$), representativeness doctrine ($p=0.5$), or full ensemble representativeness. BisectionEnsemble (U.9, ICML) embeds local ReCom exploration at each bisection node, combining the bisection tree structure with ensemble sampling. The \texttt{redist-ensemble} Rust implementation (U.10, USENIX ATC) achieves 2,500$\times$ the speed of Python GerryChain, enabling real-time plan generation during commission deliberations. Resolution-Aware Redistricting (U.11) formalises geographic granularity as a first-class parameter with a manifest system enabling independent plan verification. Track U also houses ConvergenceSweep, parameter sensitivity, simulated annealing, parallel tempering, ILP certification, and Pareto/NSGA-II multi-objective search.

\section*{Portfolio-Level Themes}

\subsection*{Impossibility Defense (Papers 01, 11)}
The core philosophical innovation: algorithms cannot be instructed to gerrymander because they structurally lack access to partisan data. Paper 01 establishes this principle theoretically; Paper 11 validates it empirically by demonstrating 62\% bias reduction without any partisan fairness objective in the algorithm specification. Compact neutral maps can still reflect geographic voter concentration.
...
\begin{description}[leftmargin=2.5cm,style=nextline,noitemsep,topsep=4pt]
    ...
    \item[Impossibility defense] Algorithm cannot be instructed to gerrymander by design: it structurally cannot access partisan data, unlike commission-drawn maps; compact neutral maps can still reflect geographic voter concentration
...
\textbf{Publication status}: This is an ongoing research program (190+ papers across 20 active tracks). Track B foundations, several Track G ensemble papers, the former Track H papers now merged into Track U, and selected Track T and U papers are accepted or ready for submission. Remaining papers across all tracks are in draft. Preprints will be available on arXiv upon initial submission.

\textbf{For the entire research program}:

\begin{quote}
Della-Libera, G. (2026). Algorithmic redistricting: A 190+ paper research program across 18 tracks. Preprint series.
\end{quote}
...
\noindent\textbf{About this summary}: This 6-page research statement provides detailed descriptions of the foundation papers of the algorithmic redistricting portfolio (190+ Papers Across 18 Research Tracks). For a brief overview, see the 3-page \emph{Guide to the Algorithmic Redistricting Portfolio}. For full findings, see individual papers. For synthesis across papers, see Paper A.0 (Synthesis Metapaper for \emph{Science}).
