...
\texttt{bisect fetch --year 2020}.
Verification of individual file hashes is performed by
\texttt{bisect label-verify} (see Section~\ref{sec:verification}).

\noindent \textbf{Download URLs} (automated via \texttt{bisect fetch}):
...
\subsection*{Fetching Data with \texttt{bisect fetch}}

The recommended way to obtain data is through the \texttt{bisect fetch} command,
which downloads, validates, and caches all required files:

\begin{verbatim}
bisect fetch --year 2020               # Download 2020 data (all 50 states)
bisect fetch --year 2020 --state VT    # Vermont only (~2 min, good for testing)
bisect fetch --year 2020 --check-only  # Verify cache, do not download
\end{verbatim}

\noindent The fetch command validates the SHA-256 of every downloaded file
against the manifest embedded in the \texttt{bisect} binary.
If a file is corrupted or modified, the command refuses to proceed.

\subsection*{Election Data}

Partisan data used in partisan-outcome metrics (Papers C.5, T.4, and related analyses)
...
\noindent This step is separate from \texttt{bisect fetch} and must be run independently.
