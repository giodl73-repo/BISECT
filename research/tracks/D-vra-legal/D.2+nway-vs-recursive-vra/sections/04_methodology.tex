...
\textbf{Platform}: \texttt{bisect} Rust binary with \texttt{--partition-mode metis-vra} and \texttt{nway} modes. Executed on consumer laptop (Windows 11, 16GB RAM) using \texttt{bisect states --workers 4} (sufficient for largest state, California: 9,129 tracts).
...
Total partitioning runs: 1,760 (880 n-way + 880 recursive bisection). Actual computational time: N-way: 0.90 hours, Recursive: 2.77 hours (total 3.67 hours on consumer laptop using \texttt{bisect states --workers 4}).
