...
\begin{itemize}
    ...
    \item \texttt{ufactor}: Population tolerance factor = 1.005 ($\pm$0.5\% of target)
...
\textbf{Objective}: Minimize edge cut while attempting to match target demographic compositions per district. The relaxed demographic tolerance ($\pm$20\%) allows flexibility when exact targets are infeasible.
...
\textbf{Intuition}: Edges between ``minority tracts'' (above threshold $\theta$) receive weight $w$ (e.g., 5$\times$ or 10$\times$ higher than normal edges). METIS minimizes \emph{weighted} edge cut, making it expensive to separate minority tracts. This naturally clusters minority populations without rigid demographic constraints.
...
\begin{itemize}
    ...
    \item \textbf{$w = 5, 10$}: Moderate weighting (5-10$\times$ higher cost to cut minority-minority edges)
...
\begin{itemize}
    ...
    \item \textbf{Constraints}: Contiguity (all districts connected), population balance ($\pm$0.5\%), 7 districts
...
\begin{itemize}
    ...
    \item \texttt{bisect} Rust binary (redistricting computation); Python 3.13 (metric computation, workflow orchestration)
...
\input{results/computational_complexity/timing_table.tex}
...
\begin{itemize}
    ...
    \item \textbf{Overhead factor}: 1.12$\times$ on average (edge-weighting adds minimal computational cost)
...
\textbf{Full Parameter Sweep}: Our experimental design tests 105 configurations (21 per state $\times$ 5 states). Total runtime: $105 \times 0.47s \approx 50$ seconds $<$ 1 minute. Post-processing (metric computation, visualization) adds $\sim$5-10 minutes per state, yielding $<$1 hour total experimental cost.
...
\textbf{Scaling Factor}: Block-level graphs have $\sim$35$\times$ more vertices. With $O(|E| + |V|\log|V|)$ complexity, expected runtime scaling is:
\[
...
\textbf{Conclusion}: Edge-weighted optimization is computationally inexpensive. The 12\% overhead is negligible compared to the substantial VRA compliance benefits demonstrated in our results (Alabama: 0 MM $\rightarrow$ 2 MM with 3.2\% compactness improvement).

\subsubsection{Production Adaptive Formula}

The ablation study above tests fixed weight factors ($w \in \{1, 5, 10, 50, 100\}$). The V4 production pipeline uses an adaptive formula that sets $w$ automatically per state without manual tuning~\citep{dellaLibera2026D0}:

\begin{equation}
w = \max\!\left(3.0,\; 10.0 \times \bigl(1.0 - 0.7 \times f_{\text{minority}}\bigr)\right)
\label{eq:adaptive_boost_d3}
\end{equation}

where $f_{\text{minority}}$ is the fraction of state tracts with minority percentage $\geq \tau$ ($\tau = 0.40$ by default). Verified V4 production outcomes for our five study states (2026-04-24):
