...
Edge-weighted optimization corrects this by making minority-minority edges \emph{expensive} to cut (e.g., 5$\times$ higher weight). METIS then minimizes \emph{weighted} edge cut, naturally keeping minority clusters together. Crucially, because these clusters are already geographically compact (high Moran's $I$), preserving them creates districts with better geometric compactness than arbitrary divisions.
...
Edge-weighted optimization effectively searches a different region of the solution space---one that respects demographic geography. Because minority clustering aligns with geographic compactness (high Moran's $I$), this alternative solution space can contain configurations that dominate baseline on both VRA compliance and compactness. Alabama's 5$\times$@45\% configuration is precisely such a dominant point: it achieves 2 MM districts (vs. 0 for baseline) \emph{and} reduces edge cut by 9.3\%.
...
\begin{itemize}
    \item Alabama 5$\times$@45\%: 2 MM, edge cut $-9.3$\%, PP +3.2\%
    \item Alabama 5$\times$@40\%: 2 MM, edge cut $-1.4$\%, PP $-2.7$\%
    \item Georgia 5$\times$@55\%: 6 MM, edge cut $-11.2$\%, PP +22.2\%
    \item Mississippi 1$\times$@40\%: 2 MM, edge cut 0.0\%, PP 0.0\%
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Population balance}: All configurations maintain $\pm$0.5\% deviation from target population, satisfying equal protection requirements.
    ...
    \item \textbf{Compactness (geometric)}: METIS minimizes \emph{weighted} edge cut. Edge weights reflect \emph{both} geographic distance (tract boundary lengths) and demographic composition. A 5$\times$ weight means the algorithm treats cutting a minority-minority boundary as equivalent to cutting five geographic boundaries---still a \emph{compactness} calculation, just one that values community cohesion.
...
\begin{itemize}
    ...
    \item \textbf{Non-Predominant Factor}: Race influences optimization but does not dictate outcomes. South Carolina's failure to reach 3 MM districts despite 1000$\times$ weights proves that geometric constraints override racial targets when the two conflict.
\end{itemize}
...
\textbf{Example Application to Alabama's Multi-Constraint Plan}: Alabama's multi-constraint approach (Paper 4 result: 0 MM districts, 280 edge cut, 0.234 PP) is \emph{dominated} by the edge-weighted 5$\times$@45\% configuration (2 MM districts, 254 edge cut, 0.242 PP). The edge-weighted plan is better on \emph{both} objectives, making multi-constraint indefensible.
...
\textbf{Frontier Presentation}: Legislatures can present the full Pareto frontier to the public, showing all optimal configurations (e.g., Alabama's frontier includes 5$\times$@40\%, 5$\times$@45\%, possibly others). Each point on the frontier represents a defensible choice with clear tradeoffs:
\begin{itemize}
    \item \textbf{5$\times$@40\%}: 2 MM, edge cut 276 (better compactness, VRA success)
    \item \textbf{5$\times$@45\%}: 2 MM, edge cut 254 (best compactness, VRA success)
\end{itemize}
...
Despite testing 20 aggressive configurations (weights up to 1000$\times$, thresholds as low as 30\%), South Carolina achieves at most 1 MM district, far short of its 3-district target. This failure is \emph{not} due to poor algorithmic performance but rather fundamental arithmetic constraints.
...
South Carolina's 386 high-minority tracts ($\geq$50\%) are spread across several clusters (Charleston area, Columbia, parts of coastal regions, rural Black Belt). Each cluster alone is too small to form a 50\% minority district (need ~189 tracts per district $\times$ 50\% = ~94 high-minority tracts per MM district). Connecting multiple clusters requires including many low-minority tracts to maintain contiguity, diluting the minority percentage below 50\%.

\textbf{Population Balance Constraint}: Even if tracts could be arbitrarily selected, population balance ($\pm$0.5\% of target) further constrains feasibility. High-minority tracts may have systematically different population densities (e.g., urban vs. rural), making it difficult to combine exactly 3 subsets that each reach 50\% minority \emph{and} equal population.
...
\begin{itemize}
    \item Favorable geography (clustering exists but isn't leveraged) $\rightarrow$ edge-weighting likely succeeds
    \item Unfavorable geography (clustering is weak or dispersed) $\rightarrow$ edge-weighting may fail
\end{itemize}
...
\input{results/enacted_comparison/enacted_comparison_table.tex}
