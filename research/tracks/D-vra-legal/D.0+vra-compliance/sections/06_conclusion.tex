...
\textbf{(2) Edge-Weighting Breakthrough}: Single-objective optimization with weighted edges doubles success rate to 80\% (4/5 states) with per-state-optimal configurations, versus 40\% for multi-constraint. With the V4 production adaptive formula (Eq.~\ref{eq:adaptive_boost}), full legal targets are met in 2/5 states; Louisiana and Mississippi each achieve 1 of 2 target districts due to stochastic boundary sensitivity (see Section~\ref{sec:discussion_adaptive}). Alabama breakthrough: 0 MM $\to$ 2 MM districts (50.8\% minority, confirmed in production).
...
\textbf{Courts}: Use 36\% threshold (compared to 42\% under multi-constraint optimization, §2.3) for feasibility assessment with edge-weighted methods. When state-wide demographics fall below 36\%, recognize genuine geographic limits. Above 36\%, edge-weighting demonstrates VRA compliance is algorithmically achievable—non-compliance suggests alternative motives.
