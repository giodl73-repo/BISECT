...
manipulation. Because \textsc{bisect} maps cannot gerrymander---they lack the
partisan data inputs that gerrymandering requires---their partisan properties
reflect geographic sorting alone. The gap between \textsc{bisect} values and
enacted map values on any metric measures the portion of partisan skew that
is attributable to deliberate mapmaker choices rather than residential patterns.
For North Carolina, this gap is approximately four times larger than what
geography alone produces, a finding that is consistent across all six metrics
and robust to the choice of election cycle.
