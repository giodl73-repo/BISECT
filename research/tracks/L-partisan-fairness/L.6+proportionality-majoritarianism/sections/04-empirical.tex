...
\subsection{Multi-Algorithm Proportionality Comparison}

Geographic sorting is a property of where voters live, not of the algorithm.
However, the \textsc{bisect} portfolio includes algorithms that interact differently with
each state's geographic sorting, producing a wide range of proportionality outcomes.
Table~\ref{tab:algo-prop-comparison} reports confirmed run outputs for key states.

\begin{table}[ht]
\centering
\caption{Proportionality gap by algorithm: confirmed from pipeline outputs.
Gaps near zero indicate near-proportional outcomes.}
\label{tab:algo-prop-comparison}
\begin{tabular}{@{}llrr@{}}
\toprule
State ($k$) & Algorithm & D Seats & Gap (pp) \\
\midrule
\multirow{3}{*}{NC ($k=14$, $v_D=49.3\%$)}
  & Standard-bisect$^\dagger$ & 6 & $-6.5$ \\
  & ApportionRegions (B.11)$^\dagger$ & \textbf{7} & $\mathbf{+0.7}$ \\
  & ProportionalSection (B.12)$^\dagger$ & 5 & $-13.6$ \\
\midrule
\multirow{2}{*}{GA ($k=14$, $v_D=50.1\%$)}
  & Standard-bisect$^\dagger$ & 5 & $-14.4$ \\
  & ProportionalSection (B.12)$^\dagger$ & \textbf{6} & $\mathbf{-7.3}$ \\
\midrule
\multirow{2}{*}{MN ($k=8$, $v_D=53.5\%$)}
  & Standard-bisect$^\dagger$ & \textbf{4} & $\mathbf{-3.6}$ \\
  & ProportionalSection (B.12)$^\dagger$ & 3 & $-16.1$ \\
\midrule
\multirow{2}{*}{WI ($k=8$, $v_D=50.3\%$)}
  & Standard-bisect$^\dagger$ & 3 & $-12.8$ \\
  & ApportionRegions (B.11)$^\dagger$ & 2 & $-25.3$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Key findings.}
(1) No single algorithm consistently achieves proportionality across all states.
Geographic voter distribution is the dominant factor.
(2) The ApportionRegions prime-factor structure achieves near-perfect proportionality
for NC ($+0.7$\,pp) because its 7-way primary factorisation groups Charlotte and the
Research Triangle together, enabling the 7D outcome.
The same structure hurts WI ($-25.3$\,pp) because Wisconsin's 2-way primary split
creates lopsided geographic halves from 50\% Democratic vote.
(3) ProportionalSection (B.12) reduces the gap in GA ($-14.4$ to $-7.3$\,pp) but
inadvertently worsens it in MN ($-3.6$ to $-16.1$\,pp) and NC.
(4) The algorithm most appropriate for proportionality in a given state depends on
the state's geographic voter distribution, not on a universal algorithm choice.
