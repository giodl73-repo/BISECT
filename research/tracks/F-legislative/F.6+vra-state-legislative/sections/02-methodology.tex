...
VRASection is the minority opportunity district module of the algorithmic redistricting pipeline. It operates as a post-processing step on the METIS recursive bisection algorithm: after an initial partition is computed, VRASection identifies census block-group units with minority populations above a threshold and adjusts the partition to preserve those units within the same district, thereby creating majority-minority or minority opportunity districts where the demographic conditions support them.

\textbf{Note on base geographic units.} All five covered states (AL, GA, LA, MS, SC) require block-group resolution under the F.3 resolution rule ($k/n_{\mathrm{tracts}} > 0.05$ for each). VRASection therefore operates on census block groups as the base unit throughout this analysis, not census tracts. Block-group adjacency files for the five covered states are generated using:
\begin{verbatim}
bisect fetch --year 2020 --resolution block_group \
  --states AL GA LA MS SC
\end{verbatim}

The algorithm proceeds in three phases:

\textbf{Phase 1: Identification.} For each potential majority-minority district, VRASection identifies the set of candidate block-group units whose minority population share exceeds the configuration threshold (typically 42\% for the purpose of this analysis). These units form a ``seed set'' that the algorithm attempts to keep in the same district.
This 42\% threshold is derived from congressional-scale analysis in~\cite{dellaLibera2026threshold} (companion paper D.1), where it was established across $N=43$ states ($r=0.78$, $p<10^{-8}$). We apply the same threshold at state house scale under the scale-invariance assumption established in Section~3.1.

\textbf{Phase 2: Aggregation.} The algorithm expands the seed set to include adjacent block-group units that raise the district's minority population share toward a majority, subject to population balance constraints. This expansion is constrained by the geographic compactness requirement: the algorithm will not add non-adjacent units (``island'' aggregation) even if doing so would produce a higher minority share.
