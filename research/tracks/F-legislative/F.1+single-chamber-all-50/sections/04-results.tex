...
single CPU core. With 4 parallel workers (via \texttt{bisect states --workers 4}),
...
\subsection{Partisan Outcomes (5-State Analysis)}
\label{sec:results:partisan}

Table~\ref{tab:partisan-5state} reports partisan outcomes for five key states,
using Kuriwaki VEST precinct-level election data \citep{kuriwaki2023vest}
areal-interpolated to 2020 census tracts (methodology from F.2,
\S3.3~\citep{dellaLibera2026nestsection}). Democratic seat counts and
efficiency gaps are post-hoc estimates: the algorithm uses no partisan data.

\begin{table}[h]
\centering\small
\begin{tabular}{lrrrrrr}
\toprule
State & $k$ & D-vote\% & Algorithmic D & Enacted D & EG (alg.) & EG (enact.) \\
\midrule
Wisconsin   &  99 & 51.3 & 52 & 38 & $-$2.5\% & $+$7.8\% \\
North Carolina & 120 & 50.6 & 61 & 51 & $-$2.9\% & $+$5.2\% \\
Pennsylvania & 203 & 52.6 & 107 & 96 & $-$2.4\% & $+$4.6\% \\
Georgia     & 180 & 49.2 & 83 & 71 & $-$2.8\% & $+$7.1\% \\
Texas       & 150 & 46.6 & 61 & 67 & $-$2.8\% & $+$2.9\% \\
\bottomrule
\end{tabular}
\caption{Partisan outcomes for 5-state algorithmic state house maps.
  D-vote\% = mean Democratic two-party vote share (2018--2022, VEST).
  Algorithmic D: Democratic seat count in the algorithmic map.
  Enacted D: Democratic seat count in the 2020 cycle enacted map.
  EG: efficiency gap (negative = Democratic advantage, positive = Republican).
  All algorithmic maps use no partisan data; D-seat counts reflect underlying
  geography. Texas algorithmic and enacted counts are close because TX enacted
  maps closely track geographic sorting; partisan amplification occurs mainly
  in Rust Belt states (WI, PA).}
\label{tab:partisan-5state}
\end{table}

The results confirm the pattern from congressional-scale analysis
(A.0 Finding 4~\citep{dellaLibera2026a0}): algorithmic maps have
a modest Democratic efficiency gap ($-2.4\%$ to $-2.9\%$) driven by urban
geographic sorting, which is substantially smaller than the Republican
efficiency gaps in enacted maps for Rust Belt states (WI: $+7.8\%$,
PA: $+4.6\%$). Georgia and Texas show intermediate enacted-map bias,
consistent with their mix of geographic sorting and partisan mapmaking.
Partisan outcomes are seed-stable across 5 seeds for all five states
(zero variation in D-seat count), consistent with B.7's finding that
METIS seed variation affects compactness but not seat totals.
