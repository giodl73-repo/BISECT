...
\begin{itemize}
  \item \textbf{Independent}: The house map and senate map are optimised
    independently (two separate \texttt{bisect} runs with no nesting constraint).
  \item \textbf{NestSection}: The house and senate maps are jointly optimised
    via NestSection (one \texttt{bisect} run with \texttt{--chamber nest}).
\end{itemize}
