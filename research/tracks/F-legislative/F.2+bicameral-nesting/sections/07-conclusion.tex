...
\begin{enumerate}
  \item \textbf{Compatibility}: 40 of 49 bicameral states have
    $\gcd(H,S) > 1$ and are compatible with NestSection. The remaining
    9 are incompatible due to coprime chamber sizes; changing one
  ...
  \item \textbf{Population balance}: All 40 compatible states achieve
...
commissions in the 40 compatible states. The algorithm is implemented in the
\texttt{bisect} Rust binary via \texttt{bisect state --chamber nest} and
...
The next step is to apply NestSection to the 9 incompatible states using
