...
\textbf{Canonical Reock} compares district area to its minimum bounding circle (MBC):
\[
  \text{Reock} = \frac{A}{\pi r^2} \in [0, 1],
\]
where $r$ is the radius of the smallest circle containing the district.
The current \textsc{bisect} reported Reock value uses a centroid-plus-maximum
boundary-radius proxy for Python-pipeline parity; K.2 separates that production
metric from exact-MBC Reock.
...
On NC coastal districts, the reported Reock proxy decreases by $\leq 0.02$ when switching from
smoothed to raw TIGER/Line boundaries, versus PP decreasing by $0.04$--$0.08$
(K.2).
This makes area-based metrics useful for coastal states, provided the proxy is
disclosed when exact-MBC reproduction is required.
