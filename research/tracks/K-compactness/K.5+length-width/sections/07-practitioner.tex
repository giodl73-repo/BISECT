...
\end{verbatim}
The resulting \texttt{compactness.json} includes the MBR-based
\texttt{length\_width\_ratio} field for each district and
\texttt{mean\_length\_width\_ratio} in the summary.  AABB values are diagnostic
only: use \texttt{axis\_aligned\_length\_width\_ratio()} in
\texttt{bisect-analysis} for orientation-bias checks, not as the production
compactness score.
