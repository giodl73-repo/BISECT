...
\begin{itemize}
  ...
  \item \textbf{Reock}: Mean $= [Reock]$ (scale $[0,1]$; higher is more compact;
    less sensitive to coastal boundary artifacts than PP). In \textsc{bisect},
    this is the centroid-plus-maximum-boundary-radius proxy, not exact polygon-MBC
    Reock. Source: K.2.
...
\noindent The submitted plan achieves PP $= [PP]$, reported Reock $= [Reock]$,
...
(PP $\geq 0.18$, reported Reock $\geq 0.30$, CH $\geq 0.82$, S $\leq 2.4$, LW $\leq 2.8$,
...
\begin{quote}
  ...
  the underlying data or software. Reock values are the \textsc{bisect}
  centroid-radius approximation unless an exact-MBC method is separately named.}
\end{quote}
