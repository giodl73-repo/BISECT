The convex hull ratio is implemented as \texttt{convex\_hull\_ratio()} in
\texttt{crates/bisect-analysis/src/compactness.rs} and is included in the
\texttt{CompactnessMetrics} struct returned by \texttt{all\_metrics()}.
All six K-track metrics are reported together via \texttt{bisect label-analyze --types compactness}.
