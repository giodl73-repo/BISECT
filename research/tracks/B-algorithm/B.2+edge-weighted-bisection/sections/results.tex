...
We evaluate edge-weighted recursive bisection on all 50 U.S. states using 2020 Census data, generating all 435 congressional districts. We compare against two baselines: (1) \textbf{Normal mode}—unweighted recursive bisection from our companion paper~\cite{dellaLibera2026recursive}, and (2) \textbf{Enacted 2020 districts}—the actual 118th Congressional District boundaries from Census Bureau TIGER/Line shapefiles, reflecting state-drawn redistricting plans post-2020 Census.

\subsection{National Summary}

Table~\ref{tab:national-summary} presents the three-way comparison. Edge-weighted recursive bisection achieves mean Polsby-Popper score of \textbf{0.367} nationally, representing a \textbf{56\% improvement} over normal mode (0.235)\footnote{This figure compares edge-weighted bisection to unweighted bisection (B.1 baseline). Compared to enacted maps, the improvement is $+20$--$22\%$; see Table~\ref{tab:compactness-headline} in A.0.} and \textbf{20\% improvement} over enacted 2020 districts (0.305).
...
\input{tables/geographic_sorting_table}
\input{tables/geographic_sorting_summary}
...
\input{tables/partition_quality_table}
\input{tables/partition_summary_table}
...
\input{tables/partitioner_comparison_table}
...
\input{tables/county_preservation_table}
\input{tables/county_preservation_summary}
