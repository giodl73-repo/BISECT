# B-algorithm Track — Panel Review Batch 2
**Scope**: B.0 (algorithm-design-overview) · B.02 (one-federal-law) · B.8 (geosection-ratio-optimal-bisection) · B.10 (subdivision-respecting-redistricting) · B.13 (nestsection-nested-multi-chamber) · B.14 (vrasection-minority-opportunity-bisection) · B.15 (stabilitysection-cross-census-stability) · B.16 (convergence-sweep) · B.17 (parameter-sensitivity) · B.18 (multi-reapportionment-stability)
...
**Directory**: `research/tracks/B-algorithm/B.0+algorithm-design-overview/`
...
**P1 — The natural-bisection-ratio discrepancy between papers.** Table 2 shows GeoSection choosing ratio 6:8 for NC ($k=14$), while B.8 (GeoSection paper) abstract reports the same result as "6:8 east/west split." However, the B.0 bakeoff abstract says GeoSection produces "5D/9R (-13.6 pp)" for NC under Config 2, while the B.8 abstract says GeoSection produces "5D/9R (-13.6 pp)" as well. These are consistent. However, Table 2 Config 2 shows "7D/7R (+0.7 pp)" for the confirmed GeoSection NC result. This directly contradicts the abstract claim of 5D/9R for GeoSection in NC and also contradicts the B.8 abstract. There is a P1 internal inconsistency: the abstract says "geographic voter sorting drives outcomes," citing -13.6 pp, but Table 2 shows +0.7 pp for GeoSection in NC. One of these numbers is wrong. The body of Section 5.3 resolves this by noting that "GeoSection produces 7D/7R (+0.7 pp) — near-proportional" in NC, making the abstract's -13.6 pp reference incorrect. **The -13.6 pp reference in the abstract corresponds to unweighted bisection (Config 1), not GeoSection (Config 2).** The abstract must be corrected.
...
**Directory**: `research/tracks/B-algorithm/B.02+one-federal-law/`
...
**P2 — 50-state zero-variance claim relies on pending B.11 data.** Multiple arguments in Section 3 forward-reference B.11 as the empirical foundation. If B.11 is not yet complete, the advocacy paper must be hedged accordingly.
...
**P1 — The "12.5 percentage-point difference is entirely an artifact of which algorithm was chosen" claim.** This strong causal claim is correct given the assumption that both algorithms have zero partisan input. But "entirely an artifact" implies no other factor (seed, compactness measure, population tolerance) contributes. The B.17 paper shows parameters contribute at most 0.3 seats nationally, which supports the "algorithm-dominated" conclusion. However, the advocacy paper should cite B.17 and use "primarily" instead of "entirely" to remain defensible.
...
**P1 — "Zero seed variance" for ApportionRegions is stated as established but B.11 is explicitly incomplete.** As noted by R1, this is an unvalidated claim presented as fact. The reproducibility standard requires either (a) reporting the actual variance from completed runs, or (b) explicitly labeling the claim as preliminary pending B.11.

**P1 — The ConvergenceSweep threshold T=600 is cited as the "provably convergent" stopping criterion.** Section 3.3 states: "ConvergenceSweep (B.16) certifies the algorithm with a stopping criterion that ensures the globally optimal map is found with probability >99.9%." But B.16 derives T=600 from Georgia as the worst-case state, with a Gumbel extreme-value model. For a law review advocacy piece, citing a statistical tail bound from B.16 as a "provably convergent" guarantee conflates a probabilistic argument with a deterministic one. The language must be adjusted: "with probability exceeding 99.9% under the fitted Gumbel model" rather than "provably convergent."
...
| B02-P1-D | P1 | "Zero seed variance" for ApportionRegions: cite as "preliminary (B.11 pending)" not established fact | R1, R5 |
...
| B02-P1-G | P1 | "Provably convergent" ConvergenceSweep: change to "convergent with probability >99.9% under the fitted Gumbel model (B.16)" | R5 |
...
| B02-P2-B | P2 | "Entirely an artifact of which algorithm was chosen": cite B.17 and change to "primarily" | R2 |
...
**Directory**: `research/tracks/B-algorithm/B.8+geosection-ratio-optimal-bisection/`
...
**P2 — The comparison to "standard minimum-edge-cut bisection (B.7 baseline)" uses B.7 results without specifying the B.7 seed count.** B.7 uses 10,000 seeds per state; B.8 uses 50 seeds per ratio. The comparison is between a 10,000-seed B.7 result and a 50-seeds-per-ratio B.8 result. If B.7 achieves a more optimal partition due to more seeds, the comparison may understate GeoSection's improvement. The seed-count asymmetry should be acknowledged.
...
| R5 Liang | 3/4 | 50-seed protocol not specified; B.7 vs. B.8 seed-count asymmetry unacknowledged |
| **Average** | **3.0/4** | |
...
**Top P1 issue: R1 — B.8 abstract reports NC GeoSection as "5D/9R (-13.6 pp)" but B.0 confirmed bakeoff Table 2 reports it as "7D/7R (+0.7 pp)"; this is a direct inter-paper numerical contradiction that must be resolved**

---

### B.8 Open Items Table
...
| B8-P2-D | P2 | B.7 vs. B.8 seed-count asymmetry (10,000 vs. 50): acknowledge in comparison section | R5 |

---

---

# B.10 — Subdivision-Respecting Redistricting: County-Sticky Edge Weights

**Directory**: `research/tracks/B-algorithm/B.10+subdivision-respecting-redistricting/`
...
# B.13 — NestSection: Consistent Multi-Chamber Redistricting via Compatible Factorization Spines

**Directory**: `research/tracks/B-algorithm/B.13+nestsection-nested-multi-chamber/`
**Venue target**: Political Analysis or JACM
**Status going in**: No prior panel review
...
# B.14 — VRASection: Geographic Alignment Score for Minority Opportunity District Bisection

**Directory**: `research/tracks/B-algorithm/B.14+vrasection-minority-opportunity-bisection/`
...
**P1 — $w_{\rm vra} = 0.40$ sensitivity analysis is described as "pending."** Section 4.3 Remark states: "sensitivity analysis (B.14 pending) suggests that $w_{\rm vra} = 0.20$ often leaves alignment underutilised, while $w_{\rm vra} = 0.60$ risks over-prioritising." This self-reference ("B.14 pending") means the sensitivity analysis cited in B.14 does not yet exist in B.14. The sensitivity analysis data must be reported in the body, or the Remark must be removed.
...
| R5 Liang | 3/4 | Self-referential "B.14 pending" sensitivity analysis; 6-state status inconsistency |
| **Average** | **3.0/4** | |
...
| B14-P1-C | P1 | Self-reference "B.14 pending" for sensitivity analysis: include the sensitivity analysis in the body or remove the claim | R5 |
...
# B.15 — StabilitySection: Cross-Census Stability of GeoSection-Optimal Redistricting Maps

**Directory**: `research/tracks/B-algorithm/B.15+stabilitysection-cross-census-stability/`
...
# B.16 — ConvergenceSweep: T=600 as the Statutory Stopping Criterion

**Directory**: `research/tracks/B-algorithm/B.16+convergence-sweep/`
...
# B.17 — How Sensitive Are Redistricting Outcomes to Algorithm Parameters? A 50-State Sweep

**Directory**: `research/tracks/B-algorithm/B.17+parameter-sensitivity/`
**Venue target**: Political Analysis
**Status going in**: No prior panel review
...
**P1 — T_stat = 600 vs. T = 400 as convergence threshold.** Table 3 shows $T = 100$ produces a slightly higher $D_{\rm nat}$ (0.1 seats) because it misses Georgia's global minimum. But $T \geq 200$ produces identical outcomes in all 50 states. The paper's conclusion recommends $T_{\rm stat} = 600$ as the statutory default based on B.16, but Table 3 shows $T = 200$ is already sufficient for identical outcomes. The paper should note this: $T = 200$ is empirically sufficient, $T = 600$ provides the safety margin justified by the Gumbel tail bound (B.16). The distinction is important for understanding the statutory conservatism.
...
**P1 — The VRA boost weight $w_{\rm vra}$ sweep ($0.2$ to $0.6$) is conducted on all 50 states but VRASection is only applicable in VRA states.** Table 5 shows $w_{\rm vra}$ variation of ±0.1 seats nationally. But $w_{\rm vra}$ affects only VRA states (those with sufficient minority VAP for an alignment signal). Sweeping $w_{\rm vra}$ across all 50 states dilutes the effect — in the 40+ non-VRA states, changing $w_{\rm vra}$ has no effect because $A \approx 0$ for all ratios. The relevant sensitivity is in the 6 VRA states from B.14. The table should report $w_{\rm vra}$ sensitivity separately for VRA states vs. the national aggregate.

**P2 — $\alpha_{\rm county}$ split reduction in Table 2 disagrees with B.10.** Table 2 shows county splits of 312 at $\alpha_c = 1.0$ and 271 at $\alpha_c = 3.0$. But B.10 Table 1 reports 487 at $\alpha_c = 1.0$ and 323 at $\alpha_c = 3.0$. The B.17 figures are substantially lower than B.10's. The discrepancy (487 vs. 312 at baseline) must be explained: is B.17 using a different definition of "county split"? A different set of states? This is a cross-paper numerical inconsistency that must be resolved.**
...
| R3 Duchin | 3/4 | $w_{\rm vra}$ sensitivity diluted by non-VRA states; county splits B.17 vs. B.10 discrepancy |
...
**P1 count: 4** (0.3 empirical vs. 0.5 theoretical not distinguished; $w_{\rm vra}$ sensitivity diluted by non-VRA states; county splits B.17 (312) vs. B.10 (487) baseline discrepancy; seed protocol unspecified)
**Top P1 issue: R3 — B.17 Table 2 shows 312 county splits at $\alpha_c = 1.0$ vs. B.10 Table 1 showing 487 county splits at the same parameter; this is a direct numerical contradiction across papers that must be resolved before any submission**

---

### B.17 Open Items Table

| ID | Severity | Description | Reviewer |
|----|----------|-------------|----------|
| B17-P1-A | P1 | Abstract "≤0.3 nationally": specify this is empirical joint-sweep bound; theoretical additive bound is 0.5 | R1 |
| B17-P1-B | P1 | $w_{\rm vra}$ sensitivity: report separately for VRA states (B.14 set: AL, NC, SC, GA, MS, LA) vs. national aggregate | R3 |
| B17-P1-C | P1 | County splits discrepancy: B.17 Table 2 baseline 312 vs. B.10 Table 1 baseline 487; resolve and explain difference | R3 |
...
| B17-P2-B | P2 | T=200 vs. T=600 distinction: note that T=200 is empirically sufficient; T=600 provides the Gumbel safety margin from B.16 | R2 |
...
# B.18 — Redistricting Stability Across Reapportionment Cycles

**Directory**: `research/tracks/B-algorithm/B.18+multi-reapportionment-stability/`
...
## B-algorithm Batch 2 Summary Table
...
| B.8 GeoSection | 3.0/4 | Accept | 6 | B.8 abstract reports NC GeoSection as "5D/9R (-13.6 pp)"; B.0 confirmed bakeoff Table 2 shows "7D/7R (+0.7 pp)"; inter-paper numerical contradiction (R1) |
| B.10 County-Sticky | 3.4/4 | Accept | 4 | Pareto-frontier analysis is single-seed per state; undisclosed in abstract (R2/R5) |
| B.13 NestSection | 3.2/4 | Accept | 5 | Paper proposes NestSection but has not run it; Oregon and Alabama figures are schematic placeholders "[to be generated by pipeline]" (R5) |
| B.14 VRASection | 3.0/4 | Accept | 6 | Abstract says 6-state comparison "pending"; Discussion presents it as a completed 3/6 finding; internal consistency error (R2/R5) |
| B.15 StabilitySection | 3.0/4 | Accept | 6 | "[TBD: 2010 sweep pending]" appears verbatim in abstract; publication-blocking placeholder (R1/R4/R5) |
| B.16 ConvergenceSweep | 3.4/4 | Accept | 5 | Abstract claims "globally optimal map deterministically"; METIS is a heuristic; global optimality is incorrect (R2) |
| B.17 Parameter Sensitivity | 3.6/4 | Accept | 4 | B.17 Table 2 baseline county splits (312) contradicts B.10 Table 1 (487) for same parameter; inter-paper numerical contradiction (R3) |
| B.18 Reapportionment Stability | 3.0/4 | Accept | 7 | Abstract "less than single MCMC step" directly contradicts Table 4 "≈10 ReCom steps"; (R2) |

---

## Cross-Cutting Findings (Batch 2)

### Finding 1 — NC GeoSection partisan result: -13.6 pp vs. +0.7 pp (affects B.0, B.8)
B.8 abstract reports GeoSection produces "5D/9R (-13.6 pp)" for North Carolina. B.0 bakeoff Table 2 (confirmed, no dagger) shows GeoSection Config 2 for NC produces "7D/7R (+0.7 pp)." These are the same algorithm on the same state — one is wrong. All papers citing NC GeoSection partisan results must use the confirmed B.0 Table 2 value. Likely resolution: the -13.6 pp figure in B.8 refers to the *B.8 draft's* run result, while B.0's confirmed value reflects a later, convergence-sweep-optimized run.

### Finding 2 — County splits baseline discrepancy: 312 vs. 487 (affects B.10, B.17)
B.10 Table 1 reports 487 county splits at $\alpha_c = 1.0$ (baseline). B.17 Table 2 reports 312 county splits at $\alpha_c = 1.0$. Both claim to be national 50-state results. This is a direct inter-paper numerical contradiction. Likely cause: different definitions of "county split" (e.g., B.10 counts split county-district pairs; B.17 may count split counties only).

### Finding 3 — DIA not consistently labeled as proposed statute (affects B.0, B.02, B.10, B.16, B.17, B.18)
...
### Finding 4 — *Callais* citation inconsistency (affects B.0, B.02, B.14)
Three papers cite the Callais case differently: B.0 and B.02 use "Louisiana v. Callais (2026)" with specific page citations; B.14 uses "Callais v. Landry (2025)." These appear to be the same case at different stages. All papers must use a single consistent citation; if the 2026 Supreme Court opinion exists, use that; if the page-36 citations are fabricated, they must be removed.

### Finding 5 — "[TBD]" and placeholder text in abstracts (affects B.15)
B.15 contains a verbatim "[TBD: 2010 sweep pending]" marker in the abstract. This is the most severe publication-blocking error in Batch 2.

### Finding 6 — All papers accepted except B.02 (Minor Revision)
Nine of ten papers score ≥3.0/4 and receive Accept verdicts. B.02 scores 2.8/4 (Minor Revision) due to R4 Stephanopoulos's score of 2/4, driven by the combination of DIA mischaracterized as enacted law, *Callais* page citations for a potentially unverified case, and *Harkenrider* efficiency-gap mischaracterization. B.17 (3.6/4) and B.16 (3.4/4) are the strongest papers in the batch.

---

*Panel convened 2026-05-07. Reviewer identities are simulated personas for pre-submission quality control. B.0 and B.02 are the capstone and advocacy papers of the B-algorithm track; all ten papers reviewed with eight accepted and two requiring revision.*
