...
satisfies the same core properties (symmetry, graceful degradation to $w_{\mathrm{base}}$ when no zones are shared) but uses an \emph{additive boost} variant rather than M.0's multiplicative attenuation (see \S\ref{sec:formula} Remark for full reconciliation).
M.6's minimum weight is $w_{\mathrm{base}}$ (zone-membership never reduces an edge below baseline),
whereas M.0's multiplicative formula can reduce edges toward zero for orthogonal character vectors.
