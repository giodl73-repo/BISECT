...
Its census tracts have \texttt{jobs\_per\_resident} values well above 5.0,
reflecting the thousands of pharmaceutical, biotechnology, and information
...
Under M.1 economic character weights, the JPR contrast ($\mathrm{JPR}_{\mathrm{RTP}} \approx 7$
vs.\ $\mathrm{JPR}_{\mathrm{suburb}} \approx 0.3$) produces very low cosine
similarity between these adjacent tracts, making the cut cheaper.
...
\begin{tabular}{llr}
  ...
  County (B.10) & \texttt{--weights-override county} & varies \\
  Economic character (M.1/B.27) & \texttt{--weights-override economic-character} & $-$6.4 \\
  \bottomrule
\end{tabular}
\end{table}
...
Note that the AR algorithm (B.11, ApportionRegions) and convergence search
(B.16) can produce further improvements; M.1 is a Layer~2 modification and
