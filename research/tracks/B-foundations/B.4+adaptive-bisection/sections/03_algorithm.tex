...
function AdaptiveBisection(G, k, minority_data):
    ...
    return districts_L union  districts_R
...
function MetisBisect(G, (k_L, k_R), minority_data):
    ...
    # Edges between high-minority tracts have weight $alpha$ > 1
...
function MaxMinorityInSubgraph(G, minority_data):
    ...
    total_minority = sum_{v in G} m_v * w_{pop}(v)
    total_pop = sum_{v in G} w_{pop}(v)
