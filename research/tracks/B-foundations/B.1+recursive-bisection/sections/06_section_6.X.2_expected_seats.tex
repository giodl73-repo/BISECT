...
Chen and Rodden \citep{chen2013unintentional} developed a predictive model for partisan bias based on political geography. Their model motivates the expected relationship between vote share and seat share given the spatial distribution of voters:
...
An alternative approach to evaluating redistricting is the \textit{seats-votes curve}, which plots the relationship between vote share and seat share across hypothetical election scenarios \citep{tufte1973relationship}. One evaluative framework asks whether that curve:
\begin{enumerate}
...
\textbf{MCMC ensemble methods.} ReCom can sample many plans under a declared constraint profile \citep{deford2019}. Comparing the deterministic output with that distribution can help separate geographic effects from an unusual algorithmic choice, but it does not by itself establish causation and is pending regeneration for this paper.
