...
These three criteria—population equality, contiguity, and compactness—constitute the traditional core of redistricting law, established through Supreme Court precedent (\textit{Wesberry v. Sanders}, \textit{Karcher v. Daggett}) and state constitutions \cite{wesberry1964,karcher1983}.
...
Graph partitioning—dividing a graph into balanced, well-connected components—is NP-hard \cite{garey1979computers}. The nationwide implementation therefore uses heuristic methods that find high-quality solutions efficiently. Bounded exact per-cut and recursive-tree certificates are now implemented separately; full State-scale proof generation remains an open solver and proof-engineering problem rather than a demonstrated impossibility.
