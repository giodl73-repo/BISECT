...
We present a recursive graph bisection method whose frozen geographic profile
operates on census population and adjacency rather than election or party data.
The historical 2020 tract experiment produced 435 district assignments under
its named implementation and tolerance. Those results are profile-specific and
predate the current block-level NRS and certified ceremony. They report that the
historical outputs:

\begin{itemize}
    \item had mean population deviation 2.79\%, with 86\% within $\pm$5\%;
    \item passed the experiment's recorded graph-contiguity checks;
    \item used edge-cut minimization as a graph-native boundary objective; and
    \item excluded partisan inputs from the frozen execution profile.
\end{itemize}
...
\subsection{The Input-Exclusion Defense}
...
Algorithmic redistricting offers a narrower evidentiary defense: a verified run
cannot directly target partisan outcomes using data excluded from its inputs.
That is a procedural constraint, not a guarantee of partisan neutrality or an
inference about upstream intent. Rule, weight, resolution, seed, and candidate-
selection choices can encode discretion even when the final executable receives
no election table.

Input exclusion is useful only when paired with precommitment and verification.
The certified model removes per-cut solver discretion under a frozen objective;
the public ceremony records when that rule and each resulting depth were
released. Partisan patterns can reflect political geography, but the evidence
does not exclude strategic selection among objectives or profiles before
genesis.
...
The process can be transparent, reproducible, and resistant to undisclosed
substitution when its inputs, rule, certificates, and ceremony transcript all
verify. Whether those procedural virtues command public or legal legitimacy is
an empirical and institutional question, not a software theorem.

Just as Huntington--Hill implements one publicly selected apportionment rule,
certified bisection implements one publicly selected districting rule. The
relevant distinction is auditable execution under precommitment, not proof that
partisan advantage is impossible.
...
The question is not whether algorithmic redistricting is perfect, but which
claims each implementation can support relative to alternatives. The current
evidence supports reproducibility, input custody, and a model-scoped path to
proof; it does not yet support a generally best map or universal legitimacy.
...
Certified and publicly staged redistricting offers different guarantees.
Independent observers can replay the calculation, verify certified cuts, and
detect substitution in the ceremony transcript. Those guarantees concern the
execution of explicit mathematical principles; the principles themselves
remain products of political and legal governance.
...
Huntington--Hill shows that a durable public rule can separate institutional
choice from mechanical execution. Certified recursive bisection and its staged
ceremony make an analogous separation testable for boundary design. Whether a
jurisdiction should adopt that rule, modify it for controlling law, or prefer a
different transparent benchmark remains a democratic decision.
