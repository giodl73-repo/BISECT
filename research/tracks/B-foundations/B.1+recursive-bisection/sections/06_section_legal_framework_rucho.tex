...
We instead emphasize the \textit{impossibility defense}: the algorithm cannot be instructed to gerrymander because it cannot access partisan data. This is a \textit{process} defense focused on inputs rather than outputs. Courts need not determine whether results are "fair"—they verify the process excluded partisan information. That is a procedural safeguard, not a claim that any resulting map is partisan-neutral in every geography.
