...
\label{sec:geographic-sorting-mechanism}
...
Chen and Rodden \citep{chen2013unintentional} formalize this intuition with a geographic-sorting measure:
\begin{equation}
...
Geographic sorting has increased over recent decades. Rodden \citep{rodden2019why} describes the growing alignment between partisan voting and population density. Candidate mechanisms include:

\begin{enumerate}
\item \textbf{Economic geography}: Knowledge-economy jobs concentrate in urban centers alongside college-educated workers.
\item \textbf{Cultural geography}: Urban and rural locations offer different amenities and social environments.
\item \textbf{Political geography}: Residential clustering can reinforce geographic partisan concentration.
\item \textbf{Migration patterns}: Age-selective migration can change the urban--rural electorate over time.
\end{enumerate}
...
To quantify geographic sorting across all 50 states, we computed the correlation between tract-level Democratic vote share (2020 presidential election) and population density for each state. The numerical summary follows; its supporting table remains to be regenerated for the public artifact package.
...
This is the classic efficiency-gap logic \citep{stephanopoulos2015partisan}, but it may arise from geography as well as intentional line drawing. The Wisconsin case below examines that distinction; causal attribution cannot be read from the efficiency gap alone.
...
Simulation studies show that political geography can constrain the partisan range of neutral plans \citep{chen2017impact}. The magnitude and direction remain State- and model-specific; such simulations do not prove a universal upper bound.
...
\textbf{Comparison to ensemble methods.} ReCom samples can characterize a reference distribution of population-balanced plans under specified constraints \citep{deford2019}. Whether this deterministic output is typical of such a distribution is an empirical bakeoff question; the relevant ensemble comparison has not yet been regenerated here.

\textbf{Comparison to commission-drawn maps.} Independent redistricting commissions in states like California, Michigan, and Colorado explicitly optimize for compactness and can produce maps with higher Polsby-Popper scores than the historical baseline reported in Section~\ref{sec:results}. Outcome comparisons require a common regenerated benchmark; compactness alone does not establish partisan neutrality.
