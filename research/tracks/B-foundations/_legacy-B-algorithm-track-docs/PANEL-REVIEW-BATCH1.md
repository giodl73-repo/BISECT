# B-foundations Track — Panel Review Batch 1
...
**Directory**: `research/tracks/B-foundations/B.1+recursive-bisection/`
**Venue target**: APSR
...
**P1 — Compactness baseline comparison error.** Table 3 in §4.3 shows "Algorithmic: 0.235 PP, Enacted 2020: 0.305 PP, Difference: -22.8%." The paper then notes that enacted districts have *higher* average compactness than algorithmic districts. This is correct but the "Note on baseline comparisons" paragraph (appended to the table) states that B.2 reports "+56% figure using the unweighted bisection as its baseline (not enacted maps)." This clarification is essential and correct — but the cross-reference to "Table~\ref{tab:compactness-headline} in A.0 Supplement" refers to a document that does not exist in the current repository. There is no A.0 Supplement in the B-foundations track. This is a broken reference that will cause confusion for reviewers and should be resolved before APSR submission.
...
**Directory**: `research/tracks/B-foundations/B.2+edge-weighted-bisection/`
**Venue target**: APSR
...
**P1 — "--weights geographic" vs. "--weights-override geographic".** The CLI description in §3.5 states edge weighting is enabled via `--weights geographic` but the current CLI uses `--weights-override geographic` (confirmed from U.1 and other papers in the track). This is a P2 flag error that must be fixed before APSR submission, as APSR reviewers using the CLI will not reproduce the results with the wrong flag.
...
**Directory**: `research/tracks/B-foundations/B.3+multi-vs-edge/`
**Venue target**: APSR
...
**Directory**: `research/tracks/B-foundations/B.4+adaptive-bisection/`
**Venue target**: APSR
...
**Directory**: `research/tracks/B-foundations/B.5+nway-vs-recursive-general/`
**Venue target**: APSR
...
**P2 — Prime-factor treatment.** T.4 (ApportionRegions) establishes that prime-factor bisection trees are the correct treatment for non-power-of-two $k$. B.5's discussion of prime-$k$ should cite T.4 and clarify whether the "post-processing" mentioned in the abstract is consistent with T.4's prime-factor approach or a distinct method.
...
| R3 Duchin | 3/4 | PP-specific result; T.4 prime-factor cross-reference missing |
...
| B5-P2-D | P2 | Cite T.4 prime-factor tree when discussing prime-k chambers | R3 |
...
**Directory**: `research/tracks/B-foundations/B.6+computational-complexity/`
...
**Directory**: `research/tracks/B-foundations/B.7+solution-space-and-seed-sensitivity/`
...
| R3 Duchin | 3/4 | Seat-share variance ≠ plan-space variance; U.1 T=600 derivation unclear |
...
| B7-P2-B | P2 | T=600 ConvergenceSweep motivation: make the connection from CV<2% to T=600 explicit or forward-reference U.1 | R3 |
...
## B-foundations Batch 1 Summary Table
...
*Panel convened 2026-05-07. Reviewer identities are simulated personas for pre-submission quality control. B.1–B.7 are the foundational papers of the B-foundations track; all seven accepted with revisions required.*
