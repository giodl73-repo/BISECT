...
**Track**: B-foundations
**Module**: track-B
**Papers reviewed**: 25 (B.0, B.02, B.1–T.12, T.11)
...
Track B is the methodological spine of the entire program. It progresses in four logical phases: foundations (B.1–B.7) establish that the base algorithm works and characterize its solution space; structure variants (B.0, B.02, T.1–T.8) show that the bisection tree can be shaped to serve any redistricting objective; search strategies (U.1–U.2) determine how to select among valid plans; and extensions (T.9–T.12) introduce algorithmic techniques from ML and MCMC literature.

**Review status at module assessment**:
- 14 papers have `_panel.yaml` with documented review history
- 11 papers (B.5, B.6, B.7, T.3, U.2, T.9, U.3, U.4, U.5, T.10, T.11, T.12) have no panel review history — either draft or accepted via external review only
- Extensions track (U.3–T.12) are accepted at 3.5–3.9/4 per PAPERS.md
...
## Sub-Track 2: Structure Variants (B.0, B.02, T.1–T.8)

**Overall assessment**: The structure sub-track is the module's most diverse, covering ten distinct bisection tree architectures. The most reviewed papers (T.1, T.2, T.4, T.5, T.6, T.7, T.8) all have panel history. B.0 (bakeoff) is uniquely accepted at 4.0/4 — the module's highest foundation score. T.3 (county-sticky) lacks panel review history.
...
**Cross-track note**: U.8 depends on B.0 for its structural-bias challenge-route argument. The cross-reference from U.8 to B.0 must be present (confirmed by U.8 reviewers).
...
### T.1 — GeoSection: Ratio-Optimal First Bisection
...
### T.2 — AreaSection: Dual [population, area] Constraint
**Score**: 3.3/4 (recheck stage)
**Venue**: APSR
...
### T.3 — County-Sticky Weights: 34% Fewer County Splits at 3% Compactness Cost
**Score**: Unscored (no _panel.yaml; draft)
**Venue**: APSR
**Status**: Draft; needs panel review

34% fewer county splits at 3% compactness cost. This is a politically significant finding — county integrity is a legal requirement in many states. No panel review history despite the paper being listed as accepted in PAPERS.md. Clarify: is T.3 accepted at a venue or is the PAPERS.md entry an error?

### T.4 — ApportionRegions: Prime-Factor Bisection Tree, NC 7D/7R
...
**Cross-track dependency**: U.9 (BisectionEnsemble) cites T.4 and explicitly inherits the prime-factor seat-allocation logic. U.9's reviewers confirmed the citation is present and the inheritance relationship is correctly scoped (BisectionEnsemble covers 2-way nodes; p-way nodes with p>2 continue to use METIS).

**Generalizability concern** (P3/Chen, P4/Duchin): The NC 7D/7R result may be NC-specific — NC's geography happens to produce proportional outcomes under the prime-factor tree, but this mechanism doesn't apply nationally. The 223D/209R national result is more aggregate and stable. A.0's synthesis must not overstate T.4 as proving "proportional outcomes in every state" when the mechanism is specific to the prime-factor tree and state-specific geography.

### T.5 — ProportionalSection: Proportionality Paradox and Rodden Gap
...
### T.6 — NestSection: Senate = 2 × House Spine Compatibility
**Score**: 3.5/4 (ready stage)
**Venue**: APSR
**Status**: Ready

Senate = 2× house spine compatibility. F.2 builds directly on T.6 for the state legislative bicameral application. The compatibility methodology is sound.

### T.7 — VRASection: Minority Geographic Alignment, Post-Callais
**Score**: 3.5/4 (ready stage)
**Venue**: Yale LJ
**Status**: Ready
...
### T.8 — StabilitySection: Bisection Tree Stability 2000–2020
...
## Sub-Track 3: Search (U.1–U.2)

**Overall assessment**: The search sub-track is the module's weakest. U.1 is in draft stage with no review history despite being the statutory foundation for the T=600 ConvergenceSweep formula used throughout the program. U.2 (parameter sensitivity) is likewise unreviewed. These are PP2 priority items.

### U.1 — ConvergenceSweep: T=600 Statutory Seed Formula
**Score**: Unscored (draft; _panel.yaml confirms draft stage)
**Venue**: APSR
...
### U.2 — Parameter Sensitivity: Partisanship Insensitive to Tuning
**Score**: Unscored (no _panel.yaml; draft)
**Venue**: Political Analysis
**Status**: Draft; must be reviewed
...
## Sub-Track 4: Extensions (T.9–T.12, T.11)

**Overall assessment**: The extensions sub-track is the module's most algorithmically sophisticated. Five of seven papers are accepted (U.3–T.12 at 3.5–3.9/4). T.9 (multi-reapportionment stability) and T.11 (geographic CVD) lack panel review history.

### T.9 — Multi-Reapportionment Stability
**Score**: Unscored (no _panel.yaml; draft)
**Status**: Draft
...
### U.3 — Simulated Annealing Bisection
...
### T.11 — CVD Geographic Extension
**Score**: Unscored (no _panel.yaml; draft; very recent as of 2026-05-07)
**Venue**: KDD
**Status**: Draft — potential redundancy with T.10

**PP1 ITEM — T.11 vs T.10 redundancy**: T.10's abstract explicitly previews Phase 2 Geographic CVD. T.11 delivers that Phase 2 with 2–6% PP improvements over the basic CVD approach. The papers are substantially redundant in terms of methodology — T.11 extends T.10's approach to geographic constraints.

**Assessment needed**: (a) Is T.11 intended to be a standalone paper or an extended version of T.10? (b) If standalone, how does the contribution differ from what T.10 already claims? (c) If the KDD paper is T.10 and T.11 is a separate followup, the naming convention (22b implies it's a variant of 22) is confusing.

**Recommended action**: Either (a) merge T.11 content into a revised T.10 extended version, clearly marking the Phase 2 geographic content; or (b) rename T.11 with a distinct code (U.6) and clearly differentiate the contribution from T.10. Module reviewers should not need to parse "b" suffixes.

### T.12 — BFS Region-Growing: Greedy Geographic District Packing
**Score**: 3.6/4 (accepted)
**Venue**: AAAI
...
### B → U.8 (structural bias argument): CLEAR
B.0 and U.2 provide the evidence base for U.8's structural-bias challenge-route argument. U.8 R3 must add cross-references to B.0 and U.2.

### T.4 → U.9 (BisectionEnsemble citation): CLEAR
U.9 explicitly cites T.4 in §2, describes the prime-factor-guided k-way splits, and correctly scopes BisectionEnsemble's coverage (2-way nodes; p>2 uses METIS). No PP1 reinvention flag.

### T.7 → D.0–D.3 (VRA methodology alignment): SHOULD VERIFY
T.7 (VRASection) and D.0 (VRA compliance via edge-weighted partitioning) address the same legal framework from different angles. The 42% threshold in D.1 and the VRASection implementation in T.7 must use consistent BVAP definitions. Panel recommends cross-referencing both in each paper's related-work section.

### U.1 → G.0 (ConvergenceSweep as baseline): NEEDS REVIEW
G.0 (ensemble methodology) cites U.1's T=600 formula as the deterministic alternative to ensemble simulation. If U.1's derivation has not been panel-reviewed, G.0's citation rests on an unvalidated result.
...
| Structure (B.0–T.8) | 10 of 11 with _panel.yaml | 3.3/4 | Moderate; T.3 gap |
| Search (U.1–U.2) | 1 of 2 with _panel.yaml | Unscored | Weak; both need review |
| Extensions (T.9–T.12, T.11) | 0 of 7 with _panel.yaml | 3.5–3.9 (external) | Accepted externally; T.9/T.11 unreviewed |
...
**PP1-B22b — T.11 redundancy decision**
T.11 appears to be the "Phase 2" work previewed in T.10. Either merge into T.10 extended version or rename T.11 with a distinct code and clearly differentiate the contribution. Do not publish both as separate papers with the current naming without clearer differentiation.

### PP2 — Important

**PP2-B16 — U.1 ConvergenceSweep panel review required**
The T=600 statutory formula is the program's foundational seed-selection standard. It is cited by B.0, G.0, G.14, and U.8. A full panel review with statistical methodology assessment is required before any downstream papers treating T=600 as validated can be submitted.

**PP2-B17 — U.2 parameter sensitivity panel review required**
The null finding requires statistical power analysis. Full panel review needed.

**PP2-B15 — T.8 Proposition 1 labeling**
T.8 Proposition 1 (stability guaranteed under mild conditions) is an empirical observation, not a mathematical proof. Relabel as "Conjecture" or provide proof.
...
**PP3-Extensions — T.9 panel review**
