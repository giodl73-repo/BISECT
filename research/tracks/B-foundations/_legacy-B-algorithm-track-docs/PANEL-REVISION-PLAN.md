...
1. **Administrative fixes** (immediate, low effort): MODULE.md description error for B.6, T.11 naming/scope decision
2. **Missing panel reviews** (3–6 weeks each): B.5, B.6, U.1, U.2 have no panel review history; T.9, B.7 are also unreviewed
3. **Paper-level revisions** (varies): T.1 R1 blocking item, T.8 Proposition relabeling, T.5 positioning

The foundations and extensions sub-tracks are strong. The search sub-track (U.1, U.2) is the module's weakest link: both papers are foundational for downstream tracks (G.0, G.14, U.8 all depend on U.1's T=600 formula) but neither has been panel-reviewed.
...
**File**: `research/tracks/B-foundations/MODULE.md`
...
### Action B-IMMED-2: Resolve T.11 vs T.10 naming and scope

**Files**: `research/T.11+cvd-geographic/` directory
**Priority**: PP1

T.10's abstract previews a "Phase 2 Geographic CVD" extension. T.11 appears to deliver that extension with 2–6% PP improvements over the basic CVD approach.

Choose one of:

**Option A (Merge)**: Incorporate T.11's geographic CVD results as a new section or appendix in T.10. Retitle T.10 as "Centroidal Voronoi Districts: Geometric and Geographic Extensions." Remove T.11 as a standalone paper.

**Option B (Rename and differentiate)**: Rename T.11 as U.6 with a title that clearly distinguishes it from T.10. In U.6's introduction, explicitly state the contribution that T.10 does not contain. In T.10's Section 7 or conclusion, note that geographic extension is covered in U.6.

**Option C (Status quo clarification)**: If T.11 is genuinely distinct (different algorithm, not just Phase 2 of T.10), keep the current naming but add a clear "Relationship to T.10" section in T.11's introduction.
...
### U.1 — ConvergenceSweep (Highest Priority)

**Why critical**: T=600 is the program's statutory standard cited by B.0, G.0, G.14, and U.8. If the T=600 derivation fails review, all downstream citations need revision.
...
### U.2 — Parameter Sensitivity (High Priority)
...
### T.1 — GeoSection: R1 Blocking Item
...
### T.8 — StabilitySection: Proposition Relabeling
...
### T.5 — ProportionalSection: Positioning
...
### T.4 — ApportionRegions: Generalizability Qualification
...
| T.4 | U.9 (cites prime-factor tree), A.0 (NC 7D/7R headline) |
| T.7 | D.0–D.3 (VRA methodology), F.6 (state legislative VRA) |
| U.1 | G.0, G.14, U.8 (T=600 formula citation) |
...
4. **T.2** → APSR (recheck; near ready)
5. **T.6, T.7, T.8** → APSR/Yale LJ (ready; T.8 Proposition rename required first)
6. **U.3–T.12** → NeurIPS/ICML/KDD/AAAI (accepted externally; submit now after T.11 decision)
7. **T.1** → APSR (after R1 blocking item resolved)
8. **B.3, T.4, T.5** → APSR (after recheck revisions)
9. **U.1, U.2** → After panel review completion
10. **B.5, B.6** → After panel review completion
11. **B.7, T.9** → After panel review completion
12. **T.11 / U.6** → After scope decision (B-IMMED-2)
...
| Week | Action |
|------|--------|
| 1 | B-IMMED-1: Correct MODULE.md B.6 description; B-IMMED-2: Decide T.11 scope |
| 1–2 | T.8: Proposition → Conjecture rename; T.5 positioning paragraph; T.1 blocking item |
| 2–4 | Initiate U.1 panel review (highest priority) |
| 3–5 | Initiate U.2, B.5 panel reviews (in parallel with U.1) |
| 5–8 | Initiate B.6 panel review (after MODULE.md correction) |
| 6–10 | Submit B.1, B.2, B.4, T.2, T.6, T.7, T.8 to target venues |
| 8–12 | Complete U.1/U.2 panel review cycles; submit or revise based on scores |
| 12+ | Submit B.5, B.6, T.1, T.9 based on review outcomes |
