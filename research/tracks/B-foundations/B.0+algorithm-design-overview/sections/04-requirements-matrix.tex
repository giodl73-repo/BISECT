...
R2: Contiguity & Art.~I const.\ floor; all state constitutions & All modes: \texttt{-contig} flag & METIS \texttt{METIS\_PartGraphRecursive} & B.1, T.1 \\[0.5em]

R3: Compactness & Most state constitutions; \shaw{} ``bizarre shape'' & Geographic edge weights (B.2) + GeoSection isoperimetric norm (T.1) & Edge weight $= $ TIGER boundary length; $\mathrm{EC}_\text{norm} = \mathrm{EC}/\sqrt{\min(i,k-i)}$ & B.2, T.1 \\[0.5em]

R4: County/subdivision preservation & \rucho{}; Harper v.\ Hall; most state redistricting statutes & County-sticky $\alpha$ (T.3) & $w'(u,v)=(1+\alpha)\cdot w_\text{geo}$ for same-county edges & T.3 \\[0.5em]

R5: VRA \S2 / Gingles Prong 1 & 52 U.S.C.\ \S10301; Gingles (1986); Allen v.\ Milligan (2023); \callais{} (2026) & VRASection alignment score (T.7) & $A=|\mathrm{MVAP}_L - \mathrm{MVAP}_R|$; $w_\text{vra}=0.40$; Mutex enforced & T.7 \\[0.5em]

R6: No racial predominance & \shaw{} (1993); \miller{} (1995); \callais{} p.~36 & VRASection $w_\text{vra}=0.40 < $ Shaw threshold; Callais p.~36 mutex enforced & Compactness weighted $1.5\times$ over alignment; no racial target & T.7 \\[0.5em]

R7: Partisan neutrality (state courts) & LWV v.\ PA; Harper v.\ Hall; Harkenrider & AreaSection proves Rodden-null: seat counts same as GeoSection in 76\,\% of states & $\mathtt{ncon}=2$, area balance; $\mathtt{ubvec}[1]=1.10$ & T.2 \\[0.5em]

R8: Census stability & Litigation argument T.8 & StabilitySection CSS $\geq 0.67$; Iowa as known-unstable case study & Lorenz drift proxy $\Delta p^* = |p^*_{2000} - p^*_{2020}|$ & T.8 \\[0.5em]
...
R10: Multi-chamber consistency & State constitutions; anti-stacking doctrine & NestSection compatible spine (T.6) & $g=\gcd(C,S,H)$; spine score $\in [0, 100]$ & T.6 \\[0.5em]

R11: HH apportionment linkage & Art.~I \S2; 2 U.S.C.\ \S2a & ApportionRegions prime factorisation (T.4) & Bisection tree from prime factors of $k$ & T.4 \\
\bottomrule
\end{tabular}
\end{table}
\end{landscape}
...
\callais{} p.~36.
The \texttt{callais\_preflight} function in the \texttt{bisect} CLI enforces
this at runtime:

\begin{verbatim}
  bisect state --state AL --partition-mode vra-section \
