...
Standard (B.1, B.7, T.1) & Minimal intervention & Standard $k/2:k/2$ \\[0.5em]
\textit{ncon=2, pop+area} & \textit{Geographic (TIGER)} & \textit{GeoSection (T.1)} \\
AreaSection (T.2) & Compactness & Isoperimetric ratio scan \\[0.5em]
\textit{ncon=1+alignment} & \textit{County-sticky ($\alpha$)} & \textit{AreaSection (T.2)} \\
VRASection (T.7) & Subdivision preservation & Dual-constraint ratio scan \\[0.5em]
\textit{ncon=2, pop+D\_votes} & & \textit{ApportionRegions (T.4)} \\
ProportionalSection (T.5) & & Prime factorisation tree \\[0.5em]
 & & \textit{NestSection (T.6)} \\
 & & Compatible spine \\[0.5em]
 & & \textit{StabilitySection (T.8)} \\
...
AreaSection (T.2) adds a second METIS constraint: land area.
...
$\mathtt{tpwgts}=[p_L, 1-p_L, 0.5, 0.5]$ for population fraction $p_L$ at ratio $p_L:1-p_L$.
The flat C-array is row-major over constraints: entries 0--1 are the part-weights for
constraint~0 (population), and entries 2--3 are the part-weights for constraint~1 (area).
That is, the layout is $[\text{pop}_L,\ \text{pop}_R,\ \text{area}_L,\ \text{area}_R]$,
\emph{not} the interleaved form $[\text{pop}_L,\ \text{area}_L,\ \text{pop}_R,\ \text{area}_R]$
which would silently mis-weight both constraints.

\textbf{Rodden-null result (T.2)}: AreaSection produces identical seat counts to
GeoSection in 76\,\% of states (25/34 with complete data).
We exclude the 7 single-district states ($k=1$: Alaska, Delaware, Montana,
North Dakota, South Dakota, Vermont, Wyoming) and 9 states where the algorithm
produces identical partisan outcomes under all tested modes, leaving 34 states
where algorithm selection affects partisan outcomes; the 76\,\% figure is
computed over this contested subset.
The 9 divergent states differ by exactly 1 seat, with no systematic partisan direction.
...
VRASection (T.7) does not add a second METIS constraint.
...
ProportionalSection (T.5, planned) is the \emph{Callais-mutex partner} of
VRASection.
...
VRASection.
The \texttt{callais\_preflight} gate in the \texttt{bisect} CLI enforces
...
Paper T.3 introduces the governmental stickiness weight:
\[
...
T.3 provides the first formal algorithmic operationalisation of this criterion
...
GeoSection (T.1) scans all feasible split ratios $1:(k-1)$ through
...
\paragraph{EC$_\text{norm}$ definition.}
...
\textbf{Key finding (T.1)}: GeoSection selects the balanced split (or near-equal
...
AreaSection (T.2) combines the GeoSection ratio scan with the dual-constraint
...
\textbf{Key finding (T.2)}: AreaSection shifts the natural ratio from asymmetric
...
ApportionRegions (T.4, planned) derives the bisection tree structure from the
...
NestSection (T.6) extends ApportionRegions to multi-chamber consistency.
...
\textbf{Anti-gerrymandering property (T.6)}: A shared geographic hierarchy
...
StabilitySection (T.8) is not a division strategy per se, but a post-hoc
...
\textbf{Key finding (T.8)}: 67\,\% of comparable states are stable at
