...
\paragraph{Prospective procedural extension.}
The original Wisconsin and national v1 protocols remain frozen. Runs
precommitted on or after 2026-09-09 also apply
\texttt{certified-ceremony-bakeoff-extension-v1}. That extension preserves the
map and computational outcomes below and adds a separate procedural evidence
table: procedure class, $k-1$ required cuts, certified-cut coverage, released
depths, prefix verification, final-tree binding, witness policy, review delay,
status and halt reason, transcript bytes, verification time, and frozen tamper-
suite result. Unsupported or halted arms remain in the matrix.

The two scoreboards cannot be collapsed into a post hoc weighted winner. Proof
strength does not imply desirable map outcomes, and assignment reproduction
does not imply model-scoped optimality or staged public disclosure. No bakeoff
reported in this section was run as an externally witnessed ceremony; those
comparative procedural cells are pending.

A bounded path-8 fixture exercises the new scoreboard without changing that
claim boundary. Its certified four-district arm completes three of three
required cut certificates in two depth releases, verifies as a prefix, and
binds the final tree. The matched METIS replay arm is classified as
\texttt{replay-only}, with zero certified cuts and \texttt{unsupported}
ceremony status. An independent script regenerates the categorical scoreboard
and rejects a timestamp mutation. The fixture uses test receipts rather than
validated external witnesses and does not rank map quality or execute the full
production tamper corpus.
