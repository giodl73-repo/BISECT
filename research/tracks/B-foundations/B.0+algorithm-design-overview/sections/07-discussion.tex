...
The requirements matrix identifies configurations for further evaluation; the
mixed confirmed, estimated, and pending bakeoff does not determine which
configuration a practitioner should use. We organise research candidates by
use case, subject to completed execution and independent legal review.
...
Evaluate \textbf{Geo+VRASection} (config.~5), $w_\text{vra} = 0.40$, as one
illustrative-plan candidate. The Alabama fixture reports two threshold-based
minority-opportunity districts; the Georgia cells are partly estimated. Neither
the reported compactness cost nor the parameter value establishes VRA
compliance or satisfies the \miller{} racial-predominance inquiry.

\paragraph{State-court partisan neutrality claim (LWV v.\ PA; Harper v.\ Hall).}
Evaluate \textbf{Geo+AreaSection} (config.~3) as a geographic-balance
sensitivity diagnostic. It cannot prove that a map is not a product of
algorithmic partisan bias.
...
configurations.
If the state constitution requires proportionality, ProportionalSection (T.5,
...
modest EC cost.
The estimated rows suggest a possible county-split/edge-cut tradeoff; a
completed common-input sweep is required to quantify it.

\paragraph{Auditability and expert witness.}
Treat \textbf{Geo+ApportionRegions} (config.~6) as pending empirical evaluation
when the primary concern is reproducibility and parameter-freedom.
...
Geographic voter sorting is a plausible source of systematic partisan
asymmetry, but this paper does not estimate its effect independently of
algorithm and profile choices.
All configurations show a mean proportionality gap of $-7$\,pp in the eight
competitive states studied across T.1--T.2.
...
This is a central mechanism studied by \citet{rodden2019}; the B-series results
motivate a preregistered causal or ensemble study rather than independently
confirming it across 44 States.
...
T.7 confirms that this does occur (55.2\,\% and 52.8\,\% Black VAP in the two
...
StabilitySection (T.8) shows that 33\,\% of states are \emph{unstable}---their
...
The ProportionalSection (T.5) and ApportionRegions (T.4) components are
described in this synthesis but not yet implemented in the \texttt{bisect} CLI.
...
census-tract GEOID); finer levels require the T.3 Phase~2--3 implementation.
...
T.8's Census Stability Score provides a deterministic stability metric that
