...
The isoperimetric normalisation of GeoSection (T.1) additionally prevents the
...
\textbf{Algorithmic interpretation}: County-sticky edge weights (T.3) multiply
...
\textbf{Algorithmic interpretation}: VRASection (T.7) adds a geographic alignment
...
The T.2 AreaSection paper demonstrated that AreaSection and GeoSection produce
...
ProportionalSection (T.5, planned) addresses this directly by incorporating
...
\textbf{Algorithmic interpretation}: StabilitySection (T.8) measures the Census
...
ApportionRegions (T.4, planned) extends this to the Huntington-Hill linkage:
...
The \texttt{bisect} CLI records every parameter in the \texttt{whatif-manifest v1}
...
\textbf{Algorithmic interpretation}: NestSection (T.6) forces all chambers
...
\textbf{Algorithmic interpretation}: ApportionRegions (T.4, planned) determines
