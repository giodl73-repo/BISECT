...
requirement as follows:\footnote{%
  All page citations to \callais{} refer to the slip opinion as issued by the Court.
  The final U.S.\ Reports citation and permanent page numbers will be updated when
  the opinion is paginated in the bound volume.  The disentanglement language quoted
  here is from the majority opinion; the slip opinion does not carry section headers
  distinguishing majority, concurrence, or dissent by page number.}
...
\begin{itemize}
  \item \textbf{VRASection (T.7)}: Geographic alignment score on minority VAP.
  ...
  \item \textbf{ProportionalSection (T.5, planned)}: Partisan vote-share balance
    constraint.
...
It is called at the start of every \texttt{bisect state} and \texttt{redist run}
...
Every \texttt{bisect} run produces a \texttt{whatif-manifest v1} JSON sidecar
...
\begin{itemize}
  \item GeoSection (stated political goal: efficient district layout):
    0D, 1 MM district.
  \item VRASection: 1D, \textbf{2 MM districts} (confirmed, T.7).
  \item Research diagnostic: the fixture produces a second threshold-based
    minority-opportunity district. This does not adjudicate strong inference or
    establish an \allenmilligan{} remedy.
\end{itemize}
...
\begin{itemize}
  ...
  \item Strong-inference status: unavailable because both comparison rows are
    estimated.
\end{itemize}
...
\begin{itemize}
  ...
  \item Strong-inference status: unavailable because the VRASection row is
    estimated.
\end{itemize}

\paragraph{Strong-inference interpretation.}
...
The ablation isolates the alignment signal within the Alabama fixture: at
$w_\text{vra} = 0$, Alabama produces $3:4$ first-bisection and
1 MM district (identical to GeoSection); at $w_\text{vra} = 0.40$, Alabama
produces $2:5$ and 2 MM districts.
This mechanism result does not identify causal partisan effects. The broader
six-mode claim is not testable from this matrix because some algorithm rows are
pending or estimated; comparison with within-algorithm seed variance likewise
requires a completed common-input execution.
