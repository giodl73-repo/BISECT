...
This synthesis paper presents a unified three-axis taxonomy of fifteen algorithmic components for congressional redistricting, mapping each to specific legal requirements and validating eight mode combinations across four contested states. The paper's primary value is in unifying a broad empirical programme (B.1–T.8) into an actionable practitioner's matrix, and in demonstrating that METIS-based bisection can be structured to satisfy post-Callais constitutional architecture.
