...
The T.4 footnote correction is the right approach to a forward-citation problem. "Companion paper T.4 demonstrates near-zero variance in NC and WI" is a statement that is true (based on preliminary runs) and appropriately hedged; the full 50-state claim is correctly deferred to T.4.
...
The T.4 citation fix is the most important change in this revision. The runtime and TIGER/Line vintage carry forward as P1 items. The paper's computational design remains the strongest in the B-series for reproducibility, but the gap between the design and the implementation (T.4 not yet complete) limits what can currently be claimed.
