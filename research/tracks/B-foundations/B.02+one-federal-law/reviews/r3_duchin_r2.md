...
I accept that this requires T.4 to be complete for a full resolution. As an interim fix, the paper should reframe the Proposition as: "ApportionRegions is constitutionally derived if and only if the intrastate HH priority value is well-defined and unique (to be demonstrated in T.4)." This converts the circular proof into a conditional claim pending empirical validation.
