...
The paper still states "runs a 50-state apportionment in approximately 30 minutes on a laptop" without qualification. The AR-specific runtime (with multi-way METIS calls for large prime factors) is not measured. This must be qualified to "GeoSection-based components" or labelled as estimated pending T.4 implementation. A statute that cites a 30-minute feasibility claim in its supporting advocacy paper is potentially vulnerable to challenge if the actual AR runtime for California (k=52, with 13-way METIS calls) is substantially longer.
