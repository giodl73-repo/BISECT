...
The Round 2 revision addresses the T.4 citation problem by replacing the specific "Table 3" reference with a companion-paper forward reference, and adds the canonical ordering resolution for repeated prime factors. The METIS k-way vs. bisection underspecification and the ubvec tolerance inconsistency are not directly addressed. Score maintained at 3.

## P1 Resolution

**P1.1 — T.4 "Table 3" citation: RESOLVED.**
The revised footnote correctly replaces the specific T.4 Table 3 citation with: "Companion paper T.4 (ApportionRegions) demonstrates near-zero partisan variance across seeds in NC and WI, the two most-litigated competitive states. Full 50-state variance tables will appear in T.4 upon completion of the ApportionRegions experimental sweep; preliminary runs confirm the pattern holds in all tested states." This is the correct epistemic framing — it accurately represents T.4's status (companion, pending full results) without fabricating a specific table number. The zero-variance claim is now correctly stated as a pattern confirmed in preliminary runs rather than as a complete 50-state result.
...
The T.4 citation fix and the canonical ordering resolution are both correct and necessary. The ubvec tolerance inconsistency and METIS k-way specification carry forward as P1 items for the journal submission version.
