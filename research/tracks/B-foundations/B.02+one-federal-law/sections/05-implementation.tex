...
The algorithm is implemented in the \texttt{bisect} Rust binary
...
This \emph{Convergence Sweep} (U.1, \citealt{b16paper}) certifies that the
...
by the U.1 analysis of US census tract graphs.
...
implemented via the T.3 subdivision-respecting edge-weighting scheme \citep{b10paper}.
...
\paragraph{County preservation and census stability.}
There is a second, independently significant reason to prefer county-sticky
edge weights in the federal statute: \emph{census stability}.
T.8 (\citealt{b15paper}) shows that 67\,\% of states produce the same
first-level bisection across the 2000 and 2010 census years.
The four unstable states --- Alabama, Iowa, South Carolina, and Utah --- all
underwent dramatic demographic shifts between census years.
County boundaries, by contrast, are among the most physically stable
features in US geography.
A redistricting algorithm that respects county structure is anchored to
fixed geographic features rather than to the decennial population snapshot,
and therefore tends to produce census-stable maps.

\begin{quote}
\textit{Legal significance}: A map that emerges from the same county-anchored
algorithm in 2000, 2010, and 2020 cannot plausibly be the product of partisan
intent.
Counties did not move.
The algorithm did not know which party would benefit from any particular
county grouping in any particular decade.
The convergence of three independent algorithmic runs --- across different
population snapshots but the same county structure --- is the strongest
possible evidence of geographic rather than partisan determination.
\end{quote}

This is the argument that connects T.3 (county preservation), T.8 (census
stability), and B.02 (the federal statute) into a single constitutional claim:
a county-anchored, prime-factored, convergence-certified map is the most
legally defensible redistricting plan available under current US doctrine.
...
\paragraph{Scope of the mutex.}
\callais{} p.~36 holds that race and party cannot be co-optimized in the
\emph{same METIS objective}: a redistricting process that combines
VRASection (minority geographic alignment score) and partisan-weighted bisection
in a single run is constitutionally suspect because a reviewing court cannot
isolate the racial motivation from the partisan motivation.
Consistent with this holding, the \dia{} prohibits combining VRASection and
ProportionalSection in a single computational run.
It does not prohibit the sequential, independent comparison of a VRASection
map against a ProportionalSection map---such comparison is the \callais{}
strong-inference test itself, and is expressly contemplated by the majority
opinion as the correct analytical framework for identifying when political goals
could have been achieved with better minority outcomes.
...
adjustment), VRASection is automatically disabled for that run.
The two modes are mutually exclusive within a single run and the gate is enforced
by the \texttt{bisect} binary at runtime: attempting to configure both will cause
...
apportionment was performed, with county boundaries weighted at
$\alpha_\text{county} = 2.0$ (subsection (d)), beginning from a seed
determined by the formula \texttt{SHA-256(census\_release\_id || `DIA\_SEED\_V1')},
and halting when 600 consecutive seeds produce no improvement in normalised
edge cut (subsection (c)(2)).

\medskip
\noindent\textbf{(c)(2) Convergence Criterion.}
The algorithm shall iterate over seeds beginning at the content-derived
seed value computed under subsection (c), halting when no improvement in
normalised edge cut has been observed for 600 consecutive seeds, and shall
use the minimum edge cut plan found across all seeds evaluated.
This criterion is certified to suffice for all 50 states at the congressional
district level by the ConvergenceSweep analysis (\citealt{b16paper}).
