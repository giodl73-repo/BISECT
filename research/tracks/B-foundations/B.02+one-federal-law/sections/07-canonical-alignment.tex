...
Publication should use a certified sequential-cut ceremony when exact split
evidence is available. Before the root cut, the administering body publishes a
genesis commitment to the rule profile, root instance, canonical district tree,
review interval, witness policy, and challenge rules. Later cuts are released
by tree depth only after the prior review window. Independent observers can
verify each disclosed prefix without receiving future descendants. A sustained
challenge preserves a halted transcript and requires an explicitly linked new
ceremony rather than silent substitution.

This publication layer does not transfer final-plan authority to software.
Legally required changes and governed departures remain the responsibility of
the authorized body and must be recorded outside the frozen benchmark. External
transparency-log or trusted-timestamp validation is necessary before statutory
deadlines can rely on ceremony timestamps.
...
pseudo-Boolean proof requests. It also implements hash-chained ceremony
genesis, announce, verify, and halt transitions; prefix verification does not
require unreleased cuts. A connected Rhode Island RCTX contains all
