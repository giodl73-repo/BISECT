...
implemented via the T.3 subdivision-respecting edge-weighting scheme
\citep{b10paper}.
...
\paragraph{Can Congress mandate a specific algorithm, not merely substantive standards?}
A more targeted objection is that the Elections Clause authorises Congress to
prescribe \emph{substantive standards} (equal population, compactness, contiguity)
but not to mandate a \emph{specific computational procedure} for achieving those standards.
This objection confuses two distinct congressional powers.
\textit{Smiley v.\ Holm}, 285 U.S. 355, 366--67 (1932), held that Article~I, \S4
authorises Congress ``to make or alter Regulations in the broadest sense,''
encompassing ``the manner of holding elections and the proceedings in connection
therewith, including the registration of voters, the selection of candidates, and
the method of choosing electors.''
Mandating a specific redistricting algorithm is a regulation of the \emph{manner}
of holding elections---the same category as mandating a specific ballot format,
electronic voting system, or registration database structure.
Courts have upheld all of these as valid exercises of Elections Clause authority.
The \dia{} does not impose a substantive political outcome; it specifies a
procedure that is required to use only census data as input.
Congress's power to prescribe such a procedure is at its zenith: it is technically
precise (eliminating discretion), politically neutral (no partisan inputs), and
grounded in an existing statutory mandate (2~U.S.C.~\usca{}~2a(a)) that Courts have
already upheld.
The litigation risk on this point is low; the best available response is the
``procedural regulation under plenary Elections Clause authority'' framing, which
has a direct precedential basis in \textit{Smiley} and in the congressional history
of election administration statutes.
...
\paragraph{Proposed VRA review and opportunity overlay.}
...
Whether this constitutes predominant racial targeting under
\shaw{}--\miller{} is a legal question; the algorithm cannot resolve it.
...
v1} JSON sidecar produced by the \texttt{bisect} CLI \citep{b0paper}.
...
manifest in approximately 30 minutes using the open-source \texttt{bisect}
binary.
The execution can be audited against its manifest. That audit does not remove
the discretion embodied in the statute, profile, data vintage, adjacency
rules, engine, seed rule, or legally required modifications. Current
release-grade independent replay remains a separate VTRACE gate.
