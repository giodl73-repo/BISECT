...
\subsection{Target CLI Invocation and YAML Configuration}

A production \pt{} surface would expose a search mode like:
\begin{verbatim}
...
This command shape is a design target unless the current build exposes the
corresponding CLI flags and tests.
...
\paragraph{Legal provenance of parallel chains.}
A special master or opposing expert may ask: does using $N$ replicas change the
distributional properties of the submitted plan compared to a single-chain
\fr{} run?
The answer is no: the submitted plan is drawn from the cold chain
($\varepsilon = \varepsilon_{\text{cold}}$, the same tolerance as single-chain
\fr{}), and the cold chain's stationary distribution is the same \fr{} target.
The hot replicas serve as exploration aids; they do not affect the cold chain's
targeting.
The audit chain records all $N$ chains' seeds, swap events, and the cold-chain
\texttt{selected\_step}, enabling full independent verification.
