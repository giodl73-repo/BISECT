...
The chain has a well-defined ReCom sampling distribution over
population-balanced redistricting plans; it is not a proof of uniformity
over all legally valid plans, and finite runs still require diagnostic
checks.
...
Within the \texttt{bisect} ecosystem, the Rust codebase has achieved
...
autocorrelation diagnostics implemented in the \texttt{bisect} binary.
The U.9 paper~\citep{dellaLibera2026h1} establishes the bisection-ensemble
connection that motivates local ReCom integration inside the BISECT
construction tree.
\re{} narrows a major reproducibility gap: G-track results previously
depended on the Python \gc{} installation; the Rust crate makes the core
ReCom substrate available inside the BISECT workspace. Full replacement of
the historical G-track runs still requires the Phase~2 cross-validation
