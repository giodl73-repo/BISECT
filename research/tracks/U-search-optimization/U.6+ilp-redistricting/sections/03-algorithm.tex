...
\If{$n > n_{\max}$}
  ...
  \Comment{size guard}
\EndIf
\State Build formulation summary $F$ with constraints
  \eqref{eq:coverage}--\eqref{eq:flow-root}
  \Comment{full MPS/LP export is an evidence-layer requirement}
...
The intended solver path uses a two-tier priority order:

\begin{enumerate}
\item \textbf{\glpk{} via \texttt{good\_lp}} --- pure Rust $+$ C binding,
  no subprocess, when available.
...
If neither solver is available and $n \leq n_{\max}$, the implementation should
return a formulation or solver-unavailable status rather than silently treating
the result as exact.

\subsection{Size Guard and Fallback}

If the subgraph at a bisection node exceeds the configured exact-solve limit,
\ilp{} is not the proving mechanism for that node:

\begin{verbatim}
WARNING: ILP solver skipped for node at depth 2 (847 tracts > 500 limit).
         Exact solver skipped; heuristic or formulation-only status required.
\end{verbatim}

The package must record a non-exact status so that the audit chain accurately
reflects which portions of the bisection tree used exact optimisation and which
used heuristic or formulation-only fallback.

\subsection{MPS File Retention}

Before courtroom-grade verification, the formulation must be serialised to a
replayable LP/MPS artifact and hash-bound into the run package.  An external
auditor can then reload it into an LP/MIP solver and verify that the committed
plan achieves the reported objective value.  Without the replayable model file
and solver status, the artifact is formulation evidence rather than an
