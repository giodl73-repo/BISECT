...
We have introduced \ilp{} redistricting as the exact-optimization boundary for
edge-cut compactness.  For small instances, the ILP formulation can support a
mathematical optimality certificate when the package includes the replayable
model, solver status, incumbent, bound, and gap.
...
\item \textbf{Small-state certification.}
  ...
  can be superior to a heuristic when the solver certificate is archived:
  it can add a model-scoped certificate that no heuristic can provide.
...
For production 50-state builds, or any state where bisection nodes exceed the
configured exact-solve limit, \metis{} or another heuristic remains the
practical default.  Any fallback must be recorded as non-exact in the audit
chain.
