...
\begin{itemize}
  ...
  \item no permitted connected cut improved the objective at any node;
  \item the final assignment is the unique canonical BISECT plan; and
  \item when the ceremony also verifies, accepted cuts were disclosed in
  canonical depth order with the frozen minimum intervals and hash chain.
\end{itemize}
...
Nor does a self-asserted timestamp prove public availability: the production
ceremony claim additionally requires validation by external witness systems.

Nor does the present implementation show that certified recursion ``wins'' on
county preservation or nationwide geometry. The 2020 national bakeoff supplies
a common-block descriptive comparison with official CD118 assignments, but it
does not measure original enacted linework and does not turn four metric
directions into a composite quality ordering. The precommitted eight-instance suite now runs the same
...
the central open engineering question. National election, demographic,
alternative-structure, and converged-ensemble comparisons remain open empirical
questions under separate input and preregistration gates.

Future bakeoffs therefore report two non-combinable result families. The first
retains map validity, objective values, downstream metrics, runtime, and
timeouts. The second reports procedure class, certified-cut coverage, prefix
verification, completion or halt status, transcript size, verifier time, and
negative-corpus results. This avoids declaring an overall winner by choosing
weights after observing a tradeoff between map outcomes and proof strength.
