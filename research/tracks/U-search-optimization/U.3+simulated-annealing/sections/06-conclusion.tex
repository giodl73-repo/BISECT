...
\sa{} bisection is implemented in the \texttt{bisect} binary under
...
algorithm:
  ...
  sa_t0_factor: 0.01
\end{verbatim}
The accompanying \texttt{U.3+sa-smoke} package gives a minimal replayable
fixture for the seed and grid-split mechanics.  It deliberately stops short of
certifying the empirical table above; publication-grade empirical claims still
require archived state run packages with assignments, metrics, parameters, and
lineage.

\paragraph{Open questions.}
Several extensions are straightforward to implement and are left for future work:
(1) combining SA bisection with the \ar{} prime-factor tree structure (T.4),
