...
(T.1), and a single seed ($s = s_0$, the content-derived seed).
The four algorithms compared are:

\begin{enumerate}
\item \textbf{METIS (baseline)}: Single \metis{} call per node, no SA.
  This is the standard T.1 configuration.
...
These state-level figures are treated as empirical claims requiring archived
run packages.  The current repository evidence for U.3 is the hash-bound
\texttt{U.3+sa-smoke} fixture, which replays deterministic seed derivation and
a small-grid annealing split; it verifies mechanics only, not Table~\ref{tab:main}.
...
This is consistent with U.2's finding that structure dominates search; the
...
\subsection{Comparison with the U.9 Baseline}

U.9~\citep{h1paper} reports PP baselines for METIS bisection on 50 states
using the geographic-weight configuration.
The NC baseline PP$= 0.217$ used in Table~\ref{tab:main} matches the U.9
50-state figure.
Our SA(steps=10) result PP$= 0.248$ represents a 14.3\% improvement, which
places SA bisection between the U.9 ``multi-seed'' result (10--12\%)
