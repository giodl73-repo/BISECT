...
\cs{} (U.1) sweeps forward through seeds, running \metis{} at each, and
halts when $T=600$ consecutive seeds produce no improvement in normalised
edge cut~\citep{b16paper}.
\be{} (U.9) runs $T$ independent \metis{} seeds in parallel and selects
...
\geosec{} (T.1) replaces the flat tree with a ratio-optimal tree that
normalises edge cut by $\sqrt{\min(i,k-i)}$~\citep{b8paper}.
\areasec{} (T.2) adds an area-swing constraint to bias toward
