...
\]
Following T.1~\citep{b8paper}, the \emph{normalised edge cut} is defined
contextually depending on the partition method:

\paragraph{Recursive bisection context.}
When $\Pi$ is produced by a recursive bisection tree (the standard \ar{}
method), the normalisation uses the split balance at each bisection level.
At level $\ell$ of the tree where the current region of $k_\ell$ districts
is split into two sub-regions of $i$ and $k_\ell - i$ districts respectively,
\[
  \ECnorm^{(\ell)}(\Pi) = \frac{\mathrm{EC}^{(\ell)}(\Pi)}{\sqrt{\min(i,\, k_\ell - i)}}.
\]
The denominator $\sqrt{\min(i, k_\ell - i)}$ normalises by the expected
edge-cut growth at balanced vs.\ unbalanced splits: more balanced splits
(larger $\min$) are penalised relatively less because they divide more population
equally.
The overall $\ECnorm(\Pi)$ is the sum of $\ECnorm^{(\ell)}$ across all levels
of the recursive tree.

\paragraph{Full $k$-way partition context.}
When $\Pi$ is produced by a direct $k$-way METIS call (used for prime-seat-count
states and for the Wisconsin result comparison in Section~\ref{sec:sufficiency}),
the bisection-level denominator is undefined.
In this context, we use the flat normalisation:
\[
  \ECnorm(\Pi) = \frac{\mathrm{EC}(\Pi)}{\sqrt{k/2}},
\]
where $\sqrt{k/2}$ is the denominator that would apply to a balanced $k$-way
partition with $i = k/2$ at every level.
This is the natural generalisation: for a fully balanced $k$-way split, each
part has the same size, and $\min(i, k-i) = k/2$ for the terminal cuts.
Results in Table~\ref{tab:convergence} use this normalisation for
direct-$k$-way states (Nebraska, New Mexico) and the recursive-bisection
normalisation for all other states; the table legend identifies which
normalisation applies.

\begin{remark}[Cross-state comparability of convergence tails]
Convergence tails $\tau$ (Definition~\ref{def:tail}) are directly comparable
across states \emph{only when the same $\ECnorm$ normalisation is applied
throughout the sweep}.
The B.7 dataset uses recursive-bisection normalisation for all states that
receive a recursive-bisection METIS call, and flat $k$-way normalisation for
Nebraska and New Mexico.
Practitioners who extend the B.7 sweep to new census cycles or new states
must apply the same call mode and the same normalisation formula consistently;
mixing normalisation conventions across states would make the resulting $\tau$
values incomparable and could produce spurious worst-case identification.
\end{remark}
...
In the atlas framing, the candidates are the full sequence of seeded plans,
not only the final minimum.
Each candidate carries its seed index, derived METIS seed, normalised edge cut,
population-balance status, and whether it improved the incumbent.
The decision rule is the first plan with minimum observed $\ECnorm$ before the
$T$-long no-improvement tail closes the sweep.
The consequence is a selected plan plus a stopping certificate: the run should
preserve the seed stream, incumbent-improvement events, $T$, $j^*$, and
$j_{\mathrm{stop}}$ so that another reader can replay why the sweep stopped.
This evidence supports a best-observed-under-threshold claim, not a proof of
global optimality.
...
the same partition $\Pi^*$ on every execution, provided METIS is invoked with
\texttt{METIS\_OPTION\_NTHREADS=1} (single-threaded mode), regardless of
hardware or operating system.
\end{proposition}

\begin{proof}
$s_0$ is determined uniquely by Definition~\ref{def:seed}.
METIS is deterministic given $s$ \emph{when run single-threaded}: the
multilevel coarsening and refinement sequence is fully specified by the seed
and graph structure.
METIS's OpenMP-parallel variants use a non-deterministic work-stealing
scheduler; the statutory build therefore sets \texttt{METIS\_OPTION\_NTHREADS=1}
...
the same floating-point model on all platforms, \textbf{and that METIS is
invoked with \texttt{METIS\_OPTION\_NTHREADS=1} (single-threaded mode)}.
The \texttt{bisect} binary ships a vendored METIS with determinism flags
...
Furthermore, \textbf{METIS must be run single-threaded} for the statutory
certificate to hold.
METIS's OpenMP-parallel variants use a non-deterministic work-stealing
scheduler: two runs with the same seed but different thread-scheduling
outcomes may produce different intermediate coarsenings and therefore
different local optima.
The statutory build therefore sets \texttt{METIS\_OPTION\_NTHREADS=1} at
initialisation, ensuring thread-count-independent determinism; independent
verifiers must use the same setting to reproduce the certified partition.
\end{remark}
...
approximately $8.1$\,min---well within the 30-day statutory submission deadline.
