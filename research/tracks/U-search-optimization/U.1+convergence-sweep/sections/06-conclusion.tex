...
\item \textbf{T=600 certifies the observed 50-state tract-level sweep.}
...
\subsection{The Gap U.1 Closes in B.02}
...
\begin{enumerate}
\item What search procedure selects the incumbent plan given the seed stream?
\item How many seeds must be tried before the search can certify it has
      converged?
\end{enumerate}

U.1 answers both questions.
...
\item \textbf{(\wesberry{}, B.1)} Congressional districts must have equal
      population.
\item \textbf{(T.4)} The \ar{} algorithm applies the \hh{} priority rule
      intrastate, exactly mirroring the interstate apportionment method whose
      constitutional pedigree is uncontested.
\item \textbf{(B.2, T.1)} The geographic edge-weighting scheme selects the
...
\item \textbf{(B.7)} The solution space is concentrated: across 1{,}500 seeds,
      ...
      outlier.
\item \textbf{(U.1, this paper)} \cs{} with $\Tstat = 600$ certifies that
      the selected plan is the minimum-$\ECnorm$ incumbent observed before a
      600-seed no-improvement tail, with $P(\tau > \Tstat) < 10^{-3}$ under
      the descriptive tail model.
\item \textbf{(U.1, this paper)} The SHA-256 seed formula ensures that no
...
\item Auditable under its own objective function and stopping rule (step 5); and
...
Whether \cs{} convergence tails scale with $n$ is unknown; the finite-output
observation in Proposition~\ref{thm:finiteseed} is not a useful runtime bound,
so empirical evidence at block resolution would be needed before block-level
redistricting could be certified with $\Tstat = 600$.
METIS complexity is $O(m \log n)$, so a 100$\times$ increase in $n$ (from
${\sim}15{,}000$ tract-level to ${\sim}1.5 \times 10^6$ block-level vertices)
increases per-seed time by approximately $100\times$ and may increase the number
of distinct local optima.
An explicit empirical certification at block-level resolution is the subject
of future work.

\item[Multi-chamber compatibility.]
T.6~\citep{b13paper} establishes a NestSection algorithm for multi-chamber
...
We chose $\Tstat = 600$ to provide an 89-seed margin---approximately 0.46
standard deviations of the fitted Gumbel distribution (the standard deviation
is $\hat\sigma \cdot \pi/\sqrt{6} \approx 150 \times 1.28 \approx 192$ seeds;
$89/192 \approx 0.46$)---against uncertainty about future census geographies.
...
\begin{quote}
  \itshape
  Our algorithm is reproducibly certified: it sweeps forward until 600
  consecutive seeds produce no improvement.
  Our empirical tract-level dataset shows no 2020 US state requires more than
  512 consecutive seeds.
  Our statistical model assigns probability below one-in-a-thousand to any US
  comparable state graph requiring more than 600.
  ...
  The map is auditable, reproducible, and certified as the best observed plan
  under the statutory threshold.
\end{quote}
