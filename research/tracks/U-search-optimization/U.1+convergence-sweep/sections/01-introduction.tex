...
produces is the best observed map under its own seed-search protocol.
...
\item \textbf{Convergence certification.} If the algorithm has not improved in $T$ consecutive
      ...
      $10^{-3}$ under the descriptive tail model fit to the 50-state sweep.
\end{enumerate}
...
Section~\ref{sec:implementation} documents the existing \texttt{bisect}
binary implementation and the federal-statute invocation string.
Section~\ref{sec:conclusion} closes by situating U.1 in the B.02
...
T.4~\citep{b11paper} established zero seed variance for the \ar{}
