...
time and read by \texttt{bisect analyze} and \texttt{bisect report} to
...
was used.
The \texttt{index\_to\_geoid\_file} field is included even for
...
\noindent
This enables verifiers to confirm the unit count and check for orphaned GEOIDs
without running any pipeline code --- the GEOID file alone is sufficient to
validate \texttt{n\_units} and confirm every GEOID is well-formed.

\subsection{Downstream Pipeline Behavior}

\textbf{Analysis} (\texttt{bisect label-analyze}): reads
...
\textbf{Reporting} (\texttt{bisect label-report}): uses \texttt{unit\_type}
...
\textbf{Verification} (\texttt{bisect label-verify}): the SHA-256 hash
...
\subsection{Smoke Package Boundary}

The repository includes \texttt{U.11+resolution-smoke}, a minimal package that
hash-binds a synthetic tract-to-county fixture.  Its verifier recomputes the
GEOID-prefix partition, checks \texttt{plan\_resolution}, \texttt{n\_units},
and \texttt{unit\_type}, aggregates fine populations to county-level totals,
and derives the coarse adjacency graph from fine adjacency.  The package is a
manifest and topology replay test; it does not substantiate the Texas
autocorrelation table or BG-level precision projections.
