# Review Synthesis — U.11: Resolution-Aware Redistricting
**Round**: 1
**Date**: 2026-05-09
...
U.11 completes the H-track by making geographic resolution a first-class pipeline parameter. U.8 (PercentileSweep) and U.9 (BisectionEnsemble) focus on the search dimension; U.10 (redist-ensemble) addresses throughput; U.11 addresses spatial granularity. The track arc requires U.11 to be credible on both the algorithmic side (GEOID derivation, multi-scale Markov chains) and the legal side (VRA implications, litigation-admissibility of resolution choices).
