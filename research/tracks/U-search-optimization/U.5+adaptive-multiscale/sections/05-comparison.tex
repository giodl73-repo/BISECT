...
(\texttt{--weights-override geographic}), and a prime-factor structure
...
\noindent\textit{Note: Edge-cut (EC) comparison is deferred to Phase 2; see B.3 for a multi-metric baseline.}
...
All ACF and compactness values are single-seed (seed $s = 42$).
The $\hat{R}$ diagnostics across 5 seeds ($s \in \{42, \ldots, 46\}$)
show $\hat{R} < 1.08$ for Democratic seat share across states, indicating
the marginal seat distribution is stable across seeds though not across
initial plans.
