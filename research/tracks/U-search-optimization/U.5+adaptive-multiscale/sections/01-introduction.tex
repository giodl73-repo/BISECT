...
\textit{Note: This holds for greedy refinement; full MCMC steps are
non-monotone by design (acceptance of worse plans maintains detailed balance).}
...
and $\gamma_t = \gamma_0 / t^{3/4}$ is a decaying step size.
This step-size schedule satisfies both Robbins--Monro conditions:
$\sum_{t=1}^\infty \gamma_t = \infty$ (since $3/4 < 1$, the series diverges) and
$\sum_{t=1}^\infty \gamma_t^2 < \infty$ (since $3/2 > 1$, the series converges).
...
\begin{enumerate}
  \item \textbf{Algorithm (\ams{}).}
    ...
    $\Delta = 50$ steps.
    Three design choices are analysed: the $\gamma_0/t^{3/4}$ step-size
