...
\begin{itemize}
  \item Use adaptive multiscale search where the current build exposes a
    checked-in mode and package path; otherwise treat it as a library substrate
    and paper protocol.
...
The \texttt{U.5+adaptive-multiscale-smoke} package now provides a replayable
verifier for that mechanics claim.  State convergence and autocorrelation
claims remain package-bound future evidence.
...
\ams{} has a Rust substrate in \texttt{bisect-multiscale}, including
\texttt{AdaptiveMultiScaleChain}-style tuning behavior.  CLI and manifest
claims should be made only where the current binary and tests expose that
surface.
