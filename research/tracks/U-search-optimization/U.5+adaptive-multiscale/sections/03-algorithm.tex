...
The repository smoke package \texttt{U.5+adaptive-multiscale-smoke} verifies
this update rule on a synthetic acceptance ledger, including $\gamma_t =
\gamma_0/t^{3/4}$, clipping to $[0.05,0.95]$, final-alpha replay, and the
shared \texttt{MSC\_STEP\_} seed formula.
...
\subsection{Target CLI and YAML}

\paragraph{Target CLI.}
\begin{verbatim}
...
Advanced parameters such as \texttt{--ms-gamma0} and
\texttt{--ms-initial-alpha} are the expected expert controls for diagnosing slow
convergence once the mode is wired to a production CLI.

\paragraph{Target YAML.}
\begin{verbatim}
algorithm:
...
Every production run of adaptive multiscale search should record the following
fields in its manifest:
...
This ensures \texttt{bisect label-verify} can confirm all search parameters
