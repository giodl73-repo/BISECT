...
undiscovered.
By sweeping forward until 600 consecutive seeds produce no improvement, \cs{}
certifies the best observed incumbent under its statutory stopping threshold.
U.1 documents the 89-seed empirical margin above Georgia's observed 2020
convergence tail and treats the Gumbel tail model as supporting
evidence~\citep{b16paper}.
...
of the sampled \recom{} ensemble for most states (G.1~\citep{g1paper}).
In the sampled G.1 ensembles, the plan is more compact than 99\%+ of sampled
\recom{} alternatives for those states.
...
that partisan seat counts are \emph{identical} across $p \in \{0.0, 0.25, 0.5,
0.75, 1.0\}$ for four confirmed states (NC, WI, GA, PA) with $T = 101$ plans.
For Texas and California (extrapolated from T.4 single-run data), projected
variation is at most 0.5 seats; full \ps{} sweeps for TX and CA are deferred
to U.10~\citep{h2paper}.

This finding extends U.2's parameter insensitivity result~\citep{b17paper}
to the search percentile dimension.
U.2 showed that partisan outcomes are insensitive to METIS parameters,
convergence threshold $T$, and edge-weight settings within a fixed algorithm
family.
U.8 shows they are equally insensitive to which compactness level within
