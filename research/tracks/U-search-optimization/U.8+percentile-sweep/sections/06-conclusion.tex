...
\times (T-1)}$, \ps{} allows the \dia{} to specify exactly which position in
...
projected Democratic seat counts are identical across
$p \in \{0.0, 0.25, 0.5, 0.75, 1.0\}$.
Texas and California remain extrapolated from T.4 single-run data, with
full sweeps deferred to U.10.
This confirms that compactness is determined by geographic structure, not by
search strategy---consistent with B.7 and U.2.
...
The partition-space insensitivity of U.2 and the percentile insensitivity
of U.8 together support a narrower claim: within the audited bisection-family
variants studied here, percentile choice is not the driver of partisan outcome.
...
\floor{p \times (T-1)}$ is a cruder quantile estimate.
