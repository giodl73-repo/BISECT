...
the challenger must produce a plan more compact than almost all sampled
G.1 ensemble plans.
...
The $p = 0.0$ plan is the minimum edge-cut member of the audited $T$-seed
bisection candidate set.
It can be certified by running all $T$ declared seeds and verifying that no
candidate in that set achieves a lower $\ECnorm$.
...
$p = 0.0$, the result has the strongest compactness posture within the same
audited sample.
When the statutory objective is convergence certification rather than a fixed
sample percentile, U.1's \cs{} threshold $T_{\mathrm{stat}}=600$ supplies the
corresponding stopping rule~\citep{b16paper}.
...
The \dia{} shall specify the compactness-extremum posture: either
\cs{} with $T_{\mathrm{stat}} = 600$ when using a convergence certificate, or
\ps{}$(0.0, 600)$ when using a fixed audited candidate distribution.
The legal rationale is the compactness doctrine: the resulting plan is the
lowest edge-cut member selected under the declared seed-search rule, subject
to population balance and contiguity within the \ar{} bisection family.
The statutory record shall state that partisan outcomes are invariant to the
compactness percentile in the four fully swept U.8 states, so the compactness
choice is not supported by evidence of partisan cherry-picking.
\end{statute}
