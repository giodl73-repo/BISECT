...
The `aswing` normalization clarification in Section 2.4 (confirming the T.2 normalization, with the cross-reference) resolves my terminological concern from Round 1.
...
The U.1 citation issue from Round 1 (U.1 appearing in the text but missing from the reference list) appears to have been partially resolved — U.1 is now in the references, but the specific claim cited to it (the Georgia tail analysis with T=100) still uses a parenthetical reference rather than the numbered citation. This is a formatting issue, not a substantive one.
