...
The critical limitation considered in this paper---stalling under difficult
large-state or large-factor configurations---is a known but underreported
problem in the GerryChain community.
When two districts are merged and the combined subgraph has $n_{\text{merge}}$
tracts, the spanning tree cut must achieve a balanced split of that merged
region while respecting the geometry inherited from surrounding districts.
For $k=38$ (Texas), each district
...
regions of $2.6\%$ each. This is a 1:1 split within the merged pair, not a
1:37 split of the full state; the difficulty is geometric sparsity of accepted
cut edges in the sampled trees.
...
it applies Wilson's algorithm to the current bisection-tree node, whose size is
approximately $n/2^\ell$ at depth $\ell$, rather than to a full-plan ensemble
state. At the root this can still be a large subgraph; the locality advantage
grows with tree depth.

\paragraph{PercentileSweep (U.8).}
...
\paragraph{BisectionTree methods (B.1, T.4).}
...
and extended in \citet{dellaLibera2026apportion} (T.4: ApportionRegions)
