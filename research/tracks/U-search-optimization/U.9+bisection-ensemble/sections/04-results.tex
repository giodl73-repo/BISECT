...
All timing results were collected on an Intel Core i7-11800H (8 cores,
2.3~GHz base / 4.6~GHz boost, 24~MB L3 cache) running Windows 11 22H2,
with Rust 1.75.0 (\texttt{rustc 1.75.0}), METIS 5.1.0 (bundled as
\texttt{redist-metis} crate), and Rayon parallelism enabled across all available cores.

All results in Tables~2--4 are single-run observations from one BisectionEnsemble
run per (state, $p$) combination. The distribution of results across independent
runs with different METIS initialisation seeds is deferred to Phase~2.
...
\caption{Bipartition failure comparison. GerryChain stalls on Texas ($k=38$):
  ...
  GerryChain configuration for acceptance rate measurements: version 0.3.2
  (\texttt{pip install gerrychain==0.3.2}), population balance tolerance
  $\varepsilon = 0.01$, non-reversible ReCom, cold start from a single
  random valid plan (seed 0), single-run observation per state. The TX
  acceptance rate row uses the pair-reselection extension described in
...
In these single-run observations, BisectionEnsemble improves compactness over
standard METIS bisection.
...
Within each accepted local candidate set, the minimum-cut variant ($p=0.0$)
achieves the best compactness by definition:
...
The key finding from these point estimates is limited: BisectionEnsemble does
not show a uniform partisan direction across the three tested states.
Partisan outcomes are stable across percentile levels $p$ for NC (zero seat
change across all five $p$ values in Table~\ref{tab:partisan}). Extension to
GA and PA is deferred to Phase~2 empirical runs.
NC reflects the geographic robustness of its near-proportional outcome
