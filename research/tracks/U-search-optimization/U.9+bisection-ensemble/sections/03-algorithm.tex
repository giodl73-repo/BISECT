...
manages $\approx n/2^\ell$ tracts: the root node ($\ell=0$) manages the full
...
grows with depth. At the root, the local ensemble still spans the whole state
subgraph; the full locality benefit is realised at deeper levels.
...
\begin{definition}[Local bisection ensemble]
  ...
  \begin{enumerate}
    ...
    \item \textbf{Select}: Sort $\Pi$ by $\text{EC}(\cdot)$. Return
          $\pi^* = \Pi[\lfloor p \cdot (m-1) \rfloor]$ (zero-indexed).
          If $m = 0$ (no plans accepted within $T$ steps), return the METIS
          initialization $\pi_0$ as a fallback. This fallback is part of the
          algorithmic contract: a finite local chain may fail to accept a
          proposal even though the large-arity full-plan failure mode has been
          avoided.
  \end{enumerate}
\end{definition}
...
In the atlas framing, a BisectionEnsemble node is a local candidate generator.
Its candidates are accepted ReCom bisections of the current node region, each
with a proposal index, sampled-tree identifier or hash, accepted/rejected
status, population deviation, and edge cut.
The decision rule is percentile selection over accepted local bisections; if no
candidate is accepted, the consequence records the METIS initialization
fallback rather than silently pretending an ensemble selection occurred.
The output carried forward is the selected bisection and its two child
workloads, while the local proposal log is the evidence boundary separating a
node-local ensemble claim from a full-plan ReCom ensemble claim.
...
\begin{proposition}[Large-arity failure avoidance]
  \label{thm:tractable}
  At every node where BisectionEnsemble is applied, the ReCom sampler is asked
  for a two-way bisection of the current node subgraph.
  The method therefore avoids the large-arity full-plan split that can stall a
  direct $k$-way ensemble. This is not a proof of rapid mixing or guaranteed
  finite-$T$ acceptance for every local subgraph.
\end{proposition}

\begin{proof}
  Algorithm~\ref{alg:bisection-ensemble} routes only $p'=2$ tree nodes to the
  local ensemble primitive. The primitive in Definition~\ref{def:be-primitive}
  always samples a bisection of the current node region $H$, so the ReCom
  sampler never has to construct a $p'$-way split with $p'>2$.
  Nodes with $p'>2$ remain outside this guarantee and continue to use METIS
  directly. A difficult local geometry can still lower acceptance, which is why
  the primitive records accepted/rejected proposals and falls back to the METIS
  initialisation when $m=0$.
\end{proof}
