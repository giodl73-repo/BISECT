...
**Track**: U-search-optimization
**Module**: track-H
**Papers reviewed**: U.8 (PercentileSweep), U.9 (BisectionEnsemble), U.10 (redist-ensemble), U.11 (resolution-aware)
...
|------|-----------|--------------------------|
| MCMC-A | Redistricting MCMC, ReCom, mixing | U.9, U.10, U.11 |
| MCMC-B | Chain diagnostics, convergence, SMC | U.10, U.11 |
| SYS-A | Rust performance, parallel computation | U.10 |
| SYS-B | HPC, ensemble scale, benchmark methodology | U.10, U.11 |
| POLSCI | Political science methodology, statutory use of algorithms | U.8, U.9 |
| STATS | Calibration, chain diagnostics, SMC | U.8, U.9, U.10 |
| PRACT | Real-world usability, litigation readiness | U.8, U.9, U.10 |
...
problem: legal posture (U.8), structural integration with bisection (U.9), computational scale
(U.10), and data resolution (U.11).

The module arc is coherent: U.8 defines what it means to "choose" a plan from a family; U.9 embeds
that choice locally inside the bisection tree; U.10 provides the Rust engine that makes it practical;
U.11 adds a new dimension of choice (resolution). Together they implement the `--search` flag
ecosystem in the `redist` binary.

**Module score estimate**: 3.35/4 (accepted). Not yet at 3.75+ threshold for board-level
distinction. The gap is reproducibility (U.9, U.10) and the absence of peer review files for U.11.
...
### U.8: PercentileSweep — Statutory Choice of Legal Posture
...
PA) and two extrapolated states (TX, CA) from T.4 single-run data. The extrapolation is clearly
labeled in R2 (abstract states "TX and CA show at most 0.5 seats variation (extrapolated from T.4
...
The T=601 sensitivity check (K-P1-C, Karypis) is appropriately deferred to U.10. The forward
...
| W10 | P1 | Structural-bias challenge route paragraph + B.0/U.2 cross-ref in §5 | Stephanopoulos |
...
| W16 | P2 | Add forward reference to U.10 for T=601 sensitivity check | Karypis, Liang |
...
### U.9: BisectionEnsemble — Local ReCom at Each Bisection Node
...
#### Cross-Track Dependency Assessment (T.4 / PP1 Question)

U.9's relationship to T.4 (ApportionRegions) is correctly handled. The related-work section (§2) explicitly cites both B.1 and T.4, describes the prime-factorization-guided k-way splits, and correctly explains BisectionEnsemble's compatibility with ApportionRegions: it applies at 2-way nodes; p-way nodes with p>2 continue to use METIS. The introduction cites ApportionRegions and GeoSection in the opening paragraph.

**Finding**: U.9 does not reinvent T.4 without acknowledgment. No PP1 flag on the T.4 cross-track dependency.
...
failure for large prime k while preserving the bisection tree guarantees from B.1/T.4. The causal
...
acknowledgment paragraphs, one hardware spec sentence. The T.4 cross-track dependency is properly
handled; no PP1 reinvention flag required. Target score after R3: 3.3–3.5/4.

---

### U.10: redist-ensemble — High-Performance Rust ReCom
...
### U.11: Resolution-Aware Redistricting
...
#### Missing Infrastructure (U.11)

U.11 is the only paper in Track H with no reviews/ directory and no REVISION-PLAN.md. The 3.8/4
...
### U.9 → T.4 (ApportionRegions): CLEAR

U.9 explicitly cites T.4 in §2, describes the prime-factorization-guided k-way splits, and
correctly scopes BisectionEnsemble's coverage (2-way nodes only; p-way nodes with p>2 use METIS).
No PP1 flag required.

### U.8 → G.1 (ensemble baseline): OPEN

U.8's adversarial bar argument depends on G.1 ensemble percentiles that are point estimates without
confidence bounds. D-P1-C (Duchin) is an open P1 item: U.8 cannot be submitted to Political
...
### U.10 → G.1 (stationarity validation): DEFERRED

U.10's pair reselection stationarity conjecture is to be settled empirically by comparing
...
1. **Coherent arc**: U.8 through U.11 form a clean research program. Each paper adds one orthogonal dimension to the search problem (posture, structure, speed, resolution) without overlap.
2. **Litigation readiness**: U.8, U.9, and U.10 each contain explicit statutory disclosure frameworks, pre-commitment protocols, and audit log requirements.
3. **Implementation depth**: U.10 is the most implementation-detailed paper in the portfolio. The falsifiable Phase 2 benchmark commitment is the correct scientific posture.
4. **T.4 cross-track handled correctly**: U.9 builds on T.4 without reinventing it. No PP1 flag.

### Track Weaknesses

1. **Reproducibility gap across U.8–U.10**: All three reviewed papers have single-run or pre-validation empirical claims.
2. **U.11 review absence**: The highest-scoring paper in the track (3.8/4) has no review files. This is a track-level hygiene failure that must be remedied before board submission of Track H as a module.
3. **VRA integration absent across U.8 and U.9**: Both papers omit VRA as a legal posture dimension.
4. **TX/CA empirical coverage thin**: The two states with the most complex redistricting problems have the weakest empirical coverage in U.8 and U.9.

### Module PP1 Flags

| Flag | Paper | Description | Priority |
|------|-------|-------------|----------|
| PP1-H0-KARCHER | U.8 | Karcher analogy in §5.1 statute box presented as established precedent rather than advocacy-by-parallel | P1 — blocking for Political Analysis |
| PP1-H0-VRA | U.8 | VRA posture absent from three-posture taxonomy; GA and TX are covered jurisdictions | P1 — blocking for statutory use |
| PP1-H1-FACTUAL | U.9 | GA/PA stability claimed in §4.4 with no supporting data in the paper | P0 — must fix before any submission |
| PP1-H3-NOREVIEWS | U.11 | 3.8/4 score with no review files — score cannot be verified against reviewer text | Track-level — must fix before board |
...
|-------|-------|--------------|-------|---------------|
| U.8 | Political Analysis | 3.4/4 | R2 → R3 required | R3 clears 6 P1 items (all prose); target 3.7–3.8/4 |
| U.9 | ICML | 3.0/4 | R2 → R3 required | R3 fixes GA/PA factual error + 4 items; target 3.3–3.5/4 |
| U.10 | USENIX ATC | 3.2/4 | Accepted | Add R04 planarity sentence; then submit |
| U.11 | GIS | 3.8/4 (unverified) | Unknown | Generate review round; verify score; then submit |
| **Track avg** | | **3.35/4** | | |
