...
the T.4 cross-track dependency is correctly handled (no PP1 reinvention flag). The revision work
divides into three categories:

- **U.10**: One sentence to add (R04 planarity). Ready for USENIX submission after that fix.
- **U.8 and U.9**: Short prose-only R3 revisions. U.8 has 6 P1 items (all legal/structural prose); U.9 has 1 must-fix factual error plus 4 should-fix items. No new experiments required for either.
- **U.11**: Infrastructure gap — review round must be generated before any board submission.

Total effort to bring the track to board-ready: approximately 2–3 focused writing sessions for U.8 R3, one short revision pass for U.9, one sentence for U.10, and one review generation session for U.11.

---

## U.10: redist-ensemble — Action Required Before Submission
...
After this fix, U.10 is ready for USENIX ATC submission. No other changes required.

---

## U.9: BisectionEnsemble — R3 Revision
...
## U.8: PercentileSweep — R3 Revision
...
> A separate challenge route argues that structural bias — if the bisection tree structure systematically favors one party across all seeds — makes percentile selection irrelevant: every percentile produces the same partisan outcome because the structure determines it. This track's response to structural-bias challenges is in B.0 and U.2: geographic determinism, not algorithmic bias, explains the observed partisan outcomes. See U.2 for the seat-variance decomposition attributing >90% of variance to structure vs. <10% to seed selection.

Add cross-references to B.0 and U.2 in §5.
**Effort**: 30 minutes.
...
- [ ] W10: Add structural-bias challenge route paragraph to §5 with B.0/U.2 cross-refs (P1)
...
- [ ] W16: Add forward reference to U.10 for T=601 sensitivity check (P2)

**Target score after R3**: 3.7–3.8/4.

---

## U.11: Resolution-Aware — Review Round Generation Required
...
U.11 has no `reviews/` directory. Before board submission of Track H as a module, the following must be created:

```
research/U.11+resolution-aware/reviews/
...
The review round should be generated using the panel's U.11 assessment in REVIEW_PANEL.md as the basis for the reviewer perspectives.

#### INFRA-H3-02: Create _panel.yaml

Create `research/U.11+resolution-aware/_panel.yaml` with at minimum:

```yaml
paper: U.11+resolution-aware
...
Once r1_*.md files are generated, create `research/U.11+resolution-aware/REVISION-PLAN.md` following the format of U.8 and U.10 REVISION-PLAN.md files.

### Paper-Level Items to Address in U.11 R1/R2
...
|------|--------|-------|---------------------|
| 1 | Add R04 planarity sentence to §3.2 | U.10 | None — do immediately |
| 2 | Submit U.10 to USENIX ATC | U.10 | Step 1 |
| 3 | Complete R3 revision (6 P1 items) | U.8 | None — prose only |
| 4 | Complete R3 revision (F-1 through F-5) | U.9 | None — prose only |
| 5 | Generate U.11 review round (r1_*.md files) | U.11 | None — can run in parallel with 3/4 |
| 6 | Submit U.8 to Political Analysis | U.8 | Step 3 |
| 7 | Submit U.9 to ICML | U.9 | Step 4 |
| 8 | Complete U.11 R1 cycle (REVISION-PLAN.md) | U.11 | Step 5 |
| 9 | Submit U.11 to GIS | U.11 | Step 8 |
...
|----|-------|-------------|
| PH2-H0 | U.8 | Actual PercentileSweep runs on TX and CA (currently T.4 extrapolations) |
| PH2-H0 | U.8 | T=601 sensitivity check for NC and GA |
| PH2-H1 | U.9 | Multi-run statistics for Tables 2–4 (10 independent runs per state/p) |
| PH2-H1 | U.9 | MultiSeedMETIS(N=100) baseline comparison |
| PH2-H2 | U.10 | criterion.rs benchmarks to confirm 50,000 steps/sec estimate |
| PH2-H2 | U.10 | Stationarity of pair reselection: G-track empirical comparison |
| PH2-H3 | U.11 | Multi-run autocorrelation validation for TX Option B (27% claim) |
| PH2-H3 | U.11 | BG-level (Option A) Polsby-Popper improvement (3–8% projection) |
| PH2-H3 | U.11 | Forest ReCom exact MH correction at county level |
