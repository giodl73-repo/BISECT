...
\textbf{Note:} this test assumes independent pairing events, which is an approximation
since pairings within the same redistricting plan are correlated. If incumbents $A$ and
$B$ are placed in the same district, this constrains the placement of incumbents $C$ and
$D$ (they cannot also be placed in that same district), so pair events share dependence
through the district-capacity constraint. The reported $p$-values are therefore
anti-conservative (they understate the true $p$-value under exact permutation). Exact
permutation tests over the space of valid redistricting plans are a Phase~2 extension.
...
\paragraph{Florida (deferred).}
Florida ($k=28$) was planned as a fourth state in this analysis; FEC address geocoding
for FL incumbents is deferred to Phase~2 pending data availability for the full set
of 2022 FL congressional candidates.
