...
We set $\alpha = 0.40$ and $\beta = 0.50$ based on values approximated from the
empirical literature on redistricting-induced retirement~\citep{ansolabehere2000incumbency};
these are not estimated from \textsc{bisect}-specific data.
The model is intended to illustrate the directional effect of algorithmic vs.\ enacted
redistricting on retirement pressure, not to produce precise predictions.
Estimation of $\alpha$ and $\beta$ from \textsc{bisect} redistricting outcomes across
multiple cycles is a Phase~2 extension.
