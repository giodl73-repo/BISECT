...
All Short-Burst results in Table~\ref{tab:comparison} use base seed
$s = 42$; METIS results use the same seed for METIS initialisation;
\cs{} uses the content-derived seed $s_0$ per U.1~\citep{b16paper}.
We use 2020 presidential two-party precinct returns
(VEST Harvard Dataverse \citep{vest2020}) as the partisan metric;
composite partisan indices would be preferable for legislative purposes
but are not available at census-tract resolution for all three states.
...
\item \textbf{\cs{}}: single \recom{} chain, $T = 1{,}000$ steps,
      ...
      (Note: the standard U.1 threshold is $T = 600$; we use $T = 1{,}000$
...
The baseline single-seed METIS Polsby-Popper scores (from T.4~\citep{b11paper})
and \be{} results (from U.9~\citep{h1paper}) are:
\begin{itemize}
...
\caption{Polsby-Popper ($\pp$) and improvement under four methods.
         ...
         Base seed $s = 42$ for all Short-Burst runs.}
\label{tab:comparison}
\begin{tabular}{lrrrrrr}
\toprule
Method & NC & \%$\uparrow$ & WI & \%$\uparrow$ & TX & \%$\uparrow$ \\
\midrule
Multi-seed METIS           & 0.237 & +9.2  & 0.214 & +8.1  & 0.202 & +9.8  \\
\cs{} ($T=1000$)           & 0.243 & +12.0 & 0.220 & +11.1 & 0.208 & +13.0 \\
\shb{} ($\ell=20$, $m=50$) & 0.254 & +17.1 & 0.231 & +16.7 & 0.221 & +20.1 \\
\be{} ($T=100$)            & 0.261 & +20.3 & 0.228 & +15.2 & 0.231 & +25.5 \\
\bottomrule
\end{tabular}
\end{table}
...
of the initial METIS plan.
In these three states under prime-factor bisection structure, the
selection mechanism does not alter partisan seat counts --- consistent
with U.2's finding that structure dominates search, though this is an
empirical observation on single runs, not a general theorem.

We also report the efficiency gap (EG)~\citep{stephanopoulos2015} to
characterise partisan bias beyond seat counts.
For all four methods, $|\mathrm{EG}| < 0.03$ on NC and WI; TX is
$|\mathrm{EG}| < 0.05$, reflecting its larger delegation and
geographic asymmetry.
No method systematically favours either party: optimising PP compactness
does not introduce detectable partisan drift across these states,
confirming that \shb{}'s compactness gains are partisan-neutral.
