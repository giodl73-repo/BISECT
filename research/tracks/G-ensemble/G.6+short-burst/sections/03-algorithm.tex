...
The repository smoke package \texttt{G.6-G.12+short-burst-smoke} verifies this
seed formula and the endpoint-retention selection rule on a synthetic endpoint
ledger.  It does not by itself establish a production command path or state
empirical result.
...
\[
  D(s, i) \;=\;
    \underbrace{\texttt{"SHORT\_BURST\_CHAIN\_"}}_{\text{19 bytes}}
    \;\|\;
...
The prefix \texttt{"SHORT\_BURST\_CHAIN\_"} serves as domain separator,
...
\subsection{Intended CLI Invocation}
...
This invocation is the target surface for a production short-burst command.
Until that surface is checked in with run packages, cite the smoke package only
for mechanics replay.
...
\begin{proposition}[Stochastic dominance --- proof sketch]
\label{thm:dominance}
...
The following is a heuristic argument; a formal proof requires bounding
the drift parameter $\Delta$, which we defer to future work.
...
the current state.
To be precise: both the endpoint and the within-burst minimum are valid
Markov chain states. The correctness issue is that \emph{selecting} the
minimum introduces a non-Markovian selection rule on the sequence of
restart points --- the selection depends on the full path, not just the
current state --- which breaks the sequential chain structure that gives
Short-Burst its distributional properties.
