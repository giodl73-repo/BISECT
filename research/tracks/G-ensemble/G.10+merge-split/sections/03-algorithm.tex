...
\begin{remark}[Bounding the pair-count assumption]
The equality $|\mathcal{P}(\pi)| = |\mathcal{P}(\pi')|$ holds when both plans
have the same number of adjacent district pairs.
In practice $|\mathcal{P}(\pi)|$ and $|\mathcal{P}(\pi')|$ can differ by at
most~2 (one pair is added or removed per step), introducing a correction factor
of at most $(|\mathcal{P}(\pi)| \pm 2) / |\mathcal{P}(\pi)|$.
For NC ($k=14$, typically $|\mathcal{P}| \approx 18$--22 pairs), this factor is
bounded within~10\% of~1, which we treat as negligible.
\end{remark}
