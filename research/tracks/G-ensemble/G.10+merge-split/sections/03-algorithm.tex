...
\paragraph{Algorithm 1. \mergesplit{} Chain.}
\begin{algorithmic}[1]
\Require Initial plan $\pi_0$, steps $S$, base seed $b$, edge-cut percentile $p$
\Ensure  Selected accepted/initial plan $\pi^*$ at EC percentile $p$
\State $\pi \gets \pi_0$
\State $\mathrm{accepted} \gets [(\ec(\pi_0),\,0,\,\pi_0)]$
\For{$t \gets 0$ \textbf{to} $S-1$}
  ...
  \State $\mathrm{pairs} \gets \mathrm{adjacent\_district\_pairs}(\pi)$
  \State $\mathrm{attempts} \gets \min(|\mathrm{pairs}|,\max(k,\min(50,|\mathrm{pairs}|)))$
  \For{$a \gets 1$ \textbf{to} $\mathrm{attempts}$}
    \State $(d_i, d_j) \gets$ next shuffled adjacent pair from $\mathrm{pairs}$
    ...
    \Else
      \State $\alpha \gets \mathrm{cuts\_fwd} / \mathrm{cuts\_rev}$
           \Comment{two-tree stochastic correction}
    \EndIf
    \If{$\mathrm{rng}_{\mathrm{fwd}}.\mathrm{sample}(\mathrm{Uniform}[0,1]) < \min(1, \alpha)$}
      \State $\pi \gets \pi'$
      \State append $(\ec(\pi'),\,t+1,\,\pi')$ to $\mathrm{accepted}$
    \EndIf
    \State \textbf{break}
  \EndFor
\EndFor
\State sort $\mathrm{accepted}$ by $(\ec,\mathrm{step})$ ascending
\State $r \gets \min(\lfloor p \cdot |\mathrm{accepted}|\rfloor,\;|\mathrm{accepted}|-1)$
\State \Return $\mathrm{accepted}[r].\mathrm{plan}$
\end{algorithmic}

\paragraph{Design choices.}
Three aspects of this specification deserve comment.
...
Third, the CLI wrapper includes the initial METIS/\cs{} plan in the candidate
list, appends only accepted states, sorts candidates by edge cut with the step
index as a deterministic tie-breaker, and returns the requested percentile.
Thus \texttt{--percentile 0.0} selects the minimum-edge-cut visited candidate,
while \texttt{--percentile 0.5} selects the median accepted/initial candidate by
edge cut.  Full ensemble analysis should retain the complete visited sequence
and the per-step \texttt{StepRecord} fields
\texttt{accepted}, \texttt{valid\_cuts\_forward},
\texttt{valid\_cuts\_reverse}, and \texttt{ratio}.
...
under the uniform target distribution, after conditioning on the same adjacent
district pair and merged region, has the balanced-cut-count component:
\[
...
After dropping the district-pair-count term, or equivalently conditioning on
equal adjacent-pair counts, the implemented ratio is:
\[
  \frac{Q(\pi' \to \pi)}{Q(\pi \to \pi')}
  = \frac{|\mathcal{C}(T_{\mathrm{fwd}}, R)|}{|\mathcal{C}(T_{\mathrm{rev}}, R)|}.
\]
The implementation correctly uses
$\mathrm{cuts\_fwd} / \mathrm{cuts\_rev}$ for this component. \qed
\end{proof}

\begin{remark}[Pair-count correction boundary]
\normalfont
Let $\mathcal{P}(\pi)$ denote the set of adjacent district pairs available from
plan $\pi$. The full proposal ratio also contains the district-pair term
\[
  \frac{|\mathcal{P}(\pi)|}{|\mathcal{P}(\pi')|},
\]
The current \texttt{MergeSplitChain} does not multiply by this factor.
That omission is usually modest when the district-adjacency graph is dense and
pair counts change little, but it is not a theorem-level zero term.
This is the main reason the implementation is described as a two-tree
stochastic correction rather than an exact uniform sampler.
\end{remark}
...
\subsection{Stochastic Balance Claim Boundary}

\begin{proposition}[Implemented stochastic correction]
\label{thm:approx-balance}
For each proposed move that reaches a forward balanced cut, the implemented
\mergesplit{} step applies a reproducible independent reverse-tree correction
to the balanced-cut-count portion of the proposal ratio:
\[
  \alpha_{\mathrm{impl}} =
  \begin{cases}
    0, & \mathrm{cuts\_rev}=0,\\
    \min(1,\mathrm{cuts\_fwd}/\mathrm{cuts\_rev}), & \mathrm{cuts\_rev}>0.
  \end{cases}
\]
This makes the main reversible-ReCom asymmetry observable and auditable, but it
does not by itself prove exact detailed balance for the finite implemented
chain.
\end{proposition}

\begin{proof}[Proof sketch]
Wilson's algorithm samples both $T_{\mathrm{fwd}}$ and $T_{\mathrm{rev}}$
from the uniform spanning-tree distribution on the same merged region $G[R]$,
using independent seed streams.  Conditional on the merged region, the only
tree-dependent cut-selection probabilities in the forward and reverse
directions are inversely proportional to their balanced-cut counts.  Therefore
the ratio $\mathrm{cuts\_fwd}/\mathrm{cuts\_rev}$ is the implemented correction
for that tree-dependent term.  The proof is deliberately limited to the
implemented contract: determinant corrections, pair-count corrections, and
global mixing diagnostics must be supplied separately when a stronger target
claim is needed.
\end{proof}
