...
Table~\ref{tab:resolution-options} summarises the three options. In the current
CLI, Option B uses the standard tract adjacency plus GEOIDs; Options A and C
are exercised only when block-group adjacency and block-group GEOIDs are
...
\midrule
A & Block group & Tract  & 11 digits & Runner path with BG data \\
B & Tract       & County & 5 digits  & Default CLI path \\
C & Block group & County & 5 digits & Runner path with BG data \\
\bottomrule
\end{tabular}
\end{table}
...
This option provides a finer spatial resolution than tracts and is useful for
states with very heterogeneous tract populations.
The runner supports this shape when block-group adjacency data are supplied;
without that data the CLI returns an instructional configuration error.
...
This is the default CLI option and the focus of this paper.
Option B requires GEOIDs but no block-group adjacency: county adjacency is
derived from the existing tract graph as described in Section~\ref{sec:algorithm}.
...
It is available through the same runner path as Option A when block-group
adjacency data are supplied.
...
The \texttt{rebalance} subroutine is shared across all three options:
it operates on the fine plan $\pi : V \to [k]$ and the fine adjacency graph
$G$, regardless of the coarse resolution.
The current CLI wrapper uses $\epsilon_c = 3\epsilon_f$ for the coarse chain.
...
\midrule
A & Block group & Derived from fine & Yes \\
B & Tract (loaded) & Derived from tract & No \\
C & Block group & Derived from fine & Yes \\
\bottomrule
\end{tabular}
\end{table}

Option B is the only option that requires no block-group adjacency beyond what
is already loaded by the standard redistricting pipeline, but it still requires
the tract GEOID map.
