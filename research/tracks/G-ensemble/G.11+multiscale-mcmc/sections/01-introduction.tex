...
This slow mixing is not a proof of non-convergence; it is a practical
obstacle.
...
Multi-scale MCMC~\citep{autry2021} addresses the large-$k$ mixing problem
...
\begin{enumerate}
  ...
  can be derived reproducibly from the existing tract adjacency graph using GEOID
  prefix matching.
  No extra data files, shapefiles, or census downloads are required.
  Section~\ref{sec:algorithm} gives the derivation and its tract-graph boundary
  ...
  BISECT as \texttt{bisect state --search multiscale}. The current
  orchestration is in \texttt{bisect-cli}'s \texttt{run\_multiscale}; the
  \texttt{bisect-multiscale} crate supplies seed, hierarchy, configuration,
  and rebalance helpers, while its standalone chain API remains a substrate
  ...
  We report a short-run autocorrelation comparison at lag 100 between standard \recom{} and
  \ms{} (Option B, $\alpha = 0.3$) on Texas ($k = 38$, $T = 2000$ steps).
  Section~\ref{sec:mixing} reports the results; the measured reduction is
  $43\%$ in lag-100 autocorrelation at $T = 2000$ steps (16\% at $T = 1000$
  steps per G.15 cross-comparison), at comparable wall-clock time. These
  figures are implementation smoke evidence, not a general mixing-time proof.
...
fine-level chain with a stronger reversible-ReCom target model. Replacing
standard ReCom with Forest ReCom inside the multi-scale wrapper is a plausible
future extension, but the coarse projection/rebalance step would still need its
