...
For small $k$ (say $k = 8$ or $k = 14$), each step affects a larger
...
The coarsening $f$ induces a \emph{coarse plan}: given a fine plan
$\pi : V \to [k]$, the coarse wrapper must choose one district label for each
coarse unit $c$.  A population-plurality rule is the natural model-level
definition, but the current \texttt{run\_multiscale} wrapper uses a simpler
fine-to-coarse sync pass over the \texttt{fine\_to\_coarse} mapping.  That
implementation detail is harmless for the synthetic tests but should be
treated as part of the current audit boundary rather than as a polished
standalone chain API.
