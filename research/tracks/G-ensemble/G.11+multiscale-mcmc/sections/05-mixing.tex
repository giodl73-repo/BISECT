...
We report a short-run comparison between standard \recom{} and \ms{} (Option B,
$\alpha = 0.3$) on
...
Both chains start from the same initial plan (the low-EC plan found by
\cs{} with $T = 100$ seeds in the recorded experiment).
...
Values are reported as means over $n_{\text{runs}}=5$ independent chains
($s \in \{42,\ldots,46\}$, $T=2{,}000$ steps). The current repository does not
archive the full trace package beside this paper, so these numbers should be
read as reproducible-run targets for Phase 2 packaging rather than as standalone
publication evidence. Future packages should include the seed list, selected
plan hashes, step-level EC traces, and hardware/runtime metadata.

At lag 100, \ms{} reduces autocorrelation from $0.51$ to $0.29$, a $43\%$
reduction.
Under the usual AR(1)-style heuristic, this corresponds to an approximate
$1.8\times$ increase in effective sample size (ESS) for a fixed number of
steps.
...
\begin{tabular}{@{}p{0.24\linewidth}p{0.14\linewidth}p{0.15\linewidth}p{0.23\linewidth}p{0.12\linewidth}@{}}
\toprule
Method
...
\noindent\emph{Runtime note.}
Runtimes were recorded on an Intel Core i7-11800H (2.3 GHz, 24 MB L3 cache,
Windows 11) using a release build. Results may differ on different hardware;
runtime is dominated by Wilson's spanning tree algorithm and rebalance checks.

In the recorded run, the coarse acceptance rate ($58\%$) is high because the county graph is
...
{\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}p{0.23\linewidth}p{0.16\linewidth}p{0.15\linewidth}p{0.17\linewidth}p{0.16\linewidth}@{}}
\toprule
Method
  & Mixing (lag 100)
  & ESS/1000 steps
...
\ms{} is the strongest large-$k$ result in this local short-run comparison
among methods with comparable per-step cost.
...
\paragraph{Comparison to Autry et al.\ speedup.}
Our approximate 1.8$\times$ ESS gain (for TX $k=38$, Option B) is lower than
Autry et al.'s 3$\times$ speedup (for NC $k=13$, three-level hierarchy). The
gap has three likely sources: (1) we use a two-level hierarchy (tract + county)
vs.\ Autry et al.'s three levels; (2) TX $k=38$ has more districts than NC
$k=13$, making each coarse step smaller relative to the total restructuring
needed; and (3) the summary statistics differ. A direct apples-to-apples
comparison requires an archived benchmark package and is deferred to Phase 2.
