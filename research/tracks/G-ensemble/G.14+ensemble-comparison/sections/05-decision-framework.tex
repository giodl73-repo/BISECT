...
\paragraph{Step~0: Is a single deterministic plan required?}
Before entering the four-question tree, practitioners should consider
whether the task requires a single, reproducible, and certifiably optimal
plan rather than an ensemble distribution.
If so, \textsc{ConvergenceSweep} ($T = 600$, \texttt{--search convergence})~%
\footnote{The T=600 ConvergenceSweep formula is certified in U.1~\cite{deluca2026convergence} (Round~2, accepted; mean reviewer score 3.67/4). The formula's theoretical foundations and 89-seed empirical margin above Georgia's certified tail are documented in that paper.}
is the appropriate tool: it produces the minimum-edge-cut plan deterministically
from the SHA-256 content-derived seed and is not an ensemble method.
The four questions below apply only when an ensemble distribution is required.
...
\begin{enumerate}[label=\textbf{Q\arabic*.}]
  ...
  \item \textbf{Is VRA compliance required?}
    ...
    \begin{quote}\small
    \textit{Note:} \texttt{--search vra-recom} requires a VRA-compliant
    initial plan.
    Use \texttt{--structure ratio-optimal-vra} (VRASection, T.7) to generate
    this plan first.
    A non-compliant initial plan will trigger the empty protected-set warning
    and produce an unconstrained chain.
    \end{quote}
  ...
  \item \textbf{Is $k \geq 15$?}
    ...
    For medium-to-large $k$ states ($k \geq 15$, including TX $k=38$,
    FL $k=28$, CA $k=52$, NY $k=26$, PA $k=17$, and OH $k=15$),
    ...
    For medium-$k$ states ($k \geq 15$, including PA $k=17$ and OH $k=15$),
    Multi-scale provides measurable mixing improvement if the coarse
    acceptance rate exceeds 20\%.
    For smaller $k < 15$, single-scale Forest ReCom or Merge-Split is
    sufficient.
    \emph{Proceed to: \ms{} (G.11). Stop.}

    \begin{quote}\small
    For states with VRA requirements \emph{and} large chambers (e.g.,
    TX $k=38$ with 7 protected districts), use \texttt{--search vra-recom}
    with an increased step count ($\geq 2\times$ the Multiscale-recommended
    count) to compensate for VRA rejection overhead.
    \end{quote}
  ...
  \item \textbf{Is a calibrated target distribution required?}
    ...
    \begin{itemize}
      \item For \emph{exact} calibration (importance-weighted particles,
            user-specified target): use \smc{} (G.7).\footnote{%
              The SMC recommendation is contingent on G.7 Phase~2 completion
              (SmcPercentile compositor, validation against R
              \texttt{redist::redist\_smc()}).
              Until Phase~2 is complete, \texttt{--search merge-split} (G.10)
              is the recommended fallback for calibrated sampling requiring a
              documented stationary distribution.}
...
%% Q2
\node[decision, below=1.6cm of q1] (q2) {$k \geq 15$?};
...
For example:
\begin{itemize}
  \item \texttt{--structure prime-factor --weights-override county --search multiscale}:
        ApportionRegions initial plan + county-respecting weights +
        Multi-scale mixing.
        Recommended for TX or CA litigation.
  \item \texttt{--structure ratio-optimal --weights-override geographic --search vra-recom}:
        GeoSection initial plan + neutral weights + VRA-constrained chain.
        Recommended for VRA Section~2 analysis.
  \item \texttt{--structure ratio-optimal --weights-override geographic --search short-burst}:
...
CVD (T.10) and BFS Growth (T.12) will add geometric algorithms to the
comparison.
