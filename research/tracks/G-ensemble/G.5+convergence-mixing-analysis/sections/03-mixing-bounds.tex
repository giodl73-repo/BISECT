...
\begin{remark}
The conjectured tighter bound $O(n^2 \log n)$ would require path congestion
at $O(n/\log n)$ rather than $O(n)$; we leave this as an open problem.
Concretely, this would require a direct path congestion argument showing that
the congestion of the canonical paths is $O(n/\log n)$ rather than $O(n)$.
This requires path congestion analysis beyond the scope of this paper;
see \citet[Theorem~13.1]{levin2009markov} for the relevant machinery.
Reducing the exponent from 3 to 2 does not affect the empirical
