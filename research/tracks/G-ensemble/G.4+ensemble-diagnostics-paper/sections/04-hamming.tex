...
\]
In all computations reported here, $\pi_0$ is the chain's initial random
spanning-tree plan. This makes the diagnostic reproducible and avoids using
the enacted plan or an optimized plan as the reference. The statistic should
be interpreted after an initial burn-in interval; before burn-in, the reference
choice can dominate the apparent autocorrelation.
...
chain mixing.
The $\ham(1) < 0.95$ rule is therefore a motion screen: it catches nearly
frozen chains, but it is not the main convergence guarantee. The operative
criteria remain multi-chain $\rhat$ and metric-level $\ess$.
