...
**Latest pass:** 2026-05-12 paper-quality and implementation-parity pass.
...
**Status 2026-05-12**: Addressed. G.4 now states 1.1 as the publication
threshold and explains that stricter validator defaults can be overridden with
`--rhat-threshold 1.1`.
...
**Status 2026-05-12**: Addressed. Section 4 now specifies the initial
spanning-tree plan reference and frames Hamming as a motion screen rather than
...
**Status 2026-05-12**: Addressed. The table and implementation JSON example now
use NC $\ess=695$ for $\rho_1=0.87$ at 10,000 steps.
...
**Status 2026-05-12**: Addressed. The statutory section now treats the six-state
table as authoritative, gives 25,000 for Texas and 50,000 for California, and
labels $n_{\rm min}(k)=\max(10{,}000,1{,}000k)$ as a conservative pre-run
budget heuristic for unstudied large states.
...
**Status 2026-05-12**: Addressed in Section 5 as source-ensemble certification
patterns. The paper now distinguishes ESS/Hamming partial certification from
multi-chain R-hat certification.
...
**Status 2026-05-12**: Addressed. Section 3 now adds the 99th-percentile ESS
minimum and requires longer chains or an explicit uncertainty statement for
1st/99th percentile claims.
...
**Status 2026-05-12**: Addressed. Section 7 now separates nominal tail false
positive rates from finite-chain ESS uncertainty.
...
**Status 2026-05-12**: Addressed. Section 2 now has a single-chain ensembles
subsection and recommends five chains for publication.
...
**Status 2026-05-12**: Addressed in Section 4.
...
**Status 2026-05-12**: Addressed. Section 3 states 500 as an operational screen
and keeps the 526 derivation visible.

## 2026-05-12 Build Check

- [x] Rebuilt with `make`.
- [x] No undefined references or citations.
- [x] No overfull boxes.
- [x] BibTeX reports `warning$ -- 0`.
- [x] PDF copied to `docs/papers/`.
