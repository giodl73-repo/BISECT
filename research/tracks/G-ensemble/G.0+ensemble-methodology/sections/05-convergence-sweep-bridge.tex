...
ceiling, and 3~GiB retained and scratch ceilings.  The first separately frozen
NH/NM/GA expansion completed five of six primary State--kernel runs before GA
Kruskal encountered host disk exhaustion; its stop rule closed the package
without retry or governed replay.  A v2 successor added actual-volume capacity
admission but closed before its first preflight because the compiled runner
rejected the successor identity and seed.  Both failures remain negative
resource and implementation-integration evidence.

A fresh v3 protocol used a new execution identity, base seed, ledger, and
package.  It passed all six excluded preflights and their normalized replays,
then completed all six governed four-chain primaries and regenerated every one
exactly in a fresh governed replay.  After the frozen 500-step burn-in, New
Hampshire and New Mexico passed split-$\rhat<1.05$ and pooled $\ess\geq100$ for
both kernels.  Georgia did not: Wilson cut fraction had split-$\rhat=1.04656$
but pooled $\ess=66.47$, while Kruskal weighted boundary cut had
split-$\rhat=1.07507$ and pooled $\ess=53.88$.  The registered all-or-nothing
decision is therefore failure, and v3 is closed without retry or extension.
Exact replay establishes deterministic execution, but it cannot override the
failed convergence gate or establish mixing and sampler equivalence.
