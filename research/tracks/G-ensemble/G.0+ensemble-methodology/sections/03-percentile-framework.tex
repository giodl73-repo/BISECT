...
metric output, and any election-input model used for partisan seats.  The current
repository records two complementary fixtures:
\begin{itemize}
  \item \texttt{docs/examples/g-ensemble-evidence-packages/G.1-G.3+active-synthetic/}
    is an active synthetic package that exercises every required artifact role.
  \item \texttt{docs/examples/g-ensemble-evidence-packages/G.1-G.3+missing-evidence/}
    records the remaining real external-trace, election, metric, diagnostic,
    RPLAN, and RCTX gaps for headline G.1--G.3 empirical percentiles.
\end{itemize}

Thus, G.0 should be read as the methodology contract.  The synthetic package
shows that the contract is executable; real state percentile claims require real
packages with the same roles.
