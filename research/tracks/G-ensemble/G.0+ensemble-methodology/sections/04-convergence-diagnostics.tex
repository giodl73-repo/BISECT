...
An \ess{} of at least 400 per statistic is the minimum floor for reliable
95\% interval estimates; the companion G.4 paper establishes 500 as the
recommended target based on empirical diagnostics across six states.
\recom{} ensembles typically achieve higher \ess{} than
...
Diagnostic thresholds are not self-authenticating.  A package that cites
$\rhat$, \ess{}, or Hamming autocorrelation must include the trace or summary
from which those quantities were computed, the seed policy, the chain count, the
burn-in/thinning rule, and the statistic definitions.  In the package vocabulary
used by the repository, these are \texttt{external-trace} and
\texttt{diagnostic} artifacts.  Missing either artifact downgrades a percentile
statement from empirical finding to protocol or design claim.

The active real G.1--G.3 package validates this artifact shape for RI, IA, and
NC. It also demonstrates why the diagnostics are claim gates: Iowa and North
Carolina cannot support precise extreme-tail claims under the preregistered ESS
rules, and the Rust and GerryChain cut distributions disagree materially in
those states.
