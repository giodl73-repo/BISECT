...
In the atlas framing, the candidates are particle proposals at each stage.
A particle proposal carries a derived seed, a connected district proposal,
a log-weight update, and a survival status.
When the effective sample size falls below the threshold, the resampling event
must record which ancestors are copied forward; after resampling, a particle
slot is not automatically the descendant of its previous occupant.
These distinctions make the sampled ensemble auditable as a sequence of
proposed, killed, weighted, copied, and finalised objects rather than as a
bag of finished plans.
...
\subsection{Weighted Percentile Selection}
\label{subsec:smc-percentile}

The SMC sampler can also export one representative plan from the weighted
ensemble. This is not a claim that the selected plan is optimal; it is a
deterministic rule for selecting a percentile of the weighted SMC distribution.

\begin{algorithm}
\caption{\smc{} Percentile Selection}
\label{alg:smc-percentile}
\begin{algorithmic}[1]
\Require Weighted plans $(\pi_i,w_i)_{i=1}^N$, score function $g$, percentile $p \in [0,1]$
\Ensure One selected plan $\pi^*$
\State sort particles by $(g(\pi_i), i)$ in increasing order
\State $c \gets 0$
\For{$i$ in sorted order}
  ...
  \EndIf
\EndFor
\State \Return the final sorted particle
\end{algorithmic}
\end{algorithm}

The implementation exposes this rule as the \texttt{smc-percentile} search
mode and as SMC ensemble selection. The stable tie-breaker by particle index is
part of the audit contract: the same weighted ensemble, score function, and
percentile must select the same particle.
...
\begin{proposition}[First-order ESS approximation]
\label{prop:ess-degrade}
Let $X=\log|\mathcal{V}(T)|$ be the log valid-cut count for a random spanning
tree of a fixed graph, with mean $\mu$ and variance $\sigma^2$.
...
This approximation is a planning model, not a formal lower bound. The Phase~2
large-$k$ runs below are therefore reported as empirical evidence rather than
as proof of a universal ESS floor. The approximation drives the practical
particle-count scaling guidance:
\begin{center}
