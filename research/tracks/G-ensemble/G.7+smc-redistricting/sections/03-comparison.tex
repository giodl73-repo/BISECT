...
reference plan); and one --- \smc{} --- is an importance-weighted calibrated
sampler for a declared target distribution.
...
\small
\begin{tabular}{@{}p{2.1cm}p{2.6cm}p{1.45cm}p{2.25cm}p{3.25cm}@{}}
\toprule
...
ShortBurst, \be{}, \cs{}, and \flip{} are developed in the companion
algorithm papers cited throughout this track
\citep{cannon2022,g6paper,h1paper,b11paper,g8paper}.
...
plan so the weighted sample is consistent for the declared target distribution
under the proposal model (Proposition~\ref{prop:weight-correct}).
No mixing assumption is required; the correction is analytic, while Monte Carlo
error is governed by the number of particles and the final \ess{}.
...
\smc{} provides a calibrated alternative: for the same computational budget,
...
The operational distinction is visible in the artifacts each method must
publish.

\begin{table}[ht]
\centering
\caption{Diagnostic evidence expected from SMC and ReCom workflows.}
\label{tab:diagnostic-comparison}
\begin{tabular}{p{0.25\linewidth}p{0.31\linewidth}p{0.31\linewidth}}
\toprule
Question & SMC evidence & ReCom evidence \\
\midrule
Reproducibility & base seed, particle seeds, resample seeds, file hash & chain
seeds, proposal parameters, output hash \\
Calibration & importance weights, final ESS, resample map & stationary target
argument plus convergence diagnostics \\
Failure mode & all particles killed, low ESS, weight collapse & non-mixing,
high R-hat, low metric ESS \\
Selected plan & weighted percentile rank and particle id & rank within accepted
chain sample after burn-in \\
\bottomrule
\end{tabular}
\end{table}
...
For uniform-target inference, \smc{} is the preferred choice when calibrated
weights and sufficient \ess{} are available. ReCom remains useful when its
stationary target, burn-in, and convergence diagnostics are documented under
G.4. \smc{} at $N = 5{,}000$ provides an approximate inference with
