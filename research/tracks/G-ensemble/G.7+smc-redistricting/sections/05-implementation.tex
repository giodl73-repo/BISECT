...
The ESS after the final stage is approximately $4{,}200$ (42\% of $N$), consistent
with the theoretical prediction from Proposition~\ref{prop:ess-degrade}
($\ess \geq \exp(-k/10) \times N = \exp(-3.8) \times 10{,}000 \approx 2{,}232$;
the observed value exceeds the bound as expected).
...
The \texttt{bisect ensemble --method smc --select-percentile 0.5} command
is implemented and tested: it runs the full SMC algorithm, then selects the
particle at the weighted median of the final ensemble (rank
$\lfloor 0.5 \times \ess \rfloor$ in the importance-weighted order).
