...
Every \smc{} run can write NDJSON (newline-delimited JSON) to the output
stream.
...
\begin{enumerate}
  ...
  \item \textbf{Seed spot-check.}
        For a randomly chosen particle and stage, derive the particle seed
        from the recorded \texttt{base\_seed} using the SHA-256 schedule
        in Section~2.2. Confirm that this is the seed passed to
