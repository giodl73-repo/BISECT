...
\begin{enumerate}
  ...
  \item \textbf{Adaptive Multi-scale and Parallel Tempering are the
        top-mixing candidates for TX.}
        AMS and PT produce similar ACF(100) for TX (0.31 vs.\ 0.34),
        within the single-run uncertainty band; both substantially outperform
        single-scale chains (FR: 0.48).
        Whether AMS or PT is faster-mixing for TX is inconclusive from
        single-run estimates and requires multi-seed validation.
...
T.13 (in planning) will introduce a spectral bisection structure algorithm
...
\texttt{bisect} algorithm portfolio.
...
All 15 algorithms are implemented in the \texttt{bisect} Rust workspace:
...
\begin{itemize}
  ...
  \item \texttt{crates/redist-ilp/} --- ILP redistricting crate (U.6)
...
reference for algorithm selection in the \texttt{bisect} platform.
