...
\texttt{--weights-override county} across all search runs.
This follows the G.14 protocol (which used \texttt{--structure ratio-optimal}
and \texttt{--weights-override geographic}) updated to the ApportionRegions composite
...
Simulated Annealing  & Structure & \texttt{--structure simulated-annealing}  & U.3 \\
CVD Graph-Distance   & Structure & \texttt{--structure centroidal-voronoi}   & T.10 \\
CVD Geographic       & Structure & \texttt{--structure centroidal-voronoi --cvd-metric geographic} & T.11 \\
BFS Growth           & Structure & \texttt{--structure bfs-growth}          & T.12 \\
ILP Exact            & Structure & \texttt{--structure ilp}                 & U.6 \\
\midrule
ConvergenceSweep     & Search & \texttt{--search convergence}           & U.1 \\
PercentileSweep      & Search & \texttt{--search percentile}            & U.8 \\
BisectionEnsemble    & Search & \texttt{--search bisection-ensemble}    & U.9 \\
...
ShortBurstForest     & Search & \texttt{--search short-burst-forest}    & G.12 \\
VRA-Aware ReCom      & Search & \texttt{--search vra-recom}             & G.13 \\
Parallel Tempering   & Search & \texttt{--search parallel-tempering}    & U.4 \\
SMC-Percentile       & Search & \texttt{--search smc-percentile}        & G.7/new \\
Adaptive Multi-scale & Search & \texttt{--search adaptive-multiscale}   & U.5 \\
\bottomrule
\end{tabular}
\end{table}
