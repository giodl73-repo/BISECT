  ...
  \item \textbf{Is certified-optimal EC required?}
        ...
        \begin{itemize}
          \item If $N \leq 500$ per bisection half (applicable to NH $k = 2$
                and very small custom jurisdictions): use \ilp{} (U.6).
          \item If $N > 500$: ILP is not tractable. Use SA (U.3) for the
        ...
        Consult U.6~\S5 for the precise certificate scope.
  ...
  \item \textbf{Is $k \geq 15$?}
        ...
        \begin{itemize}
          \item If \textbf{compactness optimisation} (lowest EC) is the goal:
                use \pt{} (U.4). Requires multi-core hardware (8--16 cores).
                For single-core constraint: use \sburst{} (G.6).
          \item If \textbf{fast mixing} (low ACF, broad distributional
                coverage) is the goal: use \ams{} (U.5) for self-tuning, or
...
\node[result, right=2.0cm of q2, yshift=1.4cm] (cvdgeo)
  {\cvdgeo{} (T.11)\\\footnotesize Best PP};
\node[result, right=2.0cm of q2, yshift=-0.2cm] (bfs)
  {\bfsgrow{} (T.12)\\\footnotesize Fast, good PP};
...
\node[result, right=2.0cm of q4, yshift=1.4cm] (pt)
  {\pt{} / \ms{} (U.4/G.11)\\\footnotesize Large-$k$ mixing};
...
\caption{%
  ...
  All flags are for the \texttt{bisect} CLI.
}
\label{tab:combined}
\small
...
VRA Section~2 analysis
  & \texttt{--structure ratio-optimal}\newline
    \texttt{--weights-override vra-aligned}\newline
...
For production use, the \texttt{bisect} ILP implementation includes a timeout
