...
We generate multi-member districts using edge-weighted recursive bisection graph partitioning, implemented in the \texttt{bisect} open-source redistricting platform \citep{redistcli2026}. The algorithm represents census tracts as nodes in a graph, with edges weighted by shared boundary length. Graph partitioning minimizes total weighted edge cuts, equivalent to minimizing total district perimeter --- producing compact districts without explicitly optimizing for a compactness metric.
...
\end{itemize}
using the \texttt{bisect states --seats-per-district M --districts k} command with seed 42 for reproducibility.
...
\textbf{Sensitivity to ufactor formulation.}
An earlier implementation passed ufactor values as floating-point strings to METIS, which uses \texttt{atoi()} for parsing: the fractional part was silently truncated to integer 1 (0.1\% tolerance) regardless of the intended value. We verified that this implementation error does \emph{not} materially affect the reported results because, at continental scale ($k \geq 62$ districts), the per-node ufactor formula yields $T/k_{\text{root}} \leq 0.16\%$ --- which rounds to the same METIS integer value as the buggy implementation. The reported population deviations ($\pm$3.7--4.3\%) are dominated by the \emph{granularity floor}: census block groups average 1,400 people, and one block-group swap represents $\sim$0.04\% of a multi-member district. METIS cannot achieve arbitrarily precise balance below this floor regardless of ufactor. The corrected per-node implementation provides a strict mathematical guarantee for smaller district counts ($k < 50$) where the floor does not bind.
...
All code, data, and results are at \url{https://giodl73-repo.github.io/REDIST/}. The \texttt{bisect} binary (version 0.1.0) reproduces all configurations from reported seeds. Each configuration's manifest records the SHA-256 hash of source data and the exact command:
\begin{verbatim}
bisect state --year 2020 --version E1 \
