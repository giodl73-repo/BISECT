...
proportionality---map closely onto the \texttt{bisect} system's weights and
...
appointed by the court runs \texttt{bisect}, the output becomes the
...
Throughout, we use the \texttt{bisect} system~\citep{dellaLibera2026recursive}
as the reference implementation.  The commands cited in the workflow table
are drawn from the \texttt{bisect} CLI reference~\citep{redistCliQuickstart}.
