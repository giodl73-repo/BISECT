...
\begin{itemize}
    ...
    \item \textbf{METIS version}: 5.1.0 via \texttt{bisect} Rust binary (C METIS FFI via \texttt{metis} crate v0.2)
    \item \textbf{Computational environment}: Rust 1.77+ (\texttt{bisect} binary); post-hoc data analysis (CSV loading, plotting) uses Python 3.13 with \texttt{redist-analysis} crate (compactness, VRA, partisan metrics)
\end{itemize}
