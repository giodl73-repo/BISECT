...
For the national mean congressional algorithmic PP estimate
$\hat{PP} = 0.361$ (2020 census, from C.0), we use a conservative
asymmetric interval: the lower bound ignores the small positive bias
(conservative for reporting) and the upper bound includes it:
\begin{equation}
  95\%\ \text{CI for } PP^{\text{true}}:
  [\hat{PP} - 1.96 \times 0.003,\; \hat{PP} + 0.003 + 1.96 \times 0.003]
  = [0.355, 0.370].
\end{equation}

The lower bound ($0.355$) remains well above the enacted plan mean PP
($0.295$, from C.5), confirming that the algorithmic advantage is
robust to spatial resolution measurement error.
