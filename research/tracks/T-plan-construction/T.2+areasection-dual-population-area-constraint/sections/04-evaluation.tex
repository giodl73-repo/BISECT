...
The reference implementation is the \texttt{bisect} Rust binary using METIS 5.2.1
...
is not eliminated by removing the urban-peeling bisection strategy that
AreaSection is designed to address.
Even with geographic balance enforced at the first bisection level, the
competitive-state set still shows Republican over-representation on average.
...
This is the central empirical boundary of AreaSection's null hypothesis:
the area constraint changes the \emph{structure} of the bisection tree
(IL: 1:16 $\to$ 8:9; NY: 1:25 $\to$ 13:13), but usually leaves seat counts
unchanged.
North Carolina is the important exception in this competitive set, shifting
from 5D/9R under GeoSection to 6D/8R under AreaSection.
The geographic balance constraint is therefore not partisan-proof in every
state; the narrower result is that its effects are small, state-specific, and
not systematically aligned with either party.
