...
Illinois splits 1:16 at the first level (Chicago peel), not 8:9.
New York splits 1:25 (New York City peel), not 13:13.
Wisconsin splits 1:7 (Milwaukee peel), not 4:4.

GeoSection (T.1) adds isoperimetric normalisation to prevent the worst form
...
The area constraint cannot be satisfied by a 1:16 split in Illinois because
...
\item \textbf{Partisan comparison} (\S\ref{sec:evaluation}): head-to-head
  GeoSection vs. AreaSection on the 34 states with complete comparison data,
  showing that most states keep the same seat allocation while a small set,
  including North Carolina, changes by one seat.
\end{enumerate}

\paragraph{Scope and relationship to T.1.}
AreaSection is a companion paper to GeoSection (T.1).
