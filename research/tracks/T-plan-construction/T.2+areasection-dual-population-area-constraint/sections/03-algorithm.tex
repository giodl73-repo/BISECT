...
The normalisation is identical to GeoSection (T.1).
...
In the atlas framing, AreaSection adds a feasibility gate before GeoSection's
candidate split comparison.
Each ratio candidate first carries a Lorenz status, then a bundle of
dual-constraint METIS attempts, and finally a normalised score if at least one
attempt is usable.
The selected split is therefore not simply the shortest boundary: it is the
candidate that passed the population-area screen, survived the METIS
dual-constraint attempt, and won the normalised comparison.
The output should carry both the chosen recursive workloads and the rejected
ratio records, because the rejected records are the evidence for the claim that
area balance forced, or did not force, the top-level geography.
