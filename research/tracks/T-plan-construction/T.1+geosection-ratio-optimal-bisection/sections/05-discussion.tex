...
Pennsylvania ($k=17$) is an example: the ratio scan selects a $1:16$ root
allocation after comparing the observed normalised edge-cut for each feasible
ratio under the same seed budget.
...
it changes by $-1\,D$.
In 37 states the seat count is unchanged.
...
GeoSection confirms the $1:16$ Philadelphia peel: the observed normalised
score for the asymmetric root allocation wins the scan against the near-equal
alternatives.
...
\subsection{Relationship to AreaSection (T.2)}

GeoSection and AreaSection (T.2) address the same caterpillar pathology but
...
For Illinois, GeoSection still selects $1:16$ (the asymmetric root allocation
...
\begin{table}[h]
\centering
\caption{GeoSection (T.1) vs.\ AreaSection (T.2) comparison.}
...
37 of 44 states produce identical seat counts under GeoSection and the
MEC baseline.
