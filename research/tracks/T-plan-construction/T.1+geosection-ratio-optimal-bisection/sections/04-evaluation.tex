...
The reference implementation is the \texttt{bisect} Rust binary using
...
at the level reported in the sweep.
Six single-district states (AK, DE, ND, SD, VT, WY) receive trivial
...
States with compact, geographically isolated metropolitan cores can receive
$1:k-1$ splits confirmed by the normalisation.
The important evidence is not a verbal city-boundary estimate but the full
ratio scan: the selected asymmetric split must have the lowest observed
$\mathrm{EC}/\sqrt{\min(i,k-i)}$ among the feasible root ratios under the
same seed budget.
Wisconsin ($k=8$, $1:7$), Tennessee ($k=9$, $1:8$), Pennsylvania
($k=17$, $1:16$), and Illinois ($k=17$, $1:16$) are examples where the scan
confirms an asymmetric root allocation.
Large-state entries with incomplete recursive or partisan data are retained
in Table~\ref{tab:natural-ratios} but should be read as ratio-scan evidence,
not as complete outcome comparisons.
...
PA  & 50.6\,\% & 7 & 8 & $-1$ & MEC \\
\midrule
\multicolumn{6}{l}{\textit{Identical seat counts (37 states)}} \\
NC  & 49.3\,\% & 5 & 5 & 0 & -- \\
AZ  & 50.2\,\% & 4 & 4 & 0 & -- \\
\ldots & & & & & \\
\bottomrule
...
\noindent\textbf{Key finding: 37 states identical, 7 differ by 1 seat.}
...
For North Carolina, published ReCom ensemble results \citep{deford2021}
...
For Wisconsin, ensemble results (citation pending) report a median of
