# T.6 — NestSection: Nested Multi-Chamber Redistricting via Shared Bisection Trees

**Paper Type**: Algorithm Design + Empirical Analysis  
**Status**: Planning (2026-05-02)  
**Series**: B (Algorithm Design)  
**Depends on**: T.1 (GeoSection — ratio-optimal bisection), T.4 (ApportionRegions — factorization tree infrastructure)  
**Companion**: T.1, T.2 (ratio selection mechanics), T.3 (subdivision hierarchy as a nesting anchor)
...
Run GeoSection (T.1) to bisect the state into `spine` regions, selecting the
...
seats per spine region. Use ApportionRegions (T.4) to find the integer
...
- **T.1 (GeoSection)**: Isoperimetric ratio-optimal bisection is the core
  geographic primitive used at every recursion level.
- **T.4 (ApportionRegions)**: Integer seat allocation to spine regions.
  NestSection requires that ApportionRegions' output be the input to each
  sub-region's GeoSection call.
- **T.2 (AreaSection)**: Optional — AreaSection can replace GeoSection at the
  spine level to force land-area balance across chambers.
- **T.3 (SubdivisionSection)**: County boundaries can serve as spine anchors,
