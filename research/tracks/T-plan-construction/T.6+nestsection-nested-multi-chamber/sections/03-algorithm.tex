...
\emph{Mode 2 is mathematically empty}: Theorem~\ref{thm:bimodal} proves that
no positive integer triple $(C, S, H)$ can produce a score in $(0, 50)$.
The mode is defined here for completeness and to establish the classification
vocabulary, but it cannot arise for any real apportionment under the GCD
score.  Approximate-nesting designs that use a non-GCD trunk are outside this
mode and belong to the Mode~3 optimization problem.
...
\subsection{The Constructive NestSection Algorithm}

Algorithm~\ref{alg:nestsection} is the constructive target for the full
three-chamber system.  It combines the number-theoretic spine with GeoSection
partitioning and ApportionRegions seat assignment.  The current implementation
boundary is narrower: BISECT validates nested chamber relationships and records
nesting rules in audit profiles, but does not yet ship this full
three-chamber constructor.
...
\paragraph{NestRefine.}  \textsc{NestRefine} splits senate districts into house
sub-districts.  When $h_i / s_i$ is not an integer, it assigns each senate
district either $\lfloor h_i/s_i \rfloor$ or $\lceil h_i/s_i \rceil$ house
districts so the total remains $h_i$.  Any house district that cannot be kept
inside one senate district is assigned to the senate district containing the
larger share of its population and is reported as a boundary-zone violation.
The total population fraction in such flagged units is the measured
$\tau_\text{pop}$, not an automatic guarantee.

\subsection{Show the Split: Clean and Uneven Nesting}

The nesting rule should be visible in the district counts, not merely named.
In a clean state such as Washington for the state legislative tiers alone
($S=49$, $H=98$), each senate district contains exactly two house districts:
\[
  \underbrace{[HH]\ [HH]\ [HH]\ \cdots\ [HH]}_{49\ \text{senate districts}}
  \quad\text{with}\quad 98 = 49 \cdot 2.
\]
No floor/ceiling assignment is needed.

New York shows the uneven case.  The state legislative tiers have
$S=63$, $H=150$, and $\gcd(S,H)=3$.  A three-region trunk gives each trunk
region 21 senate districts and 50 house districts.  Since
$50 = 8\cdot 3 + 13\cdot 2$, each trunk requires eight senate districts with
three house districts and thirteen senate districts with two:
\[
\begin{array}{ccccccccccccccccccccc}
3&2&2&3&2&2&3&2&2&3&2&2&3&2&2&3&2&2&3&2&3
\end{array}
\]
The row sums to 50 house districts across 21 senate containers.  This is the
mechanical reason Mode~3 needs explicit reporting: the construction is not
``about 2.38 houses per senate'' in prose; it is a concrete pattern of
two-house and three-house senate containers, plus any geographic boundary-zone
population needed to realize that pattern on census blocks.

\begin{remark}[Uniformity in Mode 1 states]
\label{rem:mode1-uniformity}
In Mode 1 states ($g = C$, i.e.\ $\sigma = 0$), every trunk region receives
exactly $c_i = 1$ congressional seat: since $C/g = C/C = 1$, the
\textsc{ApportionRegions} call assigns exactly one seat to each trunk region
without any inter-region rounding.
Population heterogeneity across trunk regions is handled by
\textsc{GeoSection}'s ratio-optimal bisection within each region.
The senate seats $s_i = S/g$ and house seats $h_i = H/g$ are likewise
integers (since $g = \gcd(C,S,H)$ divides $S$ and $H$), so
$h_i/s_i = H/S$ is a uniform integer quotient within each trunk region.
For Oregon ($g=6$: $H/S = 60/30 = 2$) and Alabama ($g=7$: $H/S = 105/35 = 3$),
\textsc{NestRefine} reduces to clean recursive \textsc{GeoSection} calls
with no boundary zones.
\end{remark}
...
districts exactly.  Each trunk region then contains exactly 5 senate districts
and 15 house districts, so every senate district contains exactly 3 house
districts.
...
Algorithm~\ref{alg:compatible_spines} runs in $O(\sqrt{\max(C,S,H)})$ time:
GCD computation by Euclidean algorithm runs in $O(\log)$, but prime
factorization by trial division runs in $O(\sqrt{n})$, and for seat counts
$n \leq 200$ this distinction is practically irrelevant.
The dominant cost of
