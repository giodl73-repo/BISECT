...
\subsection{Legal And Policy Framing}

\paragraph{The structural constraint argument.}
NestSection's central policy contribution is the observation that a shared
bisection hierarchy can constrain multi-chamber gerrymandering by
\emph{construction}, not only by detection.  Once the trunk is fixed by
GeoSection at the spine level,
...
optimization occurs, but it does not prove that every remaining plan is
partisan-neutral.
...
bound to the same trunk, the congressional mapmaker cannot be instructed to gerrymander in a way
that the senate mapmaker could then undo.  The constraint is symmetric and
enforced at the algorithmic level, not at the political level. It is a process
constraint, not a promise that every output map is partisan-neutral.
...
consistent with a pre-existing federal congressional map, have a stronger
state-law footing than a federal mandate would.  This paper does not claim
immunity from federal constitutional challenge.  The compatibility score
$\sigma$ operates on the relationship among all three chamber sizes, but the
structural constraint it enforces on the senate and house maps is primarily a
matter of state law.
...
\begin{enumerate}
  \item \textbf{Mandatory strict nesting} for states where $\gcd(C,S,H) \geq 5$:
        ...
        algorithmically plausible with existing graph-partitioning and nesting
        validation tools, subject to empirical population-balance checks.
  \item \textbf{Best-effort nesting} (Mode~3 with $\tau_\text{pop} \leq 0.02$)
        for states where $\gcd(C,S,H) \geq 2$: 17 states currently qualify in
        the 2020 table.
...
natural plaintiffs may include political subdivisions (counties,
municipalities) whose territories span chamber boundaries inconsistently, but
standing is a live procedural question.  Second, what is the standard of
review?  A NestSection statute should be framed as a neutral procedural rule
grounded in administrability, compactness, contiguity, and resistance to
manipulation.  Whether a court applies rational-basis review or a more exacting
standard depends on the claim and forum.  The Bimodality Gap Theorem helps by
showing that the covered class is mathematically defined rather than selected
state by state for partisan effect.
...
hypothesis requires the ensemble-diagnostics infrastructure now exposed through
the BISECT analysis and CLI crates (R-hat, ESS, Hamming
...
representatives of their choice.  The \texttt{bisect analyze --types bloc-voting}
