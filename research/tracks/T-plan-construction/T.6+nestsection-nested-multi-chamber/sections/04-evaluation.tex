...
\emph{Partially compatible} ($0 < \sigma < 50$, a mathematically empty class
under this score), and \emph{Mode~3 / weak trunk} ($\sigma \geq 50$).  The
table label ``Incompatible'' means incompatible with exact three-chamber
strict nesting, not useless for validation or best-effort alignment.
...
\paragraph{Trivially compatible ($C = 1$): 6 states.}
Alaska (AK), Delaware (DE), North Dakota (ND), South Dakota (SD), Vermont (VT),
and Wyoming (WY) all have $C = 1$ in the 2020 apportionment.
...
\paragraph{Weakly compatible ($C = 2$, $g = 2$): 3 states.}
Montana (MT, $C=2$, $S=50$, $H=100$, $g=2$) and New Hampshire
(NH, $C=2$, $S=24$, $H=400$, $g=2$), and West Virginia
(WV, $C=2$, $S=34$, $H=100$, $g=2$).  These states have a trunk of 2 regions
...
read carefully: 6 are trivially compatible ($C=1$), 3 are weakly compatible
...
\textit{60 house leaves --- to be generated by the future BISECT constructor]}
\vspace{2em}}}
...
\subsection{Case Study 2: North Carolina --- Weak-Trunk Mode 3}
...
\end{enumerate}
The target specification is $V_\text{NC} \leq 0.04$ while sharing the single
east-west trunk; an implemented run must report the achieved boundary-zone
fraction rather than assuming the target is automatically met.
...
threshold of $g \geq 3$ would include 9 states in this table: Alabama, Oregon,
Louisiana, Arizona, New Jersey, Tennessee, Ohio, Florida, and California.  A
broader $g \geq 2$ best-effort tier would include 17 states, including the
weak two-region trunks in Montana, New Hampshire, West Virginia, Iowa,
Mississippi, Kentucky, Georgia, and North Carolina.
