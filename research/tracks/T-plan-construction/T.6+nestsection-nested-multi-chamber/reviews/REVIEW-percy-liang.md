...
**Required**: Remove the 30-50% estimate from §5.3 if it is not supported by current data, and replace it with a formal hypothesis ("We hypothesize that NestSection reduces partisan variance; this will be tested using the ensemble diagnostic infrastructure from [T.1]").
