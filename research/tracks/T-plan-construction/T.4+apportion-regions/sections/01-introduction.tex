...
Specifically, the prime factorization of $n$ determines a canonical
largest-prime-first split prescription for any geographic region: split first
by the largest prime factor, then recurse on each child workload, using a
binary floor/ceiling fallback when the remaining workload is itself a large
prime. When applied to a state's census-tract graph using minimum-cut graph
...
\end{enumerate}
No political data enters. No human boundary choices are made once the
implementation, seed derivation rule, and tie-breaking policy are fixed. The resulting
...
computed once and reused. The canonical largest-prime split is computed once
and reused across seat counts that share that same top-level factor. For
example, seat counts $34 = 2 \times 17$ and $51 = 3 \times 17$ both begin with
the same canonical 17-way partition before their child workloads diverge.
...
($53$ is prime). Going from 52 to 53 requires discarding the 13-way top-level
structure and replacing it with the large-prime binary fallback
$53 \to (26,27)$. Going from 51 ($= 3 \times 17$) to 52 ($= 4 \times 13$)
replaces a 17-way top level with a 13-way top level. These discontinuities are
...
\begin{enumerate}
  \item \textbf{Algorithm}: The AR algorithm --- a deterministic,
        ...
        + census geography).
  \item \textbf{Constitutional argument}: An argument that AR maps can be
        understood as extending Article~I~\S2 (apportionment) into geography,
        rather than treating \S4 (Manner) as the only source of districting
        authority.
