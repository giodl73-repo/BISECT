...
The seed-invariance result (\S\ref{sec:eval:seeds}) narrows the practical
importance of the remaining seed parameter for the outcomes measured here.
AR with $n = 14$ and $G = \text{NC's census graph}$ produces the same 7D/7R
outcome across the tested METIS seeds.
For legal certification, however, the seed remains part of the artifact
contract: the submitted plan should fix the seed derivation rule, graph hash,
and tie-breaking policy so the exact tract assignment can be reproduced.
AR therefore replaces discretionary boundary drawing with a published
computational rule; it does not remove the need to disclose implementation
choices.
...
\begin{enumerate}
  ...
  \item \textbf{Irreducible split units}: Primes are irreducible --- a $p$-way
        split cannot be further decomposed without introducing an ordering
        choice. Composite splits would be redundant: a $6$-way split could
        mean $2\times 3$, $3\times 2$, or a direct 6-way cut.
  \item \textbf{Consistency with largest-prime-first}: The canonical ordering
        (largest prime first) maximises reuse across seat counts that share a
        large factor. Seat counts $34 = 2\times 17$ and $51 = 3\times 17$,
        for example, begin with the same 17-way geographic partition; a
        smallest-prime-first rule would split them into halves and thirds
        instead, destroying that shared top-level structure.
\end{enumerate}
...
\begin{itemize}
  \item $51 \to 52$: $[17, 3] \to [13, 2, 2]$. Complete structural break.
        The 17-way top level gives way to a 13-way top level, with a
        completely different secondary structure.
  \item $52 \to 53$: $[13, 2, 2] \to [53\text{-binary}]$. Complete structural
        break. The 13-way top level gives way to the large-prime fallback
        split $53 \to (26,27)$.
  \item $n = $ prime $\to n+1 = 2 \times \text{something}$: gaining one seat
        converts a large-prime binary fallback tree to a composite
        largest-prime tree.
\end{itemize}
...
distribution, we compare against published GerryChain ensemble results for
NC-14 \citep{herschlag2020quantifying}:
\begin{itemize}
...
geography under a disclosed implementation, seed rule, and tie-breaking
policy.
