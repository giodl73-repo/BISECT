...
bisection algorithm.
Using a 2030 projection scenario and simulation targets on 2020 census-tract
graphs, we define the following scenario findings:

\begin{enumerate}
\item \textbf{Texas is the most disruptive case.}
      $38 = 2 \times 19 \to 41$ (prime) changes the tree from 19 two-seat
      containers to a binary 20/21 fallback.
      Scenario Hamming distance: $d_{\rm Ham} = 0.23$.

\item \textbf{Florida is also dramatically affected.}
      $28 = 4 \times 7 \to 29$ (prime) produces a similar change from a
      seven-container top level to a binary 14/15 fallback.
      Scenario $d_{\rm Ham} = 0.19$.

\item \textbf{Scenario disruption is far below full MCMC mixing.}
      The worst-case reapportionment disruption ($d_{\rm Ham} = 0.23$)
      is equivalent to approximately 10 one-step \recom{} units for Texas
      under the $1/k$ scale approximation.
      A fully-mixed ensemble (10{,}000+ steps) produces $d_{\rm Ham} \approx 1$.

\item \textbf{Political redistricting is more disruptive.}
      The draft comparison range for contested political redistricting cycles
      is 35--55\% of tract assignments, $1.5$--$2.4\times$ more than the
      Texas scenario.  This comparison still needs same-metric evidence.
...
\item \textbf{Fresh recomputation is the correct statutory design.}
      ...
      No continuity bonus or grandfathering clause is required by the current
      algorithmic staging; adding one would be a separate policy choice.
\end{enumerate}
...
A state moving from a composite largest-prime-first top level to a large-prime
binary fallback (Texas, Florida) has scenario disruption
$d_{\rm Ham} \approx 0.20$--$0.23$.

This arithmetic is predictable and transparent.
States and administrators can compute scenario disruption years in advance
from projection inputs, then replace the scenario with certified census data
when the official apportionment is available.
...
\item \textbf{2030 census data.}
      ...
      A 2030 simulation requires certified 2030 census inputs and final
