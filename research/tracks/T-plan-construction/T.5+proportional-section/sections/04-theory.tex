...
\begin{remark}
The Lorenz bound predicts \textit{maximum achievable} proportionality improvement; the observed 7.1\,pp for Georgia reflects that ProportionalSection approaches (but does not reach) this bound under the feasible plan space. The bound is a ceiling, not a prediction. The $C \cdot \sigma \approx 0.7$\,pp figure describes the marginal sensitivity of the bound at $\sigma = 0.001$; the larger observed improvement occurs because ProportionalSection moves the partition across multiple topological configurations, each contributing incrementally.
\end{remark}
