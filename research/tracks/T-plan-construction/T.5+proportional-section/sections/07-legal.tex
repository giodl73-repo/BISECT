...
\begin{enumerate}
  ...
  \item that states with $\sigma < 0.05$ (free proportionality) are presently
    gerrymandered, or that such states are immune from gerrymandering claims;
    $\sigma$ describes only this paper's top-level proportional-target signal;
    or
  \item that T.5 at any $\eta$ is guaranteed to improve proportionality ---
...
T.5 is not a proposal to violate this prohibition.
...
T.5 measures those consequences.
...
For state legislative redistricting, T.5 provides a rigorous operationalisation
...
T.5's most immediate legal contribution is evidentiary.
...
If $\sigma < 0.05$ and the algorithm still produces a $-10$\,pp gap, the
shortfall is not explained by a large top-level proportional-target shift
alone. It may arise from contiguity, granularity, downstream recursive splits,
or ordinary partisan design in an enacted map; T.5 does not identify intent by
itself.
...
T.5's analysis does not challenge this holding, but it reframes what the
holding leaves open.
...
T.5's algorithmic approach offers a different posture: an algorithm that
...
In the post-\emph{Rucho} landscape, three legal developments matter for T.5:
...
\begin{enumerate}
  \item \textbf{State courts}: \emph{Rucho} explicitly invited state-court
    ...
    provisions.
    T.5's $\sigma$ classification provides a quantitative diagnostic that
    state courts can inspect: when $\sigma = 0$, the proportional top-level
    target coincides with the neutral target. That fact is evidence about one
    mechanism, not immunity from a broader partisan-gerrymandering claim.

  \item \textbf{The algorithmic neutrality standard}: T.5 defines a
    ...
    T.5's proportionality paradox shows that this observation is geographically
    incorrect for competitive states: an unbiased system (T.2) does \emph{not}
  ...
  \item \textbf{Procedural fairness as the justiciable claim}: T.5 supports
    ...
    algorithm (such as T.2 or a variant whose proportional target coincides
    with the neutral target) is a
    procedural mandate, not an outcome mandate.
    State courts in Pennsylvania and North Carolina have accepted this framing.
\end{enumerate}

T.5's central legal contribution is thus not to challenge \emph{Rucho} but to
