...
the direct precursor to the Rodden effect documented empirically in T.2.
...
--- edge-weighted bisection (B.2), GeoSection (T.1), AreaSection (T.2) ---
are the foundation of this work.
T.2 establishes the key empirical fact exploited here: the proportionality
...
T.4 (PrimeFactor) provides the Huntington-Hill seat allocation $k_D$, $k_R$
...
AreaSection (T.2) introduced $\mathtt{ncon}=2$ for redistricting with vertex
...
visualise sorting.
Spatial Lorenz curves are commonly used to describe geographic concentration.
...
population-area Lorenz curve of T.2.
