...
**Rucho engagement:** The Rucho section is now substantive rather than a brief acknowledgement. The three post-Rucho developments (state courts, algorithmic neutrality standard, procedural fairness claim) are correctly identified and the legal analysis is sound. The point that T.5's proportionality paradox is directly relevant to *Rucho*'s factual premise (that unbiased systems would produce proportional results) is the paper's strongest legal contribution and deserves the prominence it now receives.
...
**2. GA/WI reconciliation with T.2 data.** My Round 1 P1 request to cross-validate GA/WI gaps against enacted-plan T.2 data has not been addressed. The paper reports GA at −14.4pp and WI at −12.8pp under T.2 (geography-only). The T.2 paper presumably reports these same figures. But the paper does not cite the specific T.2 figures or verify that the T.5 setup (METIS 5.1.0, 1.5% tolerance) reproduces the T.2 baseline. Without this verification, the T.2 column in Table 1 (§5.1) is ungrounded.
