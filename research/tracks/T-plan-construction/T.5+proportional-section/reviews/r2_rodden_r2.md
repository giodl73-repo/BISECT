...
The empirical table (§5.1, Table 1) is the most important addition. The discovery that NC *worsens* at all η values is a genuinely surprising result that the analytical framework did not predict. The explanation — that the T.5 constraint forces a 7:7 D-bloc split but the D-bloc lacks sufficient concentration to win all 7 seats — is intuitive once stated, but not obvious from the theory. This is good empirical science.
