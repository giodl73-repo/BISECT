...
2. **T.2 baseline verification column still absent.** I asked in Round 2 for the T.2 column in Table 1 to be verified against T.2 paper figures. The paper still says "T.2 column is the geography-only baseline" without citing where those values come from. This is a P2 item.
...
1. Are the Table 1 entries (T.2 baseline, η=1.05, 1.10, 1.20) deterministic across all 30 seeds, or are they averages? If deterministic, please state this.
...
- **P2**: Add a citation for the T.2 baseline values in Table 1.
