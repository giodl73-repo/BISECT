...
**4. Confidence intervals absent.** The empirical table (Table 1) reports point estimates (−12.8pp, −0.3pp, etc.) from 30-seed runs without confidence intervals or inter-seed variance. If the results are deterministic across seeds (as in T.4), this is fine — but the paper should explicitly state whether the 30-seed runs produced identical results or averaged outcomes. If variance exists across seeds, confidence intervals are needed.
