...
1. **METIS full parameter vector (ncuts, niter, numbering) still not reported.** The Round 2 evaluation section specifies "METIS 5.1.0, 30 seeds, 1.5% balance tolerance" but niter and ncuts defaults for 5.1.0 differ from 5.0.x. The paper should specify niter and ncuts for the T.5 runs as it now does for T.4. This was my P2 item in Round 2 and is not resolved in Round 3.
...
1. For the T.5 empirical runs: what were the niter and ncuts values? Were these the METIS 5.1.0 defaults?

2. Is C(G) ≈ 4,200 for Wisconsin computed from the Lorenz curve (analytical) or from METIS run variance (empirical)?

### Suggestions

- **P1**: Add niter and ncuts to the T.5 METIS parameter specification (§5.1 or Reproducibility section).
