# Revision Plan — T.5 ProportionalSection
...
- **"What this paper does not show" disclaimer**: Added "Scope of Claims" subsection to §7 (legal) with 5 explicit disclaimers: courts cannot order this method, no partisan outcome is constitutionally required, algorithm is explicitly partisan, σ<0.05 does not imply gerrymandering, T.5 may worsen outcomes (NC example).
...
- [ ] Add niter and ncuts to T.5 METIS parameter specification (Karypis)
...
- [ ] **T.2 baseline verification** (Duchin): The T.2 column in Table 1 should be verified against the T.2 paper's reported figures, not assumed.
