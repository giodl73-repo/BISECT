...
The table below is a draft benchmark comparison, not yet an archived package
claim.
Publication use requires the same artifacts as the newer T/U papers: RPLAN,
RCTX, audit certificate, manifest hashes, CLI transcript, and a record of the
exact CVD metric and centroid file when geographic mode is used.
All draft runs use 2020 Census data, geographic boundary-length edge weights
...
\item \textbf{CVD(iters=20)}: graph-distance \cvd{} bisection with
  $\mathtt{cvd\_iters} = 20$, farthest-first seed initialisation,
...
\item \textbf{Polsby-Popper (PP)}: Mean PP compactness across all $k$ districts;
  ...
  Baseline values from U.9~\citep{h1paper}: NC $\pp = 0.217$, WI $\pp = 0.198$,
  TX $\pp = 0.184$.
...
Table~\ref{tab:main} presents the draft comparison.
...
\centering
\caption{%
  Draft algorithm comparison on NC ($k=14$), WI ($k=8$), TX ($k=38$).
  ...
  Values should be treated as benchmark targets until archived with run
  manifests.
}
\label{tab:main}
...
Convergence is faster on elongated states (WI) because the farthest-first
...
Table~\ref{tab:partisan} reports partisan seat counts (2020 presidential
two-party returns) for CVD plans compared to standard bisection.

\begin{table}[h]
\centering
\begin{tabular}{lrrrr}
\toprule
State & $k$ & CVD & Standard METIS & Single-run note \\
\midrule
NC & 14 & 7D/7R & 7D/7R & Stable \\
WI & 8  & 3D/5R\textsuperscript{\dag} & 2D/6R\textsuperscript{\dag} & One-seat variation \\
TX & 38 & 16D/22R & 16D/22R & Stable \\
\bottomrule
\end{tabular}
\caption{Draft partisan outcomes (2020 presidential, single run, seed $s=42$).
These rows are diagnostics only.  A single CVD run does not establish partisan
neutrality or proportionality; those claims require multi-seed, same-metric
packages.
\newline\footnotesize{$\dagger$ Single-run result (seed=42); partisan seat counts are seed-sensitive for competitive states. Multi-seed validation is pending.}}
\label{tab:partisan}
\end{table}
...
\paragraph{PP distribution overlap.}
To assess whether CVD plans inhabit a geometrically distinct region of plan
space (rather than merely offset METIS plans), we computed the PP histogram
overlap between 100 METIS seeds and 20 CVD variants (different base seeds on
NC $k=14$).
The CVD PP distribution is centred at
$\mu_\mathrm{CVD} = 0.242 \pm 0.008$ and the METIS PP distribution at
$\mu_\mathrm{METIS} = 0.217 \pm 0.011$; the distributions have $<15\%$ overlap,
suggesting that CVD plans occupy a geometrically distinct region of plan space.
This distributional comparison also needs an archived seed manifest before it
can carry publication weight.
...
different CVD node seed and different farthest-first initialisation), and select
