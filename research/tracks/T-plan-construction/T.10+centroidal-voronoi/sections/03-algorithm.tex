...
For bisection ($k=2$), \cvd{} assigns each tract to one of two child regions
$\{D_0, D_1\}$ and returns a partition $(S, V_v \setminus S)$ satisfying
the current implementation's half-population rebalance target:
\[
  \left|\sum_{i \in S} p_i - \frac{1}{2} P_v \right|
  \;\le\;
  \delta_v \cdot P_v,
\]
where $\delta_v$ is the per-node tolerance passed by the runner.
The surrounding \texttt{BisectionTree} records $k_L/k_v$ for the child
schedule, but \texttt{split\_subgraph\_cvd} currently rebalances the local
binary split toward one half.
Ratio-aware CVD is therefore a future implementation/evidence item for
odd-seat nodes.
...
(e.g., \texttt{"SA\_NODE\_"} for U.3's simulated annealing).
...
\subsection{Farthest-First Seed Initialisation}

The two initial seeds are placed by a farthest-first procedure.
For graph-distance \cvd{}, the distance is BFS hop count; for geographic
\cvd{}, it is projected Euclidean distance.

\begin{definition}[Farthest-First Initialisation]
\label{def:kfarthest}
...
Farthest-first initialisation maximises the initial distance between the two seeds,
preventing the degenerate configuration where both seeds begin in the same
geographic region.
For path-like subgraphs (rural county chains), farthest-first trivially places
...
The implementation computes one BFS distance vector from each seed and then
assigns each tract by comparing the two distances.  For a binary split this is
two single-source BFS passes per assignment step, still $O(m+|E_v|)$ on the
local subgraph.

\begin{definition}[Approximate Graph-Distance Medoid]
\label{def:medoid}
Let $D_j = \{t \in V_v : a(t) = j\}$ be the tracts assigned to district $j$.
Let $\mathrm{probe}_j \subseteq D_j$ be a deterministic-random sample, derived
from the CVD node seed, of at most $n_{\mathrm{probe}} = 50$ tracts from $D_j$
(or all tracts if $|D_j| \le 50$).
The \emph{approximate medoid} of $D_j$ is
\[
  \widehat{\mathrm{medoid}}_j =
  \arg\min_{i \in D_j} \sum_{t \in \mathrm{probe}_j} d_{\mathrm{BFS}}(i, t).
\]
\end{definition}

This direction matters: the candidates are all district tracts, while the
distance targets are the sampled probes.
For $|D_j| \le 50$, the probe set equals $D_j$ and the medoid is exact.

\subsection{Show the Iteration}

The algorithm is easiest to audit when the split is drawn, not merely named.
In the small grid below, \texttt{A} and \texttt{B} are the initial seeds, lower
case letters are first-iteration ownership regions, and the final row shows the
post-update split after the medoids move inward.

\begin{center}
\begin{tabular}{ccc}
\textbf{1. Seeds} & \textbf{2. Voronoi ownership} & \textbf{3. Updated split} \\
\begin{minipage}{0.22\linewidth}\ttfamily\small
A . . . . B\\
. . . . . .\\
. . . . . .\\
. . . . . .
...
On an irregular state graph, the same three objects are what the audit package
should expose: node path and seed derivation, seed-to-tract distances, the
assignment before rebalance, and any boundary swaps used to meet population
tolerance.
...
\State $\mathrm{seed}_1 \gets \arg\max_{t \in V_v} d_0[t]$
       \Comment{farthest-first: Definition~\ref{def:kfarthest}}
...
\For{$\mathrm{iter} = 0$ \textbf{to} $\mathtt{cvd\_iters} - 1$}
  \State $a \gets \mathrm{VoronoiAssign}(\mathrm{seeds}, G_v)$
         \Comment{two seed-distance BFS passes: Definition~\ref{def:voronoi-assign}}
  \For{$j \in \{0, 1\}$}
    \State $D_j \gets \{t : a(t) = j\}$
    \State $\mathrm{probe}_j \gets \mathrm{Sample}(D_j, \min(50, |D_j|))$
    \State $\mathrm{seeds}[j] \gets \arg\min_{i \in D_j}
           \sum_{t \in \mathrm{probe}_j} d_{\mathrm{BFS}}(i, t)$
...
\paragraph{Decision 1: Farthest-first seed placement.}
...
Farthest-first initialisation places seeds at distant tracts in the subgraph,
maximising coverage from the first iteration.
This reduces the typical iteration count from $\sim 20$ (random initialisation)
to $\sim 10$ (farthest-first) in the draft NC-scale runs.
...
The 200-iteration boundary-swap rebalance corrects population imbalance by
moving boundary tracts from the overweight district to the underweight
district, one tract at a time.
The CVD implementation checks for a boundary neighbor before moving a tract;
stronger post-move contiguity witnesses should be supplied by the final
RPLAN/RCTX audit package rather than inferred solely from the split routine.
...
farthest-first initialisation places $\mathrm{seed}_0$ and $\mathrm{seed}_1$
...
\begin{itemize}
\item One BFS for farthest-first initialisation: $O(m)$.
\item Per iteration: two seed-distance BFS passes ($O(m)$ each) for
...
This is linear in the local graph size for a fixed probe cap and fixed
rebalance cap, but the hidden constant is substantial because the approximate
medoid evaluates many BFS passes.
In practice, \cvd{} and \metis{} can have similar wall-clock times; precise
runtime claims should be backed by benchmark packages rather than only by the
asymptotic expression.

\subsection{CLI Invocation}

\cvd{} bisection is exposed via \texttt{--structure centroidal-voronoi}
in the \texttt{bisect} binary:
...
Every publication-grade \cvd{} run should append structured metadata to
the run manifest:
...
An auditor who knows \texttt{base\_seed} and \texttt{node\_path} can recompute
the graph-distance seed from:
\begin{verbatim}
SHA-256("CVD_INIT_" || node_path || "_" || base_seed:u64le)
\end{verbatim}
Geographic \cvd{} uses the same formula with the prefix
\texttt{"CVD\_GEO\_INIT\_"}.
Full reproducibility also requires the exact graph, population vector,
centroid file when used, metric, iteration budget, and rebalance trace.
