...
\cvd{} uses the graph-distance medoid in its default graph-distance mode:
the ``centroid'' of each district after Voronoi assignment is the tract
minimising sampled BFS hop-count distance to the district.
Geographic \cvd{} instead computes a population-weighted coordinate mean and
snaps that point back to the nearest actual tract.
Both modes ensure that the next Voronoi assignment starts from a replayable
tract anchor rather than an unspecified fractional object.
...
\cvd{} uses an approximate medoid: sample up to 50 tracts from $S$ using the
node's deterministic RNG stream and compute the candidate's BFS sum only to
those sampled probes.
The district tract minimising this approximate sum is the approximate medoid.
...
Geographic \cvd{} (Section~\ref{sec:phase2}) addresses this by
...
\caption{Structure-layer algorithms in the B-series redistricting platform.
  ...
  CVD optimises proximity under the selected metric (not edge cut).
  All algorithms enforce population balance via post-hoc rebalance.}
\label{tab:structure-comparison-bg}
\small
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{@{}p{0.17\linewidth}p{0.25\linewidth}p{0.13\linewidth}p{0.15\linewidth}p{0.20\linewidth}@{}}
\toprule
Algorithm & Optimises & Randomness & Data needed & Status \\
\midrule
\textsc{Metis} & Edge cut & Seeded & none & implemented \\
SA & Edge cut + annealing & Seeded & none & implemented \\
GeoSection & Direction + EC ratio & Seeded & none & implemented \\
AreaSection & Area-swing + EC & Seeded & none & implemented \\
\textbf{CVD} & \textbf{Graph/geographic proximity} & \textbf{Seeded}
                        & \textbf{None / centroids} & \textbf{Implemented; package evidence pending} \\
BFS Growth & Population balance + proximity & Seeded & none & T.12 \\
\bottomrule
\end{tabular}
\end{table}

\cvd{} is orthogonal to the seed-compositor layer.
\be{} or convergence sweeps can be applied over \cvd{} subproblems just as
over \metis{} subproblems.
