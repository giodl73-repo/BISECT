...
\subsection{Two-Seed Farthest Selection}
...
The implemented two-seed strategy selects seeds greedily by BFS distance:
\begin{enumerate}
...
\item The second seed is the tract with maximum BFS distance from seed 0.
\end{enumerate}
This is a graph-distance analogue of farthest-point initialisation
\citep{cormen2009}, but the implemented form is binary because BISECT grows
plans through recursive bisection.
...
This priority function fills the \emph{currently smaller} side first.
...
The priority is a heuristic, not a balance proof. Large tract populations,
frontier geometry, disconnected components, and the later rebalance pass all
affect the final deviation.
...
Compactness & Benchmark target & Proximity packing & Edge-cut oriented \\
...
apportionment.
\bfsg{} is the structure-layer algorithm that optimises the least: it is the
minimal adjacency-growth baseline against which stronger splitters can be
measured once same-metric packages exist.
