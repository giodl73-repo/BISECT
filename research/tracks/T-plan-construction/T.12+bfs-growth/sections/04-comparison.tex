...
\midrule
\multirow{3}{*}{NC ($k=14$)}
  & \bfsg{}  & 0.247 & 3{,}847 &  3.8 \\
  & \metis{} & 0.217 & 2{,}891 &  4.1 \\
  & \cvd{}   & 0.264 & 3{,}312 & 22.4 \\
\midrule
\multirow{3}{*}{WI ($k=8$)}
  & \bfsg{}  & 0.222 & 2{,}204 &  1.7 \\
  & \metis{} & 0.198 & 1{,}876 &  1.9 \\
  & \cvd{}   & 0.238 & 2{,}050 &  9.3 \\
\midrule
\multirow{3}{*}{TX ($k=38$)}
  & \bfsg{}  & 0.201 & 9{,}123 & 11.2 \\
  & \metis{} & 0.184 & 7{,}640 & 12.7 \\
  & \cvd{}   & 0.217 & 8{,}391 & 68.5 \\
\bottomrule
\end{tabular}
\end{table}
...
confirming the observation from T.1 that METIS's KL refinement optimises
...
Despite this gap, \bfsg{} still outperforms \metis{} on PP by 9--14\%,
...
The relationship between EC-optimisation and PP depends on the state geometry
and edge-weight configuration.
With geographic boundary-length weights, METIS minimises cuts along long
borders, which tends to preserve compact shapes in regularly-shaped states.
BFS Growth's round-district tendency from k-farthest seeds produces higher PP
despite not optimising EC.
...
Table~\ref{tab:partisan} reports partisan seat outcomes under the 2020
presidential vote for a single run ($\mathtt{base\_seed} = 42$,
\texttt{--search single}).
Partisan seat counts are determined by majority-vote assignment of 2020
presidential results to districts.

\begin{table}[t]
\centering
\caption{Partisan outcomes (2020 presidential, seed $s=42$, single run).
  D = Democratic-leaning seats; R = Republican-leaning seats.}
\label{tab:partisan}
\begin{tabular}{llcc}
\toprule
State & Algorithm & D seats & R seats \\
\midrule
\multirow{2}{*}{NC ($k=14$)}
  & \bfsg{}  & 7 & 7 \\
  & \metis{} & 7 & 7 \\
...
For NC and TX, both algorithms produce identical partisan outcomes,
reflecting geographic sorting that dominates the partisanship signal
regardless of district boundary placement.
WI shows a one-seat difference: \bfsg{}'s rounder districts alter the
competitive district margin, shifting one seat from R to D.
This illustrates that when geographic proximity is combined with
competitive partisan geography, the single-pass vs.\ KL-refined boundary
placement can affect partisan outcomes at the margin.
