...
Phase~1 Centroidal Voronoi Districts (\cvd{}, T.10~\citep{dellaLibera2026cvd})
...
This paper presents the Phase~2 geographic extension of Centroidal Voronoi
Districts (T.10~\cite{dellaLibera2026cvd}).
T.10 introduced the basic \cvd{} framework for redistricting, using BFS
hop-count distance and graph-distance medoid iteration to produce compact
districts via discrete Lloyd's algorithm.
The present paper extends it to incorporate geographic boundary constraints,
replacing hop-count with true Euclidean distance on EPSG:5070 projected
tract centroids, achieving 2--6\,\% additional Polsby-Popper improvement
over Phase~1.
The two papers are intended to be read together.

\subsection{Scope and Main Results}

This paper is structured as a companion to T.10~\citep{dellaLibera2026cvd}.
...
and the \texttt{bisect fetch --type centroids} pipeline step.
