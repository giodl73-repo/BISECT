...
\begin{remark}[Projection scope]
The current BISECT implementation applies the same spherical Albers transform
for every geographic CVD run. State-specific projection dispatch for Alaska,
Hawaii, Puerto Rico, and territories is a future evidence and implementation
item. Any publication package using those jurisdictions should record the
projection used and should not imply automatic EPSG dispatch until that code
exists.
\end{remark}
...
\subsection{\texorpdfstring{$\varepsilon$-Convergence Criterion}{Epsilon-Convergence Criterion}}
...
\begin{algorithm}[t]
\caption{\cvdgeo{}-\textsc{Bisect}($G_v$, $\mathbf{x}$, $\mathbf{p}$,
...
\State $(S, \bar{S}) \gets \mathrm{HalfRebalance}_{200}(a, G_v, \mathbf{p}, \mathtt{balance\_tolerance})$
\Comment{current implementation target}
...
The geographic mode is easiest to audit as a sequence of ownership pictures.
The first row shows two seeds in projected coordinate space. The second row
shows Euclidean Voronoi ownership. The third row shows the population-weighted
centroid of each owned side and the snap back to a real tract.

\begin{center}
\begin{tabular}{c@{\qquad}c@{\qquad}c}
\textbf{1. Seeds} & \textbf{2. Ownership} & \textbf{3. New seeds} \\
...
The point is not the labels ``0'' and ``1''; it is the visible ownership
frontier. Geographic CVD changes where that frontier falls because its distance
field is drawn from projected coordinates instead of graph hops.
...
\begin{proposition}[Bounded Lloyd Step for Geographic CVD]
...
The assignment and continuous centroid update are Lloyd steps in projected
space. The nearest-tract snap adds a bounded discretisation error determined
by the local tract-centroid spacing.
\end{proposition}
...
(c) Nearest-tract snap replaces the optimal $\bar{\mathbf{c}}_j$ with the closest
    ...
    converging.
Combining (a), (b), and (c), the implemented update is a Lloyd-style descent
step followed by bounded discrete snapping. Because the implementation also
has a fixed \texttt{cvd\_iters} cap, termination is guaranteed even when a
discrete two-cycle would otherwise persist.
\end{proof}
...
\item Total: $O((n_{\mathrm{iter}} + 200) \times m)$ for the implemented
  two-seed bisection node.
\end{itemize}
For $n_{\mathrm{iter}} = 20$: $O(220m) = O(m)$.
This removes the graph-medoid BFS passes used by graph-distance CVD. End-to-end
speed still depends on centroid I/O, projection cost, rebalance work, and final
validation, so the paper treats runtime comparison as an empirical benchmark
target rather than a theorem.
\end{proposition}

\subsection{Audit Chain Fields}
\label{subsec:audit}

The audit package for a geographic CVD run should expose, at minimum, the
following fields or equivalent manifest entries. Some are implemented in logs
or configuration today; others are acceptance targets for a complete archived
package:
...
"base_seed": 12345678,
"cvd_seed": 987654321,
"cvd_seed_formula": "SHA-256('CVD_GEO_INIT_' || node_path || ...)",
"node_path": "01",                    (dagger)
"iterations_to_convergence": 11,
"convergence_warning": false,
"projection": "spherical Albers / EPSG:5070 constants", (dagger)
"final_ec": 2847,
"final_pp_mean": 0.241
\end{verbatim}
...
\texttt{projection} should record the transform actually used. In the current
code path, that is the platform spherical Albers transform with EPSG:5070
constants, not state-specific dispatch.
