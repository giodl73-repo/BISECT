...
All 2020 GeoSection baseline results use 2020 Census data, TIGER tract geometries,
...
The reference implementation is the \texttt{bisect} Rust binary, partition
...
For earlier census years, this paper uses two different evidence streams.
The 2000/2010 tract-split proxy table reports 30 comparable cases where the
first-bisection tract share can be read consistently.  Full CSS still requires
the stronger package layer: archived GeoSection summaries for each year,
district assignments projected to a common geography, and same-year outcome
diagnostics.

\subsection{2020 Baseline: Full 50-State Natural Ratio Table}
\label{sec:2020-baseline}

Table~\ref{tab:natural-ratios-2020} reports the GeoSection natural ratio
baseline for multi-district states using 2020 Census data.
This is the anchor dataset for the cross-census comparison; rows marked
...
\midrule
\multicolumn{6}{l}{\textit{Large states (2020 baseline provisional for publication use)}} \\
GA  & 14 & (est.\ 5:9) & --- & --- & 50.1 \\
MO  &  8 & (est.\ 1:7) & --- & --- & 41.4 \\
TX  & 38 & 1:37 & --- & --- & 46.5 \\
FL  & 28 & 1:27 & --- & --- & 47.9 \\
...
The complete two-census CSS table is not yet available.
This section records the comparison design and the available proxy evidence
rather than a final 2000--2020 CSS result.
...
\textbf{Candidate high CSS ($\geq 0.90$): stable geography, low growth.}
...
\textbf{Candidate medium CSS ($0.70$--$0.90$): growing metros, stable ratios.}
...
\textbf{Candidate low CSS ($< 0.70$): Sun Belt growth corridor.}
...
Table~\ref{tab:ratio-stability} provides 2020 natural ratios alongside
provisional stability classifications based on the Lorenz drift proxy and
available first-split evidence.  It is a triage table for the Phase~2 package,
not a replacement for archived two-census CSS.

\begin{table}[h]
\centering
\caption{Natural ratio stability: 2000 vs.\ 2020 prediction.
  ``Same'' = predicted identical ratio.
  ``Shifted'' = ratio expected or observed to change (suburban growth).
...
\midrule
\multicolumn{6}{l}{\textit{Unstable (ratio shift confirmed)}} \\
IA  &  4 & 2:2  & Shifted  & $< 0.70$    & Low \\
\midrule
\multicolumn{6}{l}{\textit{Stable geography (high candidate CSS)}} \\
...
\midrule
\multicolumn{6}{l}{\textit{Growing metros (medium candidate CSS)}} \\
...
\midrule
\multicolumn{6}{l}{\textit{Sun Belt growth (low candidate CSS)}} \\
...
For the available 2000--2010 proxy sample, the key stability metric at the
first-bisection level is the
...
The 10 unstable or borderline cases include Alabama, Iowa, Mississippi,
South Carolina, and Utah; the largest observed regime change is Iowa.
...
Using Wilson's method, the 95\,\% confidence interval on the 67\,\%
stability rate (20/30 states) is $[48\,\%, 82\,\%]$.
The wide interval reflects the modest sample size.
The finding is best interpreted as: ``majority-stability is observed, with
the lower bound of 48\,\% still above the null of 20\,\% random stability''
(see \S\ref{sec:null-hypothesis} for the null derivation).

The 67\,\% proxy stability rate across a full census decade is the core empirical
contribution of this paper.
It implies that for the majority of states, the \emph{geographic structure
of the state appears to preserve the first split} under this proxy comparison.
These ``census-stable'' maps are strong candidates for the evidentiary
argument in \S\ref{sec:legal}.
Table~\ref{tab:three-census-css} provides the structure of the future full
result table; values marked pending require the Phase~2 package.

\begin{table}[h]
\centering
\caption{Three-census CSS (2000/2010/2020). Package computation pending.
...
\multicolumn{6}{c}{\textit{(full 50-state table pending package computation)}} \\
\bottomrule
\end{tabular}
\end{table}
...
The proxy evidence is consistent with a $1:3$ first split in this census year:
a compact Des Moines-area peel competes against a larger rural remainder.
...
The first-bisection proxy shifted to a 630/656 split ($f = 0.49$),
...
\paragraph{The Lorenz drift proxy for Iowa.}
The proxy table reads Iowa as a $1:3$-style split in 2000 (Des Moines isolated)
and a $2:2$-style split in 2010 (near-equal E/W split).
Under the Lorenz interpretation, a $1:3$ ratio at the first level indicates
$p^*(0.5) \approx 0.65$ --- the densest 50\,\% of Iowa's land contained
approximately 65\,\% of the state's population in 2000, consistent with
Des Moines metro concentration.
By 2010, Iowa City, Cedar Rapids, and Quad Cities growth had dispersed
population enough that $p^*(0.5)$ fell toward 0.55, pushing GeoSection
toward the equal split.
This is Iowa's Hypothesis~A (Lorenz drift from suburban growth), as distinct
from Hypothesis~B (tract redesign artefacts).
Iowa would be the priority case for computing actual Lorenz curves from the
2000 and 2010 tract-level data.
...
the package-complete cross-year assignments are available.
...
\multicolumn{5}{l}{\textit{Entries without paired outputs are provisional and derived from 2020 $p^*$.}} \\
\end{tabular}
\end{table}
...
working threshold (5\,\%).  A final paper should justify that threshold from
archived seed dispersion rather than treating it as a settled sampling error.

\subsection{Lorenz Drift Proxy Analysis}
\label{sec:lorenz-analysis}

While the full cross-census comparison awaits package completion,
