...
The current implementation reads tract-level demographics from the BISECT
demographics CSV and converts minority fraction times tract population into a
minority-population spatial proxy.  Where true VAP or CVAP files are available,
the same score should be recomputed on those denominators before making a
legal claim.
...
\begin{tabular}{@{}lcccl@{}}
\toprule
\textbf{State} & \textbf{$k$} & \textbf{Share} & \textbf{Group} & \textbf{Key cases} \\
\midrule
...
GeoSection (T.1) and VRASection (this paper) for each state, along with
MetisVra (B.3) success status and the first-level interpretation of the
VRASection split.  The table does not certify final Section 2 compliance.
...
\begin{tabular}{@{}lccclp{1.45in}@{}}
\toprule
\textbf{State} & \textbf{Geo} & \textbf{VRA}
  & \textbf{$A$}
  & \textbf{MetisVra} & \textbf{First-level interpretation} \\
\midrule
AL & 3:4 & \textbf{2:5} & 0.27 & No (0/28 configs) & Black Belt isolated \\
...
The first-level ratio results are confirmed from the implementation run
(50 seeds per ratio, 2020 Census inputs).
...
\paragraph{GeoSection result: 3:4 (near-equal split).}
GeoSection selects a $3:4$ split for Alabama (the confirmed natural ratio from T.1 Table~1), producing a near-equal geographic division rather than isolating Birmingham.
Although isoperimetric normalisation prevents the 1:6 caterpillar peel in Alabama
(Birmingham is not compact enough to win the normalised comparison), the 3:4 split
still leaves the Black Belt dispersed across both sub-regions,
where it can be divided in ways that dilute the Black Belt community's
voting strength.
The $1:6$ split is evaluated as a candidate ratio by VRASection (score = 350.2,
see CLI output below) but is not the GeoSection winner.
...
The $2:5$ result is empirically confirmed from the \texttt{bisect} CLI:
\begin{verbatim}
  bisect state --state AL --partition-mode vra-section --w-vra 0.40
...
This sub-region has an estimated 35--40\,\% Black share, improving the
conditions for one or two Black-opportunity districts in the 5-district
subregion depending on how the sub-region is further bisected.
Critically, the Black Belt community remains intact on one side of the
first bisection, preserving the geography relevant to Gingles Prong 1 before
any district boundaries are drawn.
...
\midrule
GeoSection  & 3:4 & 501.3 & --- & 501.3 & 0.0\,\% (baseline) \\
VRASection  & 2:5 & 448.7 & 0.42 & 322.5 & $-10.7$\,\% (VRA-aligned, lower EC) \\
\bottomrule
\end{tabular}
}
\end{table}
...
VRASection's hypothetical $1:6$ split\footnote{Note: the table above reports
GeoSection's $3:4$ as the confirmed natural ratio (T.1 Table~1)
...
The selected first-level split is therefore best read as a precondition result:
it keeps the Black Belt, Montgomery, and Mobile coastal areas on the same side
of the first bisection.  It does not by itself prove that the final 7-district
plan contains the two Black-opportunity districts required by
\emph{Allen v.\ Milligan}.  That claim requires an archived final partition,
district-level demographic analysis, and legally relevant VAP or CVAP
denominators.  The next empirical package should run
\texttt{bisect analyze --types demographic,bloc-voting} on the final plan and
report district-level opportunity evidence separately from the first-level
alignment score.

\paragraph{Interpretation.}
The EC premium of 4.3\,\% quantifies the compactness cost of the VRA-aligned
split.
A 4.3\,\% increase in normalised edge-cut is modest as an engineering
trade-off, but there is no universal legal percentage threshold for a map
modification motivated by Section 2.
...
A 4.3\,\% premium, combined with VRASection's design record that compactness
has the larger coefficient ($w_\text{vra} = 0.40$), provides evidence for a
defensible record.
