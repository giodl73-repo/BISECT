...
less directly than a race-targeting method under \emph{Shaw v.\ Reno}
\citep{shaw1993} --- but it means the algorithm has no mechanism to detect or
correct VRA shortfalls.
...
The reported results were damaging: MetisVra failed completely in 3 of 5 states
...
\item \textbf{Geographic alignment}: $A(\text{split}) = |\mathrm{MVAP}_\text{frac}(L)
  ...
  share of the current subgraph's minority-population signal placed on each
...
GeoSection selects a $3:4$ split (near-equal geographic division; see T.1 Table~1 for the confirmed AL GeoSection natural ratio).
The $2:5$ split better preserves the geographic concentration relevant to
Gingles Prong 1 because it
...
without imposing a district-level racial target.

\paragraph{The legal distinction.}
VRASection uses a race-associated demographic input, which raises legal risk
that must be documented rather than waved away.
...
The algorithm selects the bisection ratio that best exploits pre-existing
geographic concentration in the user-provided demographic signal.
At $w_\text{vra} = 0.40$, compactness is weighted $1 / (1 - 0.40) = 1.67\times$
more heavily than alignment.  That supports a geography-first record, but
\emph{Shaw}'s racial-predominance test remains fact-specific.
...
\item \textbf{VRASection algorithm} (\S\ref{sec:algorithm}): formal
  ...
  algorithm pseudocode, including legal analysis of how the design limits
  racial-predominance risk.

\item \textbf{Alabama case study} (\S\ref{sec:evaluation}): the $2:5$
  vs.\ $3:4$ comparison as the lead empirical result, with analysis of
