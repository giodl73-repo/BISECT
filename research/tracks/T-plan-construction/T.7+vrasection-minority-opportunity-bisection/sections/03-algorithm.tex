...
Let $\mathbf{m} \in \mathbb{R}_{\geq 0}^{|V|}$ be a per-tract minority
population signal.  In the current BISECT ratio-selection implementation,
$m_v$ is the tract's minority fraction multiplied by tract population; this is
a spatial proxy, not necessarily true VAP or CVAP.  When true VAP/CVAP is
available, the same formulas can be applied to those counts.
...
(2) \emph{Empirical}: sensitivity analysis (T.7 pending) suggests that
...
\subsection{Legal Design Boundary}
...
VRASection's design addresses this risk on two levels.  This is a design
argument, not a legal guarantee: courts evaluate the factual record of a map,
not merely the coefficients in an algorithm.
...
\item Use a district-level racial target as a hard constraint.
\end{itemize}

\paragraph{What the algorithm does.}
VRASection observes the spatial distribution of a user-designated
race-associated population attribute and rewards bisections that
separate high-concentration tracts from low-concentration tracts.
The defended interpretation is geographic: the algorithm
...
boundary separating minority-concentrated areas from dispersed areas.  That
signal is relevant to Gingles Prong 1, but it does not by itself establish the
full legal standard.
...
That is:
\begin{align*}
  \text{compactness contribution}
    &= (1 - w_\text{vra}) \cdot \mathrm{EC}_\text{norm}(i),\\
  \text{alignment contribution}
    &\leq w_\text{vra} \cdot \mathrm{EC}_\text{norm}(i).
\end{align*}
For $w_\text{vra} < 0.5$, the first term is larger.
Geographic compactness is quantifiably the larger term in this simplified
score decomposition for any $w_\text{vra} \in (0, 0.5)$.
The proposed default $w_\text{vra} = 0.40$ gives compactness 60\,\% weight
and alignment 40\,\% weight.  This supports a geography-first record; it does
not create a bright-line \emph{Shaw} threshold.
...
This raises \emph{Miller} predominance risk in states where race
...
selection score, which is a narrower design than direct maximisation.
...
\begin{remark}[Decoupled Optimization]
\label{rem:decoupled}
The current implementation evaluates the alignment score $A(\mathrm{split})$
on the minimum-EC bisection found for each seed, rather than jointly
optimising $\mathrm{EC}_\text{norm}$ and alignment.
Concretely, lines 7--10 of Algorithm~\ref{alg:vrasection} select $L_{\min}$
by $\min \mathrm{EC}(L)$; the alignment score of that compactness-optimal
bisection is then applied post-hoc as a ratio-selection input.
Seeds are searched for their compactness quality, not their alignment quality.

A joint optimisation would evaluate $A(\mathrm{split})$ for every seed
at every ratio and select the seed minimising
$\mathrm{score}(i, s) = \mathrm{EC}_\text{norm}(i,s) - w_\text{vra}
\cdot A(L_s) \cdot \max(\mathrm{EC}_\text{norm}(i,s),\, 1)$
directly across the joint (seed, ratio) space.
This would require evaluating the full score for all $N \cdot \lfloor k/2 \rfloor$
(seed, ratio) pairs --- identical METIS call count, but with alignment
computed for every seed rather than only the EC-minimising one.
The decoupled approach is a conservative approximation: by fixing the
compactness-optimal seed and reading off its alignment post-hoc,
VRASection may underestimate the achievable alignment at each ratio.
In the empirical results (Alabama 2:5 at $A = 0.42$), the approximation
is sufficient for the correct ratio to win by a substantial margin (27.7
score points).
The joint optimisation is left for future work; it would not change the
METIS call count but would change the seed-selection criterion.
\end{remark}
