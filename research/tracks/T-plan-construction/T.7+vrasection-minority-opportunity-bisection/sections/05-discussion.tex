...
\subsection{\texorpdfstring{$w_\text{vra}$}{w-vra} Sensitivity}
...
This regime is the more conservative exploratory setting when legal risk is
high and the state has no clear Section 2 obligation.
...
The coefficient record keeps compactness larger than the alignment term by a
20\,pp margin, which supports but does not decide the Shaw predominance inquiry.
...
The coefficient record no longer supports a geography-first argument at
$w_\text{vra} = 0.6$
...
$w_\text{vra}$ & \textbf{Compactness weight} & \textbf{Alignment weight}
  & \textbf{Coefficient record} & \textbf{Recommended use} \\
\midrule
0.0  & 100\,\% & 0\,\%   & Pure geography     & Pure compactness \\
0.2  & 80\,\%  & 20\,\%  & Geography larger   & Low-obligation states \\
0.4  & 60\,\%  & 40\,\%  & Geography larger   & \textbf{Default (Section 2 states)} \\
0.5  & 50\,\%  & 50\,\%  & Tied               & Not recommended \\
0.6  & 40\,\%  & 60\,\%  & Alignment larger   & Research only \\
1.0  & 0\,\%   & 100\,\% & Pure alignment     & Research only \\
\bottomrule
\end{tabular}
}
\end{table}

\subsection{The Shaw v. Reno Line and the Cooper v. Harris Counterfactual}
\label{sec:shaw-line}
...
\emph{Cooper v.\ Harris} (2017) clarified the operative inquiry: the court
asks whether the legislature ``placed a significant number of voters within
or without a particular district predominately because of their race.''
This is a counterfactual question, not an arithmetic one.
Courts do not inspect objective function coefficients; they ask whether
specific boundary choices are explicable on non-racial grounds.

\paragraph{The correct framework: counterfactual comparison.}
The controlling question under \emph{Shaw}/\emph{Miller}/\emph{Cooper} is:
\emph{Would the legislature have drawn this district this way even if it
were not trying to satisfy the VRA?}
VRASection can support this inquiry because the counterfactual map --- the pure
GeoSection result at $w_\text{vra} = 0$ --- is explicitly available as a
comparison point.
The counterfactual is not hypothetical; it is computed.

\begin{proposition}[VRASection counterfactual availability]
\label{prop:counterfactual}
VRASection with $w_\text{vra} = 0.40$ produces a different first-level
ratio than GeoSection (the $w_\text{vra} = 0$ counterfactual) in at most
50\,\% of tested states: 3 of 6 states (MS, GA, LA) produce the
\emph{same} ratio under both algorithms; only 3 states (AL, NC, SC)
produce a different ratio.
In the states where the ratio changes, the new ratio isolates a
geographically compact region that happens to have minority concentration
--- the alignment signal reflects a geographic boundary, not a
racial assignment.
\end{proposition}

\noindent
For the three states where VRASection and GeoSection agree (MS, GA, LA),
the alignment score did not change the outcome: the geographically optimal
split and the minority-aligned split are the same split.
This is evidence that VRASection is often detecting geographic structure that
is already consistent with compact redistricting, rather than forcing a
different racial target in every state.

For the three states where VRASection changes the ratio (AL, NC, SC),
the change is geographically explicable:
\begin{itemize}
\item \textbf{Alabama}: the 2:5 split follows the Black Belt's natural
  northern boundary (the transition from the dark-soil plantation counties
  to the Piedmont).
  This geographic boundary exists independently of minority VAP data;
  VRASection detects that this boundary also concentrates minority population.
\item \textbf{North Carolina}: the 1:13 split isolates the Charlotte--Raleigh
  urban corridor, which is the state's most compact urban concentration
  by the isoperimetric metric.
\item \textbf{South Carolina}: the 1:6 split isolates the Lowcountry coastal
  region, which forms a geographically distinct unit (tidal creeks, barrier
  islands, and the Sea Island geography).
\end{itemize}

\paragraph{The objective function weight as supporting evidence.}
The $w_\text{vra} < 0.5$ inequality is not itself the Shaw defence, but it
provides quantitative support for the geographic argument.
...
\]
At $w_\text{vra} = 0.40$ and the empirical alignment score $A = 0.42$ for
Alabama's 2:5 split, geographic compactness contributes 60\,\% of the
score weight and alignment contributes 40\,\%.
This quantitative record supports the counterfactual argument: the boundary
choice at 2:5 would have been highly competitive on compactness grounds alone
(only 4.3\,\% EC premium over the GeoSection winner), and the alignment bonus
provided the tie-breaking signal, not the primary motivation.

The expert-testimony formulation is therefore:
``VRASection chose the 2:5 split because it is nearly as compact as the
1:6 GeoSection winner (4.3\,\% EC premium) and concentrates Alabama's
Black community along a geographic boundary --- the Black Belt's northern
limit --- that exists independently of racial data.
The counterfactual GeoSection map is available for comparison.
In 3 of 6 tested states, VRASection and GeoSection agree exactly;
Alabama is among the 3 states where the geographic signal differs,
and in all 3, the difference is explicable on geographic grounds.''
...
Gingles Prong 1 signal for this algorithm to use, and a standard
compact-district algorithm is the less race-conscious choice.
...
\subsection{\texorpdfstring{Limitation: Alignment Does Not Guarantee $> 50\,\%$ Minority VAP}{Limitation: alignment does not guarantee majority-minority districts}}
...
The BISECT codebase's Callais evidence layer provides
WLS+HC3 regression infrastructure for validating racial bloc voting
under the \emph{Callais v.\ Landry} evidence framework.
