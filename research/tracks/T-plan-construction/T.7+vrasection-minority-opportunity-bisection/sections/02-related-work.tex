...
VRASection operationalises one proxy for Prong 1: the alignment score $A$
measures geographic concentration in the supplied demographic signal.  A
threshold such as $A \geq 0.15$ can flag communities for review, but it is not
itself a legal Prong 1 finding.
...
VRASection's $w_\text{vra}$ parameter makes part of the design record
quantitative:
at $w_\text{vra} < 0.5$, the compactness component of the selection score
exceeds the alignment component for any ratio.  This supports a
geography-first explanation, but it does not ensure the legal conclusion
(Section~\ref{sec:legal-analysis}).
...
The case illustrates the tension between Section 2 remedies and traditional
redistricting criteria.
VRASection is designed to produce plan candidates for this kind of review:
...
cores, producing the caterpillar pathology identified in T.1 \citep{b8paper}.
...
The algorithm rewards natural geographic concentration relevant to Gingles
Prong 1 while preserving compactness as the larger objective term relevant to
Shaw.
...
\paragraph{GeoSection (T.1): the isoperimetric ratio scan.}
