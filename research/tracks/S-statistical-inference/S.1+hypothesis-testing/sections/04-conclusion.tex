...
\begin{enumerate}
  ...
  \item \textbf{10{,}000-step ensembles are reliable for most states.}
    ...
    Texas achieves $p = 0.03$ at 10K steps (significant at 5\% but not 1\%);
    30K steps are needed for $p < 0.01$ reliability in TX (S.3).
    Note: TX's 1K-step result ($p = 0.11$) is not significant --- consistent with the
    abstract's finding that 1K-step ensembles are insufficient for TX.
